...
//-----------------------------------------------------------------------------
void Node::SetNodeNo(int _nn)
{
  global_node_number = _nn;
}
//-----------------------------------------------------------------------------
...
bool Node::CommonCell(Node *n1, Node *n2, int thiscell, int *cellnumber)
{
  ...
  for (int i=0;i<_nc;i++){
	 ...
	 if ( c1 == thiscell )
		continue;
	 for (int j=0;j<(n1->_nc);j++){
		c2 = n1->neighbor_cells[j];
		for (int k=0;k<(n2->_nc);k++){
