...
namespace dolfin
{
  ...
  /// in the y-direction of a piecewise linear function represented in
  ...
  /// Function u(mesh, x);
  /// DyMatrix Dy(mesh);
  /// real dudy = Dy.mult(x, i);

  class DyMatrix : public Matrix
  {
  public:
  
    /// Construct linear mapping for y-derivative on a given mesh
    DyMatrix(Mesh& mesh);
