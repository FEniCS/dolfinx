...
namespace dolfin {
  ...
  public:
	 ...
	 friend LogStream& operator<<(LogStream& stream, const List<T> &list)
	 {
		...
		stream << list._blocks*BLOCK_SIZE << " objects allocated in " << list._blocks << " blocks.]";
	 ...
	 public:
		...
		/// Output
		friend LogStream& operator<<(LogStream& stream, const Iterator &it)
		{
		  return stream << "block = " << it.block << " pos = " << it.pos;
		}
  ...
  protected:
	 ...
	 void iterator_start(Iterator *it){
		...
		dolfin_error("Unable to find first object in list.");
	 }
