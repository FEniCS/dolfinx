...
for d in `cat $MODULES | grep -v '#'`; do
	...
	if [ $GRID = 'yes' ]; then
		...
		echo "        display->Error(\"Solver for problem \\\"%s\\\" needs a grid.\",problem);" >> $HEADER
	else
		echo "    if ( strcasecmp(problem,\"$KEYWORD\") == 0 )" >> $HEADER
...
echo "" >> $HEADER
echo "    display->Error(\"Could not find any matching solver for problem \\\"%s\\\".\",problem);" >> $HEADER
echo "" >> $HEADER
...
echo "" >> $HEADER
echo "    display->Error(\"Could not find any matching settings for problem \\\"%s\\\".\",problem);" >> $HEADER
echo "" >> $HEADER
echo "    return 0;" >> $HEADER
echo "}" >> $HEADER
