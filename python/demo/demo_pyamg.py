...
# Now try an elasticity problem on the same mesh
...
bc = dirichletbc(
    np.zeros(3, dtype=dtype), locate_dofs_topological(V, entity_dim=tdim - 1, entities=facets), V=V
)
...
with io.XDMFFile(mesh.comm, "out_pyamg/elasticity.xdmf", "w") as file:
    file.write_mesh(mesh)
    file.write_function(uh)
