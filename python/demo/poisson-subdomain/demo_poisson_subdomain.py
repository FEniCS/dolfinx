...
# and tag different regions within the domain. For details on how to create the 
# mesh and tag different regions the reader is reffered to :ref:`Tagging mesh entities demo.
# <demo_tagging_mesh_entities>`. In this demo we begin by creating a Mesh Value Collection 
# from the XDMF file. Since mixed topology elements are not allowed in FEniCS we have to separate
# the mesh entities of different dimensions into different XDMF files. This is achieved via ::
