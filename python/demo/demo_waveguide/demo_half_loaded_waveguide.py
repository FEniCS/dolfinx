...
# Now we need to specify the type of our problem.
# Indeed, SLEPc can sometimes exploit symmetries in our system of equations to accelerate the computation. SLEPc provides
# some routines to check if our problem has any of these symmetries. Let's check it:

print(eps.isGeneralized()) # return False
print(eps.isHermitian()) # return False
print(eps.isPositive()) # return False

# To Igor, Jorgen and Jack: the `eps.isGeneralized` function returns False too, shouldn't it be `True` in our case?
#
# Since we have no symmetry to exploit, we can set our problem as a generalized non-Hermitian problem:
