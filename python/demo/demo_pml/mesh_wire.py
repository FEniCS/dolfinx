"""Gmsh mesh generation for the wire/PML geometry. Mesh generation with GMSH
...
def generate_mesh_wire(
    ...
    # Marker interior surface in background group
    boundaries = []
...
def create_dolfinx_mesh(mesh_params, tags, comm=MPI.COMM_WORLD):
    """Generate the Gmsh model on rank 0 and convert it to a DOLFINx mesh."""
    model = None
    ...
    if comm.rank == 0:
        model = generate_mesh_wire(
            ...
            mesh_params.l_dom,
            mesh_params.l_pml,
            mesh_params.in_wire_size,
            mesh_params.on_wire_size,
            mesh_params.scatt_size,
            mesh_params.pml_size,
            tags.au,
            tags.bkg,
            tags.scatt,
            tags.pml,
    ...
    mesh_data = gmshio.model_to_mesh(model, comm, 0, gdim=2, partitioner=partitioner)
