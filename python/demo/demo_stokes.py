...
def nested_iterative_solver():
    ...
    # The pressure field is determined only up to a constant. We can
    # supply the vector that spans the nullspace to the solver,  and any
    # component of the solution in this direction will be eliminated
    ...
    # using PETSc's additive fieldsplit type preconditioner
...
def block_operators():
    """Return block operators and block RHS vector."""
...
def block_solver():
    """Solve a block matrix problem using an iterative solver"""
    ...
    # Set block size on the top-left block of the preconditioner so that
    # GAMG uses the appropriate near nullspace
    ksp.getPC().setUp()
...
#
# Solve same problem, but now with monolithic matrices and a direct
# solver.


def block_monlithic():
    """Solve a block matrix problem using a direct solver"""
    ...
    # Create a block vector (x) to store the full, and solve
    x = A.createVecLeft()
    ksp.solve(b, x)
...
#
# We solve the same problem with a non-blocked direct solver approach.
