...
if MPI.COMM_WORLD.rank == 0:
    model = generate_mesh_wire(
        radius_wire, radius_dom, in_wire_size, on_wire_size, bkg_size, boundary_size, au_tag, bkg_tag, boundary_tag
    )
...
F = (
    ...
    + (1j * k0 * n_bkg + 1 / (2 * r)) * ufl.inner(ufl.cross(Es_3d, n_3d), ufl.cross(v_3d, n_3d)) * dsbc
)
