...
def write(mesh: Mesh, filename: str | Path):
    ...
    with h5py.File(fname, "w", driver="mpio", comm=comm) as inf:
        ...
        # accumulate offsets, and remove first element for every cell type except the first
        if len(cell_maps) > 1:
        ...
        # Store meta-variable used by VTKHDF5 when partitioning data. We do not partition data, as then everything
        # becomes "local index", and cannot be read in again in parallel on M processes.
        hdf.create_dataset(
            "NumberOfCells",
            (1,),
            dtype=np.int64,
