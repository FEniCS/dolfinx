...
def background_field_rz(theta: float, n_bkg: float, k0: float, m: int, x):
    k = k0 * n_bkg
    a_r = np.cos(theta) * np.exp(1j * k * x[1] * np.cos(theta)) * (1j) ** (-m + 1) * jvp(m, k * x[0] * np.sin(theta), 1)
    a_z = np.sin(theta) * np.exp(1j * k * x[1] * np.cos(theta)) * (1j) ** -m * jv(m, k * x[0] * np.sin(theta))
    return (a_r, a_z)
...
pml_coords = ufl.as_vector(
    (pml_coordinate(rho, r, alpha, k0, radius_dom, radius_pml), pml_coordinate(z, r, alpha, k0, radius_dom, radius_pml))
)
...
incident_cells = mesh.compute_incident_entities(msh.topology, scatt_facets, msh.topology.dim - 1, msh.topology.dim)
...
for m in m_list:
    ...
    rotate_to_phi = ufl.as_vector((phase + ufl.conj(phase), phase + ufl.conj(phase), phase - ufl.conj(phase)))
