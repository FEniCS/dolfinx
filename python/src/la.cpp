  ...
  void la(py::module& m)
  {
    ...
    py::class_<dolfin::LinearAlgebraObject, std::shared_ptr<dolfin::LinearAlgebraObject>,
      ...
      .def("mpi_comm", [](dolfin::LinearAlgebraObject& self)
        { return MPICommunicatorWrapper(self.mpi_comm()); });
    ...
    py::class_<dolfin::DefaultFactory, std::shared_ptr<dolfin::DefaultFactory>>
      ...
      .def("create_matrix", [](const dolfin::DefaultFactory &self, const MPICommunicatorWrapper & comm)
        { return self.create_matrix(comm.get()); })
      .def("create_vector", [](const dolfin::DefaultFactory &self, const MPICommunicatorWrapper & comm)
    ...
    py::class_<dolfin::EigenFactory, std::shared_ptr<dolfin::EigenFactory>,
      ...
      .def("create_matrix", [](const dolfin::EigenFactory &self, const MPICommunicatorWrapper & comm)
        { return self.create_matrix(comm.get()); })
      .def("create_vector", [](const dolfin::EigenFactory &self, const MPICommunicatorWrapper & comm)
        { return self.create_vector(comm.get()); });
    ...
    py::class_<dolfin::PETScFactory, std::shared_ptr<dolfin::PETScFactory>,
      ...
      .def("create_matrix", [](const dolfin::PETScFactory &self, const MPICommunicatorWrapper & comm)
        { return self.create_matrix(comm.get()); })
      .def("create_vector", [](const dolfin::EigenFactory &self, const MPICommunicatorWrapper & comm)
        { return self.create_vector(comm.get()); });
    ...
    py::class_<dolfin::TpetraFactory, std::shared_ptr<dolfin::TpetraFactory>,
      ...
      .def("create_matrix", [](const dolfin::TpetraFactory &self, const MPICommunicatorWrapper & comm)
        { return self.create_matrix(comm.get()); })
      .def("create_vector", [](const dolfin::TpetraFactory &self, const MPICommunicatorWrapper & comm)
        { return self.create_vector(comm.get()); });
    ...
    py::class_<dolfin::TpetraVector, std::shared_ptr<dolfin::TpetraVector>,
      ...
      .def(py::init([](const MPICommunicatorWrapper & comm=MPICommunicatorWrapper(MPI_COMM_WORLD))
        { return std::unique_ptr<dolfin::TpetraVector>(new dolfin::TpetraVector(comm.get())); }),
        py::arg("comm")=MPICommunicatorWrapper(MPI_COMM_WORLD))
      .def(py::init([](const MPICommunicatorWrapper & comm, std::size_t N)
