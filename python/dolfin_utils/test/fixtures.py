...
def _create_tempdir(worker_id, request):
    ...
    outputname = testfilename.replace(".py", "_tempdir_{}".format(worker_id))
...
def tempdir(worker_id, request):
    ...
    """
    gc_barrier()
    return _create_tempdir(worker_id, request)


@pytest.yield_fixture(scope="function")
def cd_tempdir(worker_id, request):
    """Return a unique directory name for this test function instance.
    ...
    gc_barrier()
    cwd = os.getcwd()
    path = _create_tempdir(worker_id, request)
    os.chdir(path)
    yield path
    os.chdir(cwd)
