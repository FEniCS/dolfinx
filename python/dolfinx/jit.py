...
def get_parameters(priority_parameters: Optional[dict] = None) -> dict:
    ...
    Notes
    -----
    The dolfinx_parameters.json files are cached on the first call. Subsequent
...
def ffcx_jit(ufl_object, form_compiler_parameters={}, jit_parameters={}):
    ...
    form_compiler_parameters
      ...
      to see all available options. Takes priority over all other parameter values, except
      for 'scalar_type' which is determined by DOLFINX.
    jit_parameters
      Parameters used in CFFI JIT compilation of C code generated by FFCX. See
      python/dolfinx/jit.py for all available parameters. Takes priority over
