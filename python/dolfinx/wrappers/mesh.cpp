...
void declare_meshtags(py::module& m, std::string type)
{
  ...
  py::class_<dolfinx::mesh::MeshTags<T>,
      ...
      .def("find",
           [](dolfinx::mesh::MeshTags<T>& self, T value)
           {
             std::vector<std::int32_t> indices = self.find(value);
             return py::array_t<std::int32_t>(indices.size(), indices.data());
           });
...
void mesh(py::module& m)
{
  ...
  m.def("exterior_facet_indices", [](const dolfinx::mesh::Topology& t)
