...
void mesh(py::module& m)
{
  ...
  py::class_<dolfinx::mesh::Geometry, std::shared_ptr<dolfinx::mesh::Geometry>>(
      ...
      .def_property_readonly("cmaps", &dolfinx::mesh::Geometry::cmaps,
                             "The coordinate maps")
  ...
  using PythonCellPartitionFunction
      = std::function<dolfinx::graph::AdjacencyList<std::int32_t>(
          ...
          const dolfinx::graph::AdjacencyList<std::int64_t>&,
          dolfinx::mesh::GhostMode)>;
  m.def("create_cell_partitioner", []() -> PythonCellPartitionFunction {
    return create_cell_partitioner_py(dolfinx::mesh::create_cell_partitioner());
  });
