...
void declare_adjacency_list_init(nb::module_& m, std::string type)
{
  ...
  nb::class_<dolfinx::graph::AdjacencyList<std::vector<T>, U>>(
      m, pyclass_name.c_str(), "Adjacency List")
      .def(
          "__init__",
          [](dolfinx::graph::AdjacencyList<std::vector<T>, U>* a,
             nb::ndarray<const T, nb::ndim<1>, nb::c_contig> adj)
          {
            std::vector<T> data(adj.data(), adj.data() + adj.size());
            new (a) dolfinx::graph::AdjacencyList<std::vector<T>, U>(
      ...
      .def(
          "links",
          [](const dolfinx::graph::AdjacencyList<std::vector<T>, U>& self,
             int i)
          {
      ...
      .def_prop_ro("num_nodes",
                   &dolfinx::graph::AdjacencyList<std::vector<T>, U>::num_nodes)
      .def("__eq__",
           &dolfinx::graph::AdjacencyList<std::vector<T>, U>::operator==,
           nb::is_operator())
      .def("__repr__", &dolfinx::graph::AdjacencyList<std::vector<T>, U>::str)
      .def("__len__",
           &dolfinx::graph::AdjacencyList<std::vector<T>, U>::num_nodes);
}
