...
void fem(py::module& m)
{
  ...
  py::class_<dolfinx::fem::CoordinateElement,
    ...
    dolfinx::array2d<double> _X(X.shape()[0], X.shape()[1]),
        _cell_geometry(cell_geometry.shape()[0], cell_geometry.shape()[1]);
  ...
  dirichletbc
    ...
    return dolfinx::fem::DirichletBC<PetscScalar>(
        g, std::vector<std::int32_t>(dofs.data(), dofs.data() + dofs.size()));
          }))
      .def(py::init(
          [](const std::shared_ptr<const dolfinx::fem::Function<PetscScalar>>&
                 g,
    ...
    std::array dofs
        = {std::vector<std::int32_t>(V_g_dofs[0].data(),
                                     V_g_dofs[0].data() + V_g_dofs[0].size()),
           std::vector<std::int32_t>(V_g_dofs[1].data(),
  ...
  m.def("assemble_matrix_petsc",
    ...
    dolfinx::fem::assemble_matrix(dolfinx::la::PETScMatrix::add_block_fn(A), a,
                                  bcs);
        });
  m.def("assemble_matrix_petsc",
        [](Mat A, const dolfinx::fem::Form<PetscScalar>& a,
  ...
  m.def("assemble_matrix_petsc_unrolled",
        [](Mat A, const dolfinx::fem::Form<PetscScalar>& a,
           const std::vector<std::shared_ptr<
               const dolfinx::fem::DirichletBC<PetscScalar>>>& bcs) {
    dolfinx::fem::assemble_matrix(dolfinx::la::PETScMatrix::add_block_expand_fn(
  ...
  m.def("add_diagonal",
    ...
    dolfinx::fem::add_diagonal(dolfinx::la::PETScMatrix::add_fn(A), V, bcs,
                               diagonal);
        });
  m.def(
      "assemble_matrix",
  ...
  m.def(
    ...
    if (x0.ndim() == 0)
    {
      dolfinx::fem::set_bc<PetscScalar>(xtl::span(b.mutable_data(), b.size()),
                                        bcs, scale);
    }
    else if (x0.ndim() == 1)
    {
      dolfinx::fem::set_bc<PetscScalar>(xtl::span(b.mutable_data(), b.size()),
                                        bcs, xtl::span(x0.data(), x0.shape(0)),
                                        scale);
    }
    else
  ...
  py::class_<dolfinx::fem::Form<PetscScalar>,
    ...
    return dolfinx::fem::Form<PetscScalar>(spaces, _integrals, coefficients,
  ...
  py::class_<dolfinx::fem::Function<PetscScalar>,
    ...
    auto _f = [&f](const xt::xtensor<double, 2>& x) -> xt::xarray<PetscScalar> {
      auto strides = x.strides();
      std::transform(strides.begin(), strides.end(), strides.begin(),
                     [](auto s) { return s * sizeof(double); });
      py::array_t _x(x.shape(), strides, x.data(), py::none());
    ...
    const std::function<void(PetscScalar*, int, int, const double*)> f
        = reinterpret_cast<void (*)(PetscScalar*, int, int, const double*)>(
            addr);

    auto _f = [&f](xt::xarray<PetscScalar>& values,
                   const xt::xtensor<double, 2>& x) -> void {
      f(values.data(), int(values.shape(1)), int(values.shape(0)), x.data());
    };
    ...
    auto cell_map = self.function_space()->mesh()->topology().index_map(tdim);
    assert(cell_map);
    ...
    const auto x = dolfinx::fem::interpolation_coords(
        *self.function_space()->element(), *self.function_space()->mesh(),
        cells);
  ...
  py::class_<dolfinx::fem::Expression<PetscScalar>,
    ...
    auto tabulate_expression_ptr
        = (void (*)(PetscScalar*, const PetscScalar*, const PetscScalar*,
                    const double*))addr.cast<std::uintptr_t>();
    dolfinx::array2d<double> _X(X.shape()[0], X.shape()[1]);
    std::copy_n(X.data(), X.size(), _X.data());
    return dolfinx::fem::Expression<PetscScalar>(
        coefficients, constants, mesh, _X, tabulate_expression_ptr, value_size);
               }),
    ...
    dolfinx::array2d<PetscScalar> _values(
        active_cells.shape()[0], self.num_points() * self.value_size());
    self.eval(xtl::span(active_cells.data(), active_cells.size()), _values);
