...
// From https://github.com/pybind/pybind11/issues/1042
template <typename T>
inline py::array_t<T> as_pyarray2d(dolfinx::ndarray<T, 2>&& array)
{
  auto shape = array.shape;
  auto strides = array.strides();
  auto data = array.data();
  std::unique_ptr<dolfinx::ndarray<T, 2>> array_ptr
      = std::make_unique<dolfinx::ndarray<T, 2>>(std::move(array));
  auto capsule = py::capsule(array_ptr.get(), [](void* p) {
    std::unique_ptr<dolfinx::ndarray<T, 2>>(
        reinterpret_cast<dolfinx::ndarray<T, 2>*>(p));
  });
  array_ptr.release();
