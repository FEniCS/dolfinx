...
void refinement(nb::module_& m)
{
  ...
  m.def(
      ...
      [](const dolfinx::mesh::Mesh<double>& mesh,
         const nb::ndarray<std::int32_t>& edges, bool redistribute)
      {
        assert(edges.ndim() == 1);
        return dolfinx::refinement::refine(
            mesh, std::span(edges.data(), edges.shape(0)), redistribute);
      },
  ...
  m.def(
      ...
      [](const dolfinx::mesh::Mesh<double>& mesh0,
         nb::ndarray<std::int32_t> edges, bool redistribute,
         dolfinx::refinement::plaza::Option option)
      {
        assert(edges.ndim() == 1);
        auto [mesh1, cell, facet] = dolfinx::refinement::plaza::refine(
            mesh0, std::span<const std::int32_t>(edges.data(), edges.shape(0)),
            redistribute, option);
        return std::tuple{std::move(mesh1), as_nbarray(std::move(cell)),
                          as_nbarray(std::move(facet))};
      },
  ...
  m.def(
      ...
      [](const dolfinx::mesh::MeshTags<std::int32_t>& parent_meshtag,
         ...
         const nb::ndarray<std::int32_t>& parent_cell,
         const nb::ndarray<std::int8_t>& parent_facet)
      {
