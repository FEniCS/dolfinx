...
void refinement(nb::module_& m)
{
  ...
  m.def(
      ...
      [](const dolfinx::mesh::Mesh<float>& mesh,
         const nb::ndarray<std::int32_t, nb::numpy>& edges, bool redistribute)
      {
        assert(edges.ndim() == 1);
  ...
  m.def(
      ...
      [](const dolfinx::mesh::MeshTags<std::int32_t>& parent_meshtag,
         ...
         const nb::ndarray<std::int32_t, nb::numpy>& parent_cell,
         const nb::ndarray<std::int8_t, nb::numpy>& parent_facet)
      {
