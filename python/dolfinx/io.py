...
_dolfin_to_vtk_cell = {cpp.mesh.CellType.point: 1, cpp.mesh.CellType.interval: 3,
                       cpp.mesh.CellType.triangle: 5, cpp.mesh.CellType.quadrilateral: 9,
                       cpp.mesh.CellType.tetrahedron: 10, cpp.mesh.CellType.hexahedron: 12}
# NOTE: Should be using arbitrary order lagrangian cells, see:
...
def mesh_to_pyvista(mesh, dim):
    ...
    cells = cpp.mesh.entities_to_geometry(mesh, dim, numpy.arange(num_cells), False)

    # Get VTK cell type
    dolfin_cell_type = cpp.mesh.cell_entity_type(mesh.topology.cell_type, dim)
    cell_types = numpy.full(num_cells, _dolfin_to_vtk_cell[dolfin_cell_type])
    ...
    flattened_cells = numpy.hstack([numpy.full((num_cells, 1), cells.shape[1]), cells]).reshape(-1)
