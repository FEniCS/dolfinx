...
def _(A: PETSc.Mat,
    ...
    if _a.function_spaces[0].id == _a.function_spaces[1].id:
        A.assemblyBegin(MatAssemblyType.FLUSH)
        A.assemblyEnd(MatAssemblyType.FLUSH)
        ...
        for j, a_sub in enumerate(a_row):
            if a_sub is not None:
                ...
                if a_sub.function_spaces[0].id == a_sub.function_spaces[1].id:
                    Asub.assemblyBegin(MatAssemblyType.FLUSH)
                    Asub.assemblyEnd(MatAssemblyType.FLUSH)
                    cpp.fem.insert_diagonal_block(Asub, a_sub.function_spaces[0], bcs, diagonal)
