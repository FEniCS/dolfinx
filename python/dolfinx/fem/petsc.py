...
class BlockSNESProblem:
    def __init__(
        self,
        F: typing.Union[list[dolfinx.fem.Form], list[ufl.form.Form]],
        u: list[dolfinx.fem.Function],
        J: typing.Optional[
            typing.Union[list[list[dolfinx.fem.Form]], list[list[ufl.form.Form]], None]
        ] = None,
        P: typing.Optional[
            typing.Union[list[list[dolfinx.fem.Form]], list[list[ufl.form.Form]], None]
        ] = None,
        ...
        assert len(F) == len(u), "Number of residuals and unknowns must match"
        assert len(u) > 0, "At least one unknown must be provided"
        self._L = _create_form(
            F, form_compiler_options=form_compiler_options, jit_options=jit_options
        )
    ...
    def L(self) -> list[Form]:
        """Compiled linear forms of the residual :math:`F_i(u, v), i=0,...,N`"""
        return self._L

    @property
    def a(self) -> list[list[Form]]:
        """Compiled bilinear form of the Jacobian :math:`J_ij, i,j=0,...,N`"""
        return self._a

    @property
    def P(self) -> Form:
        """Compiled bilinear form of the preconditioner :math:`P_ij, i,j=0,...,N`"""
        return self._a_prec
