...
def test_custom_partitioner(tempdir, Nx, cell_type):
    ...
    mesh = dolfinx.BoxMesh(comm, points, [Nx, Nx, Nx], cell_type, GhostMode.shared_facet)
    ...
    with XDMFFile(MPI.COMM_WORLD, filename, "r") as file:
        ...
        topology = file.read_topology_data()
    ...
    def partitioner(*args):
        midpoints = np.mean(x_global[topology], axis=1)
        dest = np.floor(midpoints[:, 0] % comm.size).astype(np.int32)
        return dolfinx.cpp.graph.AdjacencyList_int32(dest)

    new_mesh = dolfinx.mesh.create_mesh(comm, topology, x, domain, GhostMode.none, partitioner)
    ...
    assert num_cells > 0
    assert np.all(cell_midpoints[:, 0] >= comm.rank)
    assert np.all(cell_midpoints[:, 0] <= comm.rank + 1)
