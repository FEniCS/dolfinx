...
def test_ghost_vertex_1d():
    mesh = create_unit_interval_mesh(MPI.COMM_WORLD, 20, ghost_mode=GhostMode.shared_vertex)
...
def test_ghost_facet_1d():
    N = 40
    mesh = create_unit_interval_mesh(MPI.COMM_WORLD, N, ghost_mode=GhostMode.shared_facet)
...
def test_ghost_3d(mode):
    N = 2
    num_cells = N * N * N * 6
    mesh = create_unit_cube_mesh(MPI.COMM_WORLD, N, N, N, ghost_mode=mode)
    if mesh.comm.size > 1:
...
def test_ghost_connectivities(mode):
    # Ghosted mesh
    meshG = create_unit_square_mesh(MPI.COMM_WORLD, 4, 4, ghost_mode=mode)
    meshG.topology.create_connectivity(1, 2)

    # Reference mesh, not ghosted, not parallel
    meshR = create_unit_square_mesh(MPI.COMM_SELF, 4, 4, ghost_mode=GhostMode.none)
