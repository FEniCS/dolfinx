...
def create_tensor_product_element(cell_type, degree, variant):
    ...
    element = basix.create_element(family, cell_type, degree, variant, dof_ordering=dof_ordering)
    return element
...
def test_quad_rect():
    ...
    element = create_tensor_product_element(
        basix.CellType.quadrilateral, 1, basix.LagrangeVariant.gll_warped
    )
    cmap = CoordinateElement_float64(element._e)
    _mesh = create_quad_rectangle_float64(
        MPI.COMM_WORLD, [[0.0, 0.0], [1.0, 1.0]], [20, 20], cmap, None
    )
