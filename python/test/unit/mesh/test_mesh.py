...
def mesh1d():
    ...
    mesh1d = create_unit_interval_mesh(MPI.COMM_WORLD, 4)
...
def mesh3d():
    ...
    mesh3d = create_unit_cube_mesh(MPI.COMM_WORLD, 1, 1, 1)
...
@pytest.fixture
def c0(mesh3d):
    """Original tetrahedron from create_unit_cube_mesh(MPI.COMM_WORLD, 1, 1, 1)"""
...
@pytest.fixture
def interval():
    return create_unit_interval_mesh(MPI.COMM_WORLD, 18)
...
@pytest.fixture
def cube():
    return create_unit_cube_mesh(MPI.COMM_WORLD, 3, 3, 3)


@pytest.fixture
def box():
    return create_box_mesh(MPI.COMM_WORLD, [np.array([0, 0, 0]),
...
@pytest.fixture
def mesh():
    return create_unit_square_mesh(MPI.COMM_WORLD, 3, 3)
...
def test_create_unit_square_meshDistributed():
    """Create mesh of unit square."""
    mesh = create_unit_square_mesh(MPI.COMM_WORLD, 5, 7)
...
def test_create_unit_square_meshLocal():
    """Create mesh of unit square."""
    mesh = create_unit_square_mesh(MPI.COMM_SELF, 5, 7)
    assert mesh.topology.index_map(0).size_global == 48
    assert mesh.topology.index_map(2).size_global == 70
    assert mesh.geometry.dim == 2


def test_create_unit_cube_meshDistributed():
    """Create mesh of unit cube."""
    mesh = create_unit_cube_mesh(MPI.COMM_WORLD, 5, 7, 9)
...
def test_create_unit_cube_meshLocal():
    """Create mesh of unit cube."""
    mesh = create_unit_cube_mesh(MPI.COMM_SELF, 5, 7, 9)
    assert mesh.topology.index_map(0).size_global == 480
...
def test_UnitQuadMesh():
    mesh = create_unit_square_mesh(MPI.COMM_WORLD, 5, 7, CellType.quadrilateral)
    assert mesh.topology.index_map(0).size_global == 48
...
def test_UnitHexMesh():
    mesh = create_unit_cube_mesh(MPI.COMM_WORLD, 5, 7, 9, CellType.hexahedron)
    assert mesh.topology.index_map(0).size_global == 480
...
def test_create_box_meshPrism():
    mesh = create_box_mesh(MPI.COMM_WORLD, [[0., 0., 0.], [1., 1., 1.]], [2, 3, 4], CellType.prism, GhostMode.none)
...
mesh_factories = [
    ...
    (create_unit_square_mesh, (MPI.COMM_WORLD, 4, 4)),
    (create_unit_cube_mesh, (MPI.COMM_WORLD, 2, 2, 2)),
    (create_unit_square_mesh, (MPI.COMM_WORLD, 4, 4, CellType.quadrilateral)),
    (create_unit_cube_mesh, (MPI.COMM_WORLD, 2, 2, 2, CellType.hexahedron)),
...
def xfail_ghosted_quads_hexes(mesh_factory, ghost_mode):
    ...
    if mesh_factory in [create_unit_square_mesh, create_unit_cube_mesh]:
...
def xtest_mesh_topology_lifetime():
    ...
    mesh = create_unit_square_mesh(MPI.COMM_WORLD, 4, 4)
