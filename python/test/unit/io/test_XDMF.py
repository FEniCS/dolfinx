...
def test_save_1d_scalar(tempdir, encoding):
    ...
    u = Function(V)
    u.vector().vec().set(1.0 + (1j if has_petsc_complex else 0))
...
def test_save_3d_vector_series(tempdir, encoding):
    ...
    with XDMFFile(mesh.mpi_comm(), filename, encoding=encoding) as file:
        u.vector().vec().set(1.0 + (1j if has_petsc_complex else 0))
        file.write(u, 0.1)
        u.vector().vec().set(2.0 + (2j if has_petsc_complex else 0))
        file.write(u, 0.2)
        u.vector().vec().set(3.0 + (3j if has_petsc_complex else 0))
        file.write(u, 0.3)
