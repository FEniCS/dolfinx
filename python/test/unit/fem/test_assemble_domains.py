...
from dolfinx.fem import (Constant, DirichletBC, Function, FunctionSpace,
                         ...
                         assemble_vector, create_form, set_bc)
...
def test_assembly_dx_domains(mode):
    ...
    # Assemble matrix
    a = create_form(w * ufl.inner(u, v) * (dx(1) + dx(2) + dx(3)))
    A = assemble_matrix(a)
    A.assemble()
    ...
    # Assemble vector
    L = create_form(ufl.inner(w, v) * (dx(1) + dx(2) + dx(3)))
    b = assemble_vector(L)
    ...
    L2 = create_form(ufl.inner(w, v) * dx)
    ...
    # Assemble scalar
    L = create_form(w * (dx(1) + dx(2) + dx(3)))
    s = assemble_scalar(L)
...
def test_assembly_ds_domains(mode):
    ...
    # Assemble matrix
    a = create_form(w * ufl.inner(u, v) * (ds(1) + ds(2) + ds(3) + ds(6)))
    A = assemble_matrix(a)
    A.assemble()
    norm1 = A.norm()
    a2 = create_form(w * ufl.inner(u, v) * ds)
    A2 = assemble_matrix(a2)
    A2.assemble()
    norm2 = A2.norm()
    ...
    # Assemble vector
    L = create_form(ufl.inner(w, v) * (ds(1) + ds(2) + ds(3) + ds(6)))
    b = assemble_vector(L)
    ...
    L2 = create_form(ufl.inner(w, v) * ds)
    b2 = assemble_vector(L2)
    ...
    # Assemble scalar
    L = create_form(w * (ds(1) + ds(2) + ds(3) + ds(6)))
    s = assemble_scalar(L)
    s = mesh.comm.allreduce(s, op=MPI.SUM)
    L2 = create_form(w * ds)
    s2 = assemble_scalar(L2)
...
def test_assembly_dS_domains(mode):
    ...
    val = assemble_scalar(create_form(one * ufl.dS))
...
def test_additivity(mode):
    ...
    J1 = mesh.comm.allreduce(assemble_scalar(create_form(j1)), op=MPI.SUM)
    J2 = mesh.comm.allreduce(assemble_scalar(create_form(j2)), op=MPI.SUM)
    J3 = mesh.comm.allreduce(assemble_scalar(create_form(j3)), op=MPI.SUM)

    # Sum forms and assemble the result
    J12 = mesh.comm.allreduce(assemble_scalar(create_form(j1 + j2)), op=MPI.SUM)
    J13 = mesh.comm.allreduce(assemble_scalar(create_form(j1 + j3)), op=MPI.SUM)
    J23 = mesh.comm.allreduce(assemble_scalar(create_form(j2 + j3)), op=MPI.SUM)
    J123 = mesh.comm.allreduce(assemble_scalar(create_form(j1 + j2 + j3)), op=MPI.SUM)
