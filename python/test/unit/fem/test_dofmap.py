...
import numpy as np
from dolfin import Mesh, FunctionSpace, MPI, fem
...
@pytest.mark.parametrize("points, celltype", [(np.array([[0, 0], [0, 2], [1, 0], [1, 2]]),
                                              ...
                                              (np.array([[0, 0], [0, 2], [0, 1/2], [0, 1], [0, 3/2],
                                                         ...
                                                         [1/4, 0], [1/4, 2], [1/4, 1/2], [1/4, 1], [1/4, 3/2],
                                                         [2/4, 0], [2/4, 2], [2/4, 1/2], [2/4, 1], [2/4, 3/2],
                                                         [3/4, 0], [3/4, 2], [3/4, 1/2], [3/4, 1], [3/4, 3/2],
                                                         ]),
                                               CellType.quadrilateral)])
def test_higher_order_dofmap(points, celltype):
    ...
    V = FunctionSpace(mesh, ("Lagrange", 3))
    dofmap = V.dofmap
    edge_dofs_local = [dofmap.dof_layout.entity_dofs(1, e) for e in [1]]
