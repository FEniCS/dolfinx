...
def test_rank0(dtype):
    ...
    assert np.allclose(b2.x.array, b.x.array, rtol=np.sqrt(np.finfo(dtype).eps), atol=np.sqrt(np.finfo(dtype).eps))
...
def test_simple_evaluation(dtype):
    ...
    assert np.allclose(grad_f_evaluated, grad_f_exact, rtol=np.sqrt(np.finfo(dtype).eps), atol=np.sqrt(np.finfo(dtype).eps))
