...
class TestPETScSolverWrappers:
    ...
    def test_compare_solvers(self, mode):
        ...
        msh = create_unit_square(MPI.COMM_WORLD, 12, 12, ghost_mode=mode)
    ...
    def test_mixed_system(self, mode, kind):
        ...
        msh = create_unit_square(MPI.COMM_WORLD, 12, 12, ghost_mode=mode, dtype=PETSc.RealType)

        def top_bc(x):
            return np.isclose(x[1], 1.0)

        msh.topology.create_connectivity(msh.topology.dim - 1, msh.topology.dim)
        bndry_facets = locate_entities_boundary(msh, msh.topology.dim - 1, top_bc)
        ...
        if kind is None:
            ...
            V = functionspace(msh, el_0)
            Q = functionspace(msh, el_1)
        ...
        g_expr = Expression(g, Q.element.interpolation_points)
        u_bc = Function(V)
        ...
        if kind is None:
            a = a00 + a11
            L = L0 + L1
            dofs_V = locate_dofs_topological((W.sub(0), V), msh.topology.dim - 1, bndry_facets)
            dofs_Q = locate_dofs_topological((W.sub(1), Q), msh.topology.dim - 1, bndry_facets)
            bcs = [dirichletbc(u_bc, dofs_V, W.sub(0)), dirichletbc(p_bc, dofs_Q, W.sub(1))]
        else:
            a = [[a00, None], [None, a11]]
            L = [L0, L1]
            dofs_V = locate_dofs_topological(V, msh.topology.dim - 1, bndry_facets)
            dofs_Q = locate_dofs_topological(Q, msh.topology.dim - 1, bndry_facets)
            bcs = [dirichletbc(u_bc, dofs_V), dirichletbc(p_bc, dofs_Q)]
        ...
        if kind is None:
            ...
            uh, ph = wh
        error_uh = form(ufl.inner(uh - f, uh - f) * ufl.dx)
        error_ph = form(ufl.inner(ph - g, ph - g) * ufl.dx)
        local_uh_L2 = assemble_scalar(error_uh)
        local_ph_L2 = assemble_scalar(error_ph)
        ...
        tol = 1e5 * np.finfo(PETSc.RealType()).eps
        # print("TTTT:", tol)
