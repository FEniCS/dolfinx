...
def run_dg_test(mesh, V, degree):
    ...
    L = inner(f, v) * dx_ - inner(k * u_exact * n, grad(v)) * ds_ + (alpha / h) * inner(k * u_exact, v) * ds_
...
def test_curl_curl_eigenvalue(family, order):
    ...
    mesh = create_rectangle(
        MPI.COMM_WORLD, [np.array([0.0, 0.0]), np.array([np.pi, np.pi])], [24, 24], CellType.triangle
    )
...
def test_biharmonic(family):
    ...
    mesh = create_rectangle(MPI.COMM_WORLD, [np.array([0.0, 0.0]), np.array([1.0, 1.0])], [16, 16], CellType.triangle)

    e = mixed_element(
        [element(family, basix.CellType.triangle, 1), element(basix.ElementFamily.P, basix.CellType.triangle, 2)]
    )
    ...
    u_error_numerator = np.sqrt(
        mesh.comm.allreduce(
            assemble_scalar(
                form(inner(u_exact - x_h[4], u_exact - x_h[4]) * dx(mesh, metadata={"quadrature_degree": 6}))
            ),
            op=MPI.SUM,
        )
    )
    u_error_denominator = np.sqrt(
        mesh.comm.allreduce(
            assemble_scalar(form(inner(u_exact, u_exact) * dx(mesh, metadata={"quadrature_degree": 6}))), op=MPI.SUM
        )
    )
    ...
    sigma_error_denominator = np.sqrt(
        mesh.comm.allreduce(
            assemble_scalar(form(inner(sigma_exact, sigma_exact) * dx(mesh, metadata={"quadrature_degree": 6}))),
            op=MPI.SUM,
        )
    )
