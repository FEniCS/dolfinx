...
@pytest.mark.parametrize("family", ["RTCE"])
@pytest.mark.parametrize("order", [1, 2, 3])
def test_curl_curl_eigenvalue_RTCE(family, order):
    """curl curl eigenvalue problem.
    ...
    mesh = create_rectangle(MPI.COMM_WORLD, [np.array([0.0, 0.0]),
                                             np.array([np.pi, np.pi])], [24, 24], CellType.quadrilateral)

    element = ufl.FiniteElement(family, ufl.quadrilateral, order)
    V = FunctionSpace(mesh, element)
