...
def test_petsc_assemble_manifold():
    ...
    points = np.array([[0.0, 0.0], [0.2, 0.0], [0.4, 0.0], [0.6, 0.0], [0.8, 0.0], [1.0, 0.0]], dtype=default_real_type)
    cells = np.array([[0, 1], [1, 2], [2, 3], [3, 4], [4, 5]], dtype=np.int32)
    domain = ufl.Mesh(
        element(basix.ElementFamily.P, basix.CellType.interval, 1, gdim=points.shape[1], shape=(points.shape[1],))
    )
...
def test_matrix_assembly_block(mode):
    ...
    def nest():
        ...
        A = petsc_assemble_matrix_nest(
            a_block, bcs=[bc], mat_types=[["baij", "aij", "aij"], ["aij", "", "aij"], ["aij", "aij", "aij"]]
        )
        A.assemble()
        with pytest.raises(RuntimeError):
