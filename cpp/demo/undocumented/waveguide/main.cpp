...
int main()
{
  ...
  if (cutoff == -1.0)
    log::info("Unable to find dominant mode.");
  else
    log::info("Cutoff frequency = %g", cutoff);
...
int main()
{
  log::info("Sorry, this demo is only available when DOLFIN is compiled with PETSc and SLEPc.");
  return 0;
}
