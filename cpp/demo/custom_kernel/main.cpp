...
/// (see `assemble_matrix1`/`assemble_vector1` below).
...
void assemble(MPI_Comm comm)
{
  ...
  // equivalent UFL form) rather than hand-written. FFCx only compiles
  // the requested scalar type (float64), so this variant only runs
  // for T = double.
