                        ...
                        std::span<const std::int32_t> cells)
{
  ...
  std::vector kernel_data{fem::integral_data<T, std::span<const int32_t>>(
      -1, kernel_t<T>(kernel), cells)};
...
{
  auto mesh = V->mesh();
  std::vector kernal_data{fem::integral_data<T, std::span<const int32_t>>(
      -1, kernel_t<T>(kernel), cells)};
