...
// * Drive that same lower-level lambda-kernel path with a kernel that
//   is not hand-written but generated by FFCx from a UFL form
//   (`mass.py`), by wrapping the generated `tabulate_tensor` function
//   in a lambda. `mass.c` is `#include`d directly into this file
//   (rather than compiled and linked as a separate translation unit),
//   so the generated kernel body is visible to the compiler at its
//   call site and can be inlined into the assembler loop, the same as
...
// mass.c is included directly, rather than compiled and linked as its
// own translation unit, so the FFCx-generated tabulate_tensor bodies
// below are visible to the compiler at the point they are called (see
// ffcx_cell_kernel() and its use in assemble()) and can be inlined
// into the assembler loop -- an indirect call through a function
// pointer into a separately-compiled object cannot be inlined this
// way. `restrict` is a bare C keyword the generated code relies on;
// ufcx.h (via mass.h) defines it as a macro only for its own
// declarations and undefines it again before finishing, so it is
// redefined here for mass.c's function bodies. FFCx-generated kernels
// routinely leave some of the fixed tabulate_tensor parameters unused
// (e.g. a cell integral never uses entity_local_index).
...
/// @brief Extract the double-precision cell-integral kernel from an
/// FFCx-generated form, for use as a kernel with the low-level
/// `impl::assemble_cells`/`impl::assemble_cells_matrix` bypass path
/// (see `assemble_matrix1`/`assemble_vector1` below). Because mass.c
/// is `#include`d directly into this file rather than linked from a
/// separate translation unit, the returned function's body is visible
/// to the compiler here and can be inlined into the assembler loop
/// through the lambda that calls it.
/// @param form FFCx-generated form, with exactly one cell integral
/// (id 0) and no facet/vertex/ridge integrals.
/// @return `tabulate_tensor` function for the form's cell integral.
...
void assemble(MPI_Comm comm)
{
  ...
  // Assemble matrix and vector through the same lambda-kernel bypass
  // path, but with a kernel generated by FFCx from mass.py (an
  // equivalent UFL form) rather than hand-written. Since mass.c is
  // included directly above rather than linked separately, the
  // generated kernel body is visible to the compiler here too, and is
  // eligible for the same inlining into the assembler loop as the
  // hand-written kernel above -- a kernel reached only through a
  // function pointer into a separately-compiled object (as with
  // fem::Form's type-erased kernel dispatch) cannot be inlined this
  // way. FFCx only compiles the requested scalar type (float64), so
  // this variant only runs for T = double.
  if constexpr (std::is_same_v<T, double>)
  {
    ...
    assemble_matrix1<T>(mesh->geometry(), *V->dofmap(), kernel_a_ffcx, cells);
    assemble_vector1<T>(mesh->geometry(), *V->dofmap(), kernel_L_ffcx, cells);
  }
