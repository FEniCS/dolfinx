...
int main(int argc, char* argv[])
{
...

  {
    ...
    auto mesh = std::make_shared<mesh::Mesh>(
        generation::BoxMesh::create(comm, {{{0.0, 0.0, 0.0}, {5.0, 1.0, 1.0}}},
                                    {50, 10, 10}, cmap, mesh::GhostMode::none));
    ...
    mesh = std::make_shared<mesh::Mesh>(mesh::add_ghost_layer(*mesh));

    auto V = fem::create_functionspace(create_functionspace_form_poisson_a, "u",
                                       mesh);

    auto u = std::make_shared<fem::Function<PetscScalar>>(V);
    auto& array = u->x()->mutable_array();
    int sz = u->x()->map()->size_local();
    std::fill(array.begin(), array.begin() + sz, mpi_rank + 1);
    std::fill(array.begin() + sz, array.end(), 0);
    ...
    //   int_vertices.erase(std::unique(int_vertices.begin(),
    //   int_vertices.end()),
    //                      int_vertices.end());
    ...
    //     owner[i]); assert(it != destinations.end()); int pos =
    //     std::distance(destinations.begin(), it); send_sizes[pos]++;
    //   }
    ...
    //   {
    //     auto it = std::find(destinations.begin(), destinations.end(),
    //     owner[i]); assert(it != destinations.end()); int p =
    //     std::distance(destinations.begin(), it); int& pos = insert_pos[p];
    ...
    //   global_indices_owned(vertex_procs_owned.size());
    ...
    //   1); std::vector<std::int32_t> pos = offsets; std::vector<std::int32_t>
    //   data(offsets.back(), mpi_rank);
    //   {
    //     // Merge maps?
    //     int j = 0;
    //     for (auto const& [vertex, neighbors] : vertex_procs_ghost)
    ...
    //     cell_data.insert(cell_data.end(), local_set.begin(),
    //     local_set.end()); counter[c] = local_set.size() + 1;
    //   }

    //   std::vector<std::int32_t> new_offsets(counter.size() + 1, 0);
    //   std::partial_sum(counter.begin(), counter.end(), new_offsets.begin() +
    //   1); graph::AdjacencyList<std::int32_t> dest(cell_data, new_offsets);
    ...
    //       vert_map->local_to_global(local, global);
    //       global_array.insert(global_array.end(), global.begin(),
    //       global.end()); counter[i] += global.size();
    //     }
    //     std::vector<std::int32_t> offsets(counter.size() + 1, 0);
    //     std::partial_sum(counter.begin(), counter.end(), offsets.begin() +
    //     1);

    //     graph::AdjacencyList<std::int64_t> cell_vertices(global_array,
    //     offsets);
    ...
    //     auto V =
    //     fem::create_functionspace(create_functionspace_form_poisson_a,
    //                                        "u", new_mesh);
    ...
    //     "/Xdmf/Domain/Grid[@Name='mesh']/Geometry";
