...
void solver(MPI_Comm comm)
{
  ...
  // b <- b - A * x_bc
  fem::set_bc<T, U>(ui->x()->mutable_array(), {bc}, std::nullopt, T(-1));
  ...
  fem::set_bc<T, U>(b.mutable_array(), {bc}, std::nullopt, T(0));
  ...
  auto action = [&M, &ui, &bc, &coeff, &constants](auto& x, auto& y)
  {
    ...
    fem::set_bc<T, U>(y.mutable_array(), {bc}, std::nullopt, T(0));
  ...
  fem::set_bc<T, U>(u->x()->mutable_array(), {bc}, std::nullopt, T(1));
