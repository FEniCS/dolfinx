    ...
    const std::vector<std::int8_t>& facet)
{
  ...
  if (meshtag.dim() != tdim - 1)
    throw dolfinx::runtime_error("Input meshtag is not facet-based");
  if (meshtag.mesh()->topology().index_map(tdim)->num_ghosts() > 0)
    throw dolfinx::runtime_error("Ghosted meshes are not supported");

  auto c_to_f = meshtag.mesh()->topology().connectivity(tdim, tdim - 1);
  if (!c_to_f)
    throw dolfinx::runtime_error("Parent mesh is missing cell-facet connectivity.");
  ...
  if (!c_to_f_refined)
  {
    throw dolfinx::runtime_error("Refined mesh is missing cell-facet connectivity.");
  }
    ...
    const std::vector<std::int32_t>& cell)
{
  const int tdim = meshtag.mesh()->topology().dim();
  if (meshtag.dim() != tdim)
    throw dolfinx::runtime_error("Input meshtag is not cell-based");
