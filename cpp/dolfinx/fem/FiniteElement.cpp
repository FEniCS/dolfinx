  ...
  if (_sub_elements.size() != 0)
  {
    if (_bs == 1)
    {
      ...
      { // scope
        ...
        ed.resize(sub_ed.size());
        for (std::size_t d = 0; d < ed.size(); ++d)
          ed[d].resize(sub_ed[d].size());
      }
      int offset = 0;
      for (std::size_t i = 0; i < _sub_elements.size(); ++i)
      {
        std::vector<std::vector<std::set<int>>> sub_ed
            = _sub_elements[i]->entity_dofs();
        for (std::size_t d = 0; d < ed.size(); ++d)
          for (std::size_t e = 0; e < ed[d].size(); ++e)
            for (int dof : sub_ed[d][e])
              ed[d][e].insert(offset + dof);
        offset += _sub_elements[i]->space_dimension();
      }
      return ed;
    }
    else
...
FiniteElement::entity_closure_dofs(bool scalar_element) const
{
  ...
  if (_sub_elements.size() != 0)
  {
    if (_bs == 1)
    {
      ...
      {
        std::vector<std::vector<std::set<int>>> sub_ed
            = _sub_elements[i]->entity_closure_dofs();
        for (std::size_t d = 0; d < ed.size(); ++d)
          for (std::size_t e = 0; e < ed[d].size(); ++e)
            for (int dof : sub_ed[d][e])
              ed[d][e].insert(offset + dof);
