...
FunctionSpace::FunctionSpace(std::shared_ptr<const mesh::Mesh> mesh,
    ...
    : _mesh(mesh), _meshView(), _element(element), _dofmap(dofmap),
      _id(common::UniqueIdGenerator::id()), _root_space_id(_id)
{
  // Do nothing
}
//-----------------------------------------------------------------------------
FunctionSpace::FunctionSpace(std::shared_ptr<const mesh::MeshView> meshView,
                             std::shared_ptr<const fem::FiniteElement> element,
                             std::shared_ptr<const fem::DofMap> dofmap)
    : _mesh(meshView->parent_mesh()), _meshView(meshView), _element(element), _dofmap(dofmap),
      _id(common::UniqueIdGenerator::id()), _root_space_id(_id)
{
  // Do nothing
}
...
//-----------------------------------------------------------------------------
std::shared_ptr<const mesh::MeshView> FunctionSpace::mesh_view() const { return _meshView; }
//-----------------------------------------------------------------------------
