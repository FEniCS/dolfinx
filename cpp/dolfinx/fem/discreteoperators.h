                          ...
                          const FunctionSpace<U>& V1, auto&& mat_set)
{
  ...
  const std::size_t value_size0 = e0->value_size(gdim) / bs0;
  const std::size_t value_size1 = e1->value_size(gdim) / bs1;
  ...
  std::vector<U> basis_values_b(Xshape[0] * bs0 * dim0 * e1->value_size(gdim));
  mdspan3_t basis_values(basis_values_b.data(), Xshape[0], bs0 * dim0,
                         e1->value_size(gdim));
  std::vector<U> mapped_values_b(Xshape[0] * bs0 * dim0 * e1->value_size(gdim));
  ...
  for (std::int32_t c = 0; c < num_cells; ++c)
  {
    ...
    if (interpolation_ident)
    {
      MDSPAN_IMPL_STANDARD_NAMESPACE::mdspan<
          ...
          A(Ab.data(), Xshape[0], e1->value_size(gdim), space_dim0);
