                   ...
                   la::MatSet<U> auto&& mat_set)
{
  ...
  // DirectDofTransform or the generic std::function-based closures
  // applies for e0/e1 -- see FiniteElement::is_direct_transform_eligible
  // for the rationale. Both elements are checked once, up front, rather
  // than nesting a with_dof_transformation_fn call per element, so the
  // per-cell loop body appears only once and at a single indent level.
  auto run = [&mesh, &gdim, &Phi0, &dPhi0, &cell_info, &curl, &Pi_1,
              &space_dim1, &space_dim0, &Ab, &mat_set, &dofmap1,
              &dofmap0](const auto& apply_dof_transformation0,
      ...
      for (std::size_t p = 0; p < dPhi0.extent(0); ++p) // point
      {
        ...
        if (apply_dof_transformation0)
        {
          apply_dof_transformation0(
              std::span(dPhi0.data_handle() + offset, size), cell_info, c,
  ...
  if (e0->is_direct_transform_eligible() and e1->is_direct_transform_eligible())
  {
    DirectDofTransform<T, doftransform::standard> P0(*e0);
    DirectDofTransform<T, doftransform::inverse_transpose> P1(*e1);
    run(P0, P1);
  }
  else
  {
    auto P0 = e0->template dof_transformation_fn<T>(doftransform::standard);
    auto P1 = e1->template dof_transformation_fn<U>(
                       ...
                       auto&& mat_set)
{
  ...
  e1.template with_dof_transformation_fn<T, doftransform::inverse_transpose>(
      [&topology, &tdim, &Ab, &cell_info, &ndofs0, &mat_set, &dofmap1,
       &dofmap0](const auto& apply_inverse_dof_transform)
      {
                          ...
                          const FunctionSpace<U>& V1, auto&& mat_add)
{
  ...
  // FiniteElement::is_direct_transform_eligible for the rationale. Both
  // elements are checked once, up front, rather than nesting a
  // with_dof_transformation_fn call per element, so the per-cell loop
  // body appears only once and at a single indent level.
  auto run
      = [&num_cells, &x_dofmap, &gdim, &coord_dofs, &x_g, &J_b, &J, &K_b, &K,
         &detJ, &det_scratch, &cmap, &tdim, &phi, &Xshape,
         &basis_derivatives_reference0_b, &basis_reference0_b,
         &basis_reference0, &dim0, &value_size_ref0, &cell_info, &basis0,
         &push_forward_fn0, &basis_values, &bs0, &value_size0, &mapped_values,
         &pull_back_fn1, &interpolation_ident, &Ab, &V1, &space_dim0, &bs1,
         &Pi_1, &space_dim1, &dofmap1, &row_bs, &num_owned_rows, &row_added,
         &dofmap0, &mat_add](const auto& apply_dof_transformation0,
                             const auto& apply_inverse_dof_transform1)
  {
    for (std::int32_t c = 0; c < num_cells; ++c)
    {
  ...
  if (e0->is_direct_transform_eligible() and e1->is_direct_transform_eligible())
  {
    DirectDofTransform<U, doftransform::standard> P0(*e0);
    DirectDofTransform<U, doftransform::inverse_transpose> P1(*e1);
    run(P0, P1);
  }
  else
  {
    auto P0 = e0->template dof_transformation_fn<U>(doftransform::standard);
    auto P1 = e1->template dof_transformation_fn<T>(
        doftransform::inverse_transpose);
    run(P0, P1);
  }
}
