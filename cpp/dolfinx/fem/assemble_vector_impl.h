    ...
    std::span<const std::int8_t> bc_markers1, std::span<const T> x0, T scale)
{
  ...
  for (std::size_t index = 0; index < cells.size(); ++index)
  {
    ...
    auto dmap1 = stdex::submdspan(dofmap1, c, MDSPAN_IMPL_STANDARD_NAMESPACE::full_extent);
    ...
    auto dmap0 = stdex::submdspan(dofmap0, c, MDSPAN_IMPL_STANDARD_NAMESPACE::full_extent);
    ...
    std::span<const std::int8_t> bc_markers1, std::span<const T> x0, T scale)
{
  ...
  for (std::size_t index = 0; index < facets.size(); index += 2)
  {
    ...
    // Get dof maps for cell
    auto dmap1 = stdex::submdspan(dofmap1, cell, MDSPAN_IMPL_STANDARD_NAMESPACE::full_extent);
    ...
    // Size data structure for assembly
    auto dmap0 = stdex::submdspan(dofmap0, cell, MDSPAN_IMPL_STANDARD_NAMESPACE::full_extent);
    ...
    std::span<const T> x0, T scale)
{
  ...
  for (std::size_t index = 0; index < facets.size(); index += 4)
  {
    ...
    // Get cell geometry
    auto x_dofs0 = stdex::submdspan(x_dofmap, cells[0], MDSPAN_IMPL_STANDARD_NAMESPACE::full_extent);
    for (std::size_t i = 0; i < x_dofs0.size(); ++i)
    {
      std::copy_n(std::next(x.begin(), 3 * x_dofs0[i]), 3,
                  std::next(cdofs0.begin(), 3 * i));
    }
    auto x_dofs1 = stdex::submdspan(x_dofmap, cells[1], MDSPAN_IMPL_STANDARD_NAMESPACE::full_extent);
