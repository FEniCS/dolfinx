    ...
    std::span<const std::int8_t> bc_markers1, std::span<const T> x0, T scale)
{
  ...
  for (std::size_t index = 0; index < cells.size(); ++index)
  {
    ...
    auto dmap1 = stdex::submdspan(dofmap1, c,
    ...
    auto x_dofs = stdex::submdspan(x_dofmap, c,
                                   MDSPAN_IMPL_STANDARD_NAMESPACE::full_extent);
    ...
    auto dmap0 = stdex::submdspan(dofmap0, c,
                                  MDSPAN_IMPL_STANDARD_NAMESPACE::full_extent);
    ...
    std::span<const std::int8_t> bc_markers1, std::span<const T> x0, T scale)
{
  ...
  for (std::size_t index = 0; index < facets.size(); index += 2)
  {
    ...
    // Get dof maps for cell
    auto dmap1 = stdex::submdspan(dofmap1, cell,
                                  MDSPAN_IMPL_STANDARD_NAMESPACE::full_extent);
    ...
    // Size data structure for assembly
    auto dmap0 = stdex::submdspan(dofmap0, cell,
                                  MDSPAN_IMPL_STANDARD_NAMESPACE::full_extent);
    ...
    std::span<const std::uint32_t> cell_info)
{
  ...
  for (std::size_t index = 0; index < cells.size(); ++index)
  {
    ...
    auto dofs = stdex::submdspan(dofmap, c,
                                 MDSPAN_IMPL_STANDARD_NAMESPACE::full_extent);
    if constexpr (_bs > 0)
    {
    ...
    std::span<const std::uint32_t> cell_info)
{
  ...
  for (std::size_t index = 0; index < facets.size(); index += 2)
  {
    ...
    auto dofs = stdex::submdspan(dofmap, cell,
                                 MDSPAN_IMPL_STANDARD_NAMESPACE::full_extent);
    if constexpr (_bs > 0)
    {
      for (std::size_t i = 0; i < dofs.size(); ++i)
