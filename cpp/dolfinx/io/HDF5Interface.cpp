...
bool has_group(const hid_t handle, const std::string& group_name)
{
  ...
  if (lapl_id < 0)
    throw dolfinx::runtime_error("Failed to create HDF5 property list");

  htri_t link_status = H5Lexists(handle, group_name.c_str(), lapl_id);
  if (link_status < 0)
    throw dolfinx::runtime_error("Failed to check existence of HDF5 link in group");

  if (link_status == 0)
  {
    if (H5Pclose(lapl_id) < 0)
      throw dolfinx::runtime_error("Call to H5Pclose unsuccessful");
    return false;
  }
...
#endif
  if (err < 0)
    throw dolfinx::runtime_error("Call to H5Oget_info_by_name unsuccessful");
                               ...
                               const std::string& mode, const bool use_mpi_io)
{
  ...
  if (use_mpi_io)
  {
    ...
    if (H5Pset_fapl_mpio(plist_id, comm, info) < 0)
      throw dolfinx::runtime_error("Call to H5Pset_fapl_mpio unsuccessful");
    MPI_Info_free(&info);
  }
#endif
  ...
  if (mode == "w") // Create file for write, overwriting any existing file
  {
    ...
    if (file_id < 0)
      throw dolfinx::runtime_error("Failed to create HDF5 file.");
  }
  else if (mode == "a") // Open file to append, creating if does not exist
  {
    ...
    if (file_id < 0)
    {
      throw dolfinx::runtime_error("Failed to create/open HDF5 file (append mode).");
    }
  }
  else if (mode == "r") // Open file to read
  {
    if (std::filesystem::exists(filename))
    {
      ...
      if (file_id < 0)
        throw dolfinx::runtime_error("Failed to open HDF5 file.");
    }
    else
    {
      throw dolfinx::runtime_error("Unable to open HDF5 file. File "
                             + filename.native() + " does not exist.");
    }
  }

  if (H5Pclose(plist_id) < 0)
    throw dolfinx::runtime_error("Failed to close HDF5 file property list.");
...
void HDF5Interface::close_file(const hid_t handle)
{
  if (H5Fclose(handle) < 0)
    throw dolfinx::runtime_error("Failed to close HDF5 file.");
}
//-----------------------------------------------------------------------------
void HDF5Interface::flush_file(const hid_t handle)
{
  if (H5Fflush(handle, H5F_SCOPE_GLOBAL) < 0)
    throw dolfinx::runtime_error("Failed to flush HDF5 file.");
}
//-----------------------------------------------------------------------------
std::filesystem::path HDF5Interface::get_filename(hid_t handle)
{
  ...
  if (length < 0)
    throw dolfinx::runtime_error("Failed to get HDF5 filename from handle.");
...
void HDF5Interface::add_group(const hid_t handle, const std::string& group_name)
{
  ...
  while (pos != std::string::npos)
  {
    ...
    if (!has_group(handle, parent_name))
    {
      ...
      if (group_id_vis < 0)
        throw dolfinx::runtime_error("Failed to add HDF5 group");

      if (H5Gclose(group_id_vis) < 0)
        throw dolfinx::runtime_error("Failed to close HDF5 group");
    }
  }
}
//-----------------------------------------------------------------------------
                                 ...
                                 const std::string& dataset_path)
{
  ...
  if (dset_id < 0)
    throw dolfinx::runtime_error("Failed to open HDF5 dataset by name");

  const hid_t space = H5Dget_space(dset_id);
  if (space < 0)
    throw dolfinx::runtime_error("Failed to get dataspace of dataset");
  ...
  if (H5Sclose(space) < 0)
    throw dolfinx::runtime_error("Call to H5Sclose unsuccessful");
  if (H5Dclose(dset_id) < 0)
    throw dolfinx::runtime_error("Call to H5Dclose unsuccessful");
...
void HDF5Interface::set_mpi_atomicity(const hid_t handle, const bool atomic)
{
  ...
  if (H5Fset_mpi_atomicity(handle, atomic) < 0)
    throw dolfinx::runtime_error("Setting the MPI atomicity flag failed");
#endif
}
//-----------------------------------------------------------------------------
bool HDF5Interface::get_mpi_atomicity(const hid_t handle)
{
  ...
  if (H5Fget_mpi_atomicity(handle, &atomic) < 0)
    throw dolfinx::runtime_error("Getting the MPI atomicity flag failed");
#endif
