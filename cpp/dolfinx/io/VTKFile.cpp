              ...
              int tdim, pugi::xml_node& piece_node)
{
  ...
  if (!cellmap.ghosts().empty())
  {
    ...
    auto minmax = std::ranges::minmax_element(ghosts);
    min_idx = std::min(min_idx, *minmax.min);
    max_idx = std::max(max_idx, *minmax.max);
  }
  ...
  if (!x_id.empty())
  {
    auto minmax = std::ranges::minmax_element(x_id);
    point_id_node.append_attribute("RangeMin") = *minmax.min;
    point_id_node.append_attribute("RangeMax") = *minmax.max;
  }
  ...
  if (!x_ghost.empty())
  {
    auto minmax = std::ranges::minmax_element(x_ghost);
    point_ghost_node.append_attribute("RangeMin") = *minmax.min;
    point_ghost_node.append_attribute("RangeMax") = *minmax.max;
  }
}
//----------------------------------------------------------------------------
