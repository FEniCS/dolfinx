    ...
    const std::filesystem::path& filename)
{
  ...
  add_mesh_new(x, cells, mesh0->topology().cell_type(), mesh0->topology().dim(),
               piece_node);
  ...
  for (auto _u : u)
  {
    auto V = _u.get().function_space();
    assert(V);
    pugi::xml_node data_node = is_cellwise(*V) ? piece_node.child("CellData")
