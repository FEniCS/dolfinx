...
void fides_initialize_mesh_attributes(adios2::IO& io, const mesh::Mesh& mesh)
{
  ...
  if (num_dofs_g != num_vertices_per_cell)
    throw dolfinx::runtime_error("Fides only supports lowest-order meshes.");
...
{
  if (u.empty())
    throw dolfinx::runtime_error("FidesWriter fem::Function list is empty");
  ...
  if (element0->is_mixed())
    throw dolfinx::runtime_error("Mixed functions are not supported by VTXWriter");
  ...
  {
    throw dolfinx::runtime_error("Only Lagrange functions are "
  ...
  for (auto& v : _u)
  {
    std::visit(
        [&](const auto& u)
        {
          ...
          {
            throw dolfinx::runtime_error("Only first order Lagrange spaces are "
...
{
  if (u.empty())
    throw dolfinx::runtime_error("VTXWriter fem::Function list is empty");
  ...
  if (!element0->interpolation_ident())
  {
    throw dolfinx::runtime_error("Only (discontinuous) Lagrange functions are "
                           "supported. Interpolate Functions before output.");
  }
