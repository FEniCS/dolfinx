                  ...
                  hid_t h5_id, const std::string& name)
{
  ...
  MPI_Allreduce(&local_num_values, &global_num_values, 1, dolfinx::MPI::mpi_t<std::int64_t>, MPI_SUM,
                comm);
  const std::int64_t num_local = num_active_entities;
  std::int64_t offset = 0;
  MPI_Exscan(&num_local, &offset, 1, dolfinx::MPI::mpi_t<std::int64_t>, MPI_SUM, comm);
