...
//-----------------------------------------------------------------------------
void xdmf_mesh::add_topology_data_time(
    MPI_Comm comm, pugi::xml_node& xml_node, const hid_t h5_id,
    const std::string path_prefix, const mesh::Topology& topology,
    const mesh::Geometry& geometry, const int dim,
    const xtl::span<const std::int32_t>& active_entities, const double t)
{
  ...
  // Add topology DataItem node
  const std::string h5_path = path_prefix + "/topology/" + t_str;
  const std::vector<std::int64_t> shape
      = {num_entities_global, num_nodes_per_entity};
  const std::string number_type = "Int";
...
//-----------------------------------------------------------------------------
void xdmf_mesh::add_geometry_data_time(MPI_Comm comm, pugi::xml_node& xml_node,
                                       const hid_t h5_id,
                                       const std::string path_prefix,
                                       const mesh::Geometry& geometry,
                                       const double t)
{
  ...
  // Add geometry DataItem node
  const std::string h5_path = path_prefix + "/geometry/" + t_str;
  const std::vector<std::int64_t> shape = {num_points, width};
...
//----------------------------------------------------------------------------
void xdmf_mesh::add_mesh_time(MPI_Comm comm, pugi::xml_node& xml_node,
                              const hid_t h5_id, const mesh::Mesh& mesh,
                              const std::string name, const double t)
{
  LOG(INFO) << "Adding mesh to node \"" << xml_node.path('/') << "\"";
  ...
  add_topology_data_time(
      comm, grid_node, h5_id, path_prefix, mesh.topology(), mesh.geometry(),
      tdim, xtl::span<std::int32_t>(active_cells.data(), num_cells), t);

  // Add geometry node and attributes (including writing data)
  add_geometry_data_time(comm, grid_node, h5_id, path_prefix, mesh.geometry(),
                         t);
}
//----------------------------------------------------------------------------
