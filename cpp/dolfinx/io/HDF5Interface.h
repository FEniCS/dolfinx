...
public:
  ...
  template <typename T>
  static T get_attribute(const hid_t handle, const std::string& dataset_path,
                         const std::string& attribute_name);

  /// Add attribute to dataset or group
  template <typename T>
  static void add_attribute(const hid_t handle, const std::string& dataset_path,
                            const std::string& attribute_name,
...
private:
  ...
  template <typename T>
  static T get_attribute_value(const hid_t attr_type, const hid_t attr_id);

  template <typename T>
  static std::vector<T> get_attribute_vector(const hid_t attr_type,
                                             const hid_t attr_id);

  static std::string get_attribute_str(const hid_t attr_type,
                                       const hid_t attr_id);
...
//---------------------------------------------------------------------------
template <typename T>
inline T HDF5Interface::get_attribute(hid_t handle,
                                      const std::string& dataset_path,
                                      const std::string& attribute_name)
{
  herr_t status;
  ...
  if constexpr (std::is_same<T, std::string>::value)
    attribute_value = get_attribute_str(attr_type, attr_id);
  else if constexpr (std::is_same<T,
                                  std::vector<typename T::value_type>>::value)
  {
    attribute_value = get_attribute_vector<T::value_type>(attr_type, attr_id);
  }
  else
    attribute_value = get_attribute_value<T>(attr_type, attr_id);
...
//--------------------------------------------------------------------------
template <typename T>
inline void HDF5Interface::add_attribute(const hid_t handle,
                                         const std::string& dataset_path,
                                         const std::string& attribute_name,
                                         const T& attribute_value)
{
  ...
  hid_t dset_id = H5Oopen(handle, dataset_path.c_str(), H5P_DEFAULT);
  assert(dset_id != HDF5_FAIL);
...
//--------------------------------------------------------------------------
template <typename T>
inline T HDF5Interface::get_attribute_value(const hid_t attr_type,
                                            const hid_t attr_id)
{
  const hid_t h5type = hdf5_type<T>();
...
//---------------------------------------------------------------------------
template <typename T>
inline std::vector<T> HDF5Interface::get_attribute_vector(const hid_t attr_type,
                                                          const hid_t attr_id)
{
  const hid_t h5type = hdf5_type<T>();
  ...
  std::vector<T> attribute_value(cur_size[0]);
