...
/// @brief Distribute fixed-width nodes to destination ranks.
///
/// The global index of each node is assumed to be the local index plus
/// the offset for this rank.
///
/// @param[in] comm MPI Communicator
/// @param[in] list The list to distribute, as a flattened 2D row major array
...
std::tuple<std::vector<std::int64_t>, std::vector<std::int64_t>,
           std::vector<int>>
distribute(MPI_Comm comm, std::span<const std::int64_t> list, int width,
           const graph::AdjacencyList<std::int32_t>& destinations);
