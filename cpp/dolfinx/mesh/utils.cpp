    ...
    std::vector<std::vector<std::int64_t>>& original_idx, int num_threads)
{
  ...
  for (std::size_t i = 0; i < celltypes.size(); ++i)
  {
    ...
    // cells only
    auto [graph, edge_wt, unmatched_facet_data] = build_local_dual_graph(
        std::vector{celltypes[i]}, std::vector{cells_v_owned},
        max_facet_to_cell_links, num_threads,
        std::vector<std::span<const std::int32_t>>{});

    // Store unmatched_facets for current cell type
    facets.emplace_back(std::move(unmatched_facet_data.facets),
                        unmatched_facet_data.num_columns);
