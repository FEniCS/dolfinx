...
//-----------------------------------------------------------------------------
graph::AdjacencyList<std::int32_t> mesh::compute_cell_destinations(
    const graph::AdjacencyList<std::int64_t>& cells, mesh::CellType celltype,
    const fem::ElementDofLayout& doflayout,
    const mesh::CellPartitionFunction& partitioner, MPI_Comm comm, int npart)
{
  int tdim = cell_dim(celltype);
  return partitioner(comm, npart, tdim,
