    ...
    int dim)
{
  ...
  // Loops over cell types in mesh
  std::vector<std::vector<std::int32_t>> cell_type_entities(cell_lists.size());
  std::vector<std::size_t> cell_type_offsets{0};
  for (std::size_t k = 0; k < cell_lists.size(); ++k)
  {
    // Loop over entity types of dimension dim for this cell type
  ...
  // Build entity list of all entities (defined by vertices) of type
  // `entity_type`, looping over cells of each type.
  int num_vertices_per_entity = num_cell_vertices(entity_type);
  ...
  for (std::size_t k = 0; k < cell_lists.size(); ++k)
  {
    ...
    // Map from cell vertices to entity vertices
    mesh::CellType cell_type = std::get<0>(cell_lists[k]);
    ...
    std::cout << "Using " << num_threads << " threads\n";
    {
      ...
      {
        auto [c0, c1]
            = dolfinx::MPI::local_range(t, cells.num_nodes(), num_threads);
        threads[t] = std::jthread(
            cell_entry, c0, c1 - c0,
            std::span(entity_list.data() + cell_type_offsets[k],
                      entity_list.data() + cell_type_offsets[k + 1]),
  ...
  std::int32_t entity_count = 0;
  {
    ...
    for (std::size_t j = 0; j < cell_type_offsets.back(); ++j)
    {
      auto it
  ...
  for (std::size_t i = 0; i < cell_type_offsets.back(); ++i)
  {
