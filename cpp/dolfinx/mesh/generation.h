...
/// across MPI ranks. See ::AnyCellPartitionFunction.
...
Mesh<T> create_box(MPI_Comm comm, MPI_Comm subcomm,
                   ...
                   AnyCellPartitionFunction partitioner = {},
...
Mesh<T> create_rectangle(MPI_Comm comm, std::array<std::array<T, 2>, 2> p,
                         ...
                         int gdim = 2)
{
  return create_rectangle<T>(comm, p, n, celltype, AnyCellPartitionFunction(),
                             diagonal, gdim);
}
...
Mesh<T> create_interval(MPI_Comm comm, std::int64_t n, std::array<T, 2> p,
                        ...
                        AnyCellPartitionFunction partitioner = {}, int gdim = 1,
                        const CellReorderFunction& reorder_fn
                        = graph::reorder_rcm)
{
  if (gdim < 1 || gdim > 3)
  ...
  if (dolfinx::MPI::rank(comm) == 0)
  {
    ...
    if (gdim == 1)
    {
      return create_mesh(comm, MPI_COMM_SELF, cells, std::nullopt, element,
    ...
    return create_mesh(comm, MPI_COMM_SELF, cells, std::nullopt, element,
                       ...
                       partitioner, ghost_mode, 2, 1, reorder_fn);
  }
  else
  {
    return create_mesh(comm, MPI_COMM_NULL, {}, std::nullopt, element,
                       MPI_COMM_NULL, std::vector<T>{},
                  ...
                  GhostMode ghost_mode, const CellReorderFunction& reorder_fn)
{
  ...
  return create_mesh(comm, subcomm, cells, std::nullopt, element, subcomm, x,
                     {x.size() / 3, 3}, partitioner, ghost_mode, 2, 1,
                     reorder_fn);
}
