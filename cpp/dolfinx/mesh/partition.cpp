...
mesh::create_cell_partitioner(graph::partition_fn partfn, int num_threads)
{
  if (num_threads < 1)
    throw std::runtime_error("num_threads must be >= 1.");

  return [partfn = std::move(partfn), num_threads](
...
//-----------------------------------------------------------------------------
mesh::CellPartitionFunction mesh::create_cell_partitioner(int num_threads)
{
  return create_cell_partitioner(graph::partition_graph, num_threads);
}
//-----------------------------------------------------------------------------
...
{
  return partfn;
  // return [partfn = std::move(partfn)](MPI_Comm comm, int nparts,
  ...
  //   return partfn(comm, nparts, x, gdim);
  // };
}
//-----------------------------------------------------------------------------
mesh::HybridCellPartitionFunction
mesh::create_hybrid_cell_partitioner(int num_threads,
                                     graph::hybrid_partition_fn partfn)
{
  return [num_threads, partfn = std::move(partfn)](
             MPI_Comm comm, int nparts, const std::vector<CellType>& cell_types,
             const std::vector<std::span<const std::int64_t>>& cells,
             std::optional<std::int32_t> max_facet_to_cell_links,
