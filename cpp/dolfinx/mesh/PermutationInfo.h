...
/// Information about how the entities of each cell should be permuted
/// to get to a low-to-high ordering
...
public:
  ...
  /// a facet of that cell. This data is used to permute the quadrature
  ...
  /// 32 bit int is used for each cell. For 2D cells, one bit is used
  /// for each edge, to represent whether or not the edge is reversed:
  /// the least significant bit is for edge 0, the next for edge 1, etc.
  /// For 3D cells, three bits are used for each face, and for each
  /// edge: the least significant bit says whether or not face 0 is
  /// reflected, the next 2 bits say how many times face 0 is rotated;
  /// the next three bits are for face 1, then three for face 2, etc;
  /// after all the faces, there is 1 bit for each edge to say whether
