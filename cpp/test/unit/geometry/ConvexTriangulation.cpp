...
bool has_degenerate(const std::vector<std::vector<Point>>& triangulation,
                    std::size_t dim)
{
  for (const std::vector<Point>& tri : triangulation)
  {
...
bool triangulation_selfintersects(
    const std::vector<std::vector<Point>>& triangulation, std::size_t dim)
{
    ...
    const std::vector<std::vector<dolfin::Point>>& triangulation)
{
  ...
  for (const std::vector<dolfin::Point>& tri : triangulation)
  {
    ...
    const double v
        = (x0[0] * (x1[1] * x2[2] + x3[1] * x1[2] + x2[1] * x3[2]
...
TEST_CASE("Convex triangulation test")
{
  SECTION("test trivial case]")
  {
    std::vector<Point> input
        = {{Point(0, 0, 0), Point(0, 0, 1), Point(0, 1, 0), Point(1, 0, 0)}};

    std::vector<std::vector<Point>> tri
        = ConvexTriangulation::triangulate_graham_scan_3d(input);
  ...
  {
    std::vector<Point> input{Point(0, 0, 0), Point(0, 0, 1), Point(0, 1, 0),
                             Point(0, 1, 1), Point(1, 0, 0), Point(1, 0, 1),
  ...
  {
    std::vector<Point> input{Point(0.7, 0.6, 0.1),
                             Point(0.7, 0.6, 0.5),
                             Point(0.1, 0.1, 0.1),
  ...
  {
    std::vector<Point> input{
        Point(0.7, 0.6, 0.5),   Point(0.7, 0.1, 0.1),   Point(0.8, 0, 0),
        Point(0.1, 0.1, 0.1),   Point(0.16, 0.1, 0.1),  Point(0.592, 0.1, 0.1),
        Point(0.16, 0.1, 0.14), Point(0.52, 0.1, 0.38), Point(0.7, 0.1, 0.38)};
  ...
  {
    std::vector<Point> input{Point(0.495926, 0.512037, 0.144444),
