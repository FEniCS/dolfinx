...
//     auto x
//         = std::array{from.geometry().x()[3 * i], from.geometry().x()[3 * i +
//         1],
//                      from.geometry().x()[3 * i + 2]};
//     auto global_x
...
//         !std::holds_alternative<refinement::IdentityPartitionerPlaceholder>(
//             ghost_mode));
...
//   for (auto n : {10})
//   {
//     TEST_inclusion(dolfinx::mesh::create_interval<TestType>(MPI_COMM_WORLD,
//     n,
//                                                             {0.0, 1.0}));
//   }
// }
