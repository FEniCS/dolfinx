                   ...
                   "[refinement][mark][equidistribution]", double, float)
{
  ...
  {
    CHECK(v.size() == 1);
    // Check sum reduction ignores ghosts.
    v[0] = static_cast<TestType>(size + 1);
  }

  TestType theta = 0.5;
  auto indices = mark_equidistribution(std::span<const TestType>(v), im, theta);
  ...
  TestType norm = static_cast<TestType>(size) * (size - 1) / 2;
  TestType threshold = theta * theta * norm / static_cast<TestType>(size);
  auto mark = [&threshold](auto e) { return e > threshold; };
