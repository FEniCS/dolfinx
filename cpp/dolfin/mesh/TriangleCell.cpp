...
std::size_t TriangleCell::num_entities(std::size_t dim) const
{
  ...
  case 2:
    return 1; // cells
  default:
    LOG(ERROR) << "Cannot access number of entities of triangle cell."
...
double TriangleCell::volume(const MeshEntity& triangle) const
{
  ...
  if (triangle.dim() != 2)
  {
    // glog::error("TriangleCell.cpp", "compute volume (area) of triangle
    // cell",
...
double TriangleCell::circumradius(const MeshEntity& triangle) const
{
  ...
  if (triangle.dim() != 2)
  {
    // glog::error("TriangleCell.cpp", "compute diameter of triangle cell",
    //               "Illegal mesh entity, not a triangle");
    throw std::runtime_error("Illegal topological dimension");
  }
...
geometry::Point TriangleCell::normal(const Cell& cell, std::size_t facet) const
{
  ...
  {
    // glog::error("TriangleCell.cpp", "find normal",
...
geometry::Point TriangleCell::cell_normal(const Cell& cell) const
{
  ...
  if (gdim > 3)
  {
    // glog::error("TriangleCell.cpp", "compute cell normal",
...
std::size_t TriangleCell::find_edge(std::size_t i, const Cell& cell) const
{
  ...
  // glog::error("TriangleCell.cpp", "find specified edge in cell",
