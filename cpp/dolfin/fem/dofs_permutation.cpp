  ...
  default:
                              ...
                              const std::vector<std::int64_t> cell_vs)
    {
      if (cell_vs[v[0]] < cell_vs[v[1]] and cell_vs[v[0]] < cell_vs[v[2]])
        return Eigen::Array<std::int8_t, 1, 2>(0,
                                               cell_vs[v[1]] > cell_vs[v[2]]);
      else if (cell_vs[v[1]] < cell_vs[v[0]] and cell_vs[v[1]] < cell_vs[v[2]])
        return Eigen::Array<std::int8_t, 1, 2>(1,
                                               cell_vs[v[2]] > cell_vs[v[0]]);
      else if (cell_vs[v[2]] < cell_vs[v[0]] and cell_vs[v[2]] < cell_vs[v[1]])
        return Eigen::Array<std::int8_t, 1, 2>(2,
                                               cell_vs[v[0]] > cell_vs[v[1]]);
      else
      {
                                 ...
                                 const std::vector<std::int64_t> cell_vs)
    {
          ...
          and cell_vs[v[0]] < cell_vs[v[3]])
      {
        const Eigen::Array<std::int8_t, 1, 2> tri_orders
            = calculate_triangle_orders(
                Eigen::Array<int, 1, 3>(v[1], v[2], v[3]), cell_vs);
        return Eigen::Array<std::int8_t, 1, 4>(0, 0, tri_orders[0],
                                               tri_orders[1]);
      }
      else if (cell_vs[v[1]] < cell_vs[v[0]] and cell_vs[v[1]] < cell_vs[v[2]]
               and cell_vs[v[1]] < cell_vs[v[3]])
      {
        const Eigen::Array<std::int8_t, 1, 2> tri_orders
            = calculate_triangle_orders(
                Eigen::Array<int, 1, 3>(v[2], v[0], v[3]), cell_vs);
        return Eigen::Array<std::int8_t, 1, 4>(1, 0, tri_orders[0],
                                               tri_orders[1]);
      }
      else if (cell_vs[v[2]] < cell_vs[v[0]] and cell_vs[v[2]] < cell_vs[v[1]]
               and cell_vs[v[2]] < cell_vs[v[3]])
      {
        const Eigen::Array<std::int8_t, 1, 2> tri_orders
            = calculate_triangle_orders(
                Eigen::Array<int, 1, 3>(v[0], v[1], v[3]), cell_vs);
        return Eigen::Array<std::int8_t, 1, 4>(2, 0, tri_orders[0],
                                               tri_orders[1]);
      }
               ...
               and cell_vs[v[3]] < cell_vs[v[2]])
      {
        const Eigen::Array<std::int8_t, 1, 2> tri_orders
            = calculate_triangle_orders(
                Eigen::Array<int, 1, 3>(v[1], v[0], v[2]), cell_vs);
        return Eigen::Array<std::int8_t, 1, 4>(0, 1, tri_orders[0],
                                               tri_orders[1]);
      }
      else
      {
                                   ...
                                   const std::vector<std::int64_t> cell_vs)
    {
      if (cell_vs[v[0]] < cell_vs[v[1]] and cell_vs[v[0]] < cell_vs[v[2]]
        ...
        return Eigen::Array<std::int8_t, 1, 2>(0,
               ...
               and cell_vs[v[1]] < cell_vs[v[3]])
        return Eigen::Array<std::int8_t, 1, 2>(1,
                                               cell_vs[v[3]] > cell_vs[v[0]]);
      else if (cell_vs[v[3]] < cell_vs[v[0]] and cell_vs[v[3]] < cell_vs[v[1]]
               and cell_vs[v[3]] < cell_vs[v[2]])
        return Eigen::Array<std::int8_t, 1, 2>(2,
                                               cell_vs[v[2]] > cell_vs[v[1]]);
      else if (cell_vs[v[2]] < cell_vs[v[0]] and cell_vs[v[2]] < cell_vs[v[1]]
               and cell_vs[v[2]] < cell_vs[v[3]])
        return Eigen::Array<std::int8_t, 1, 2>(3,
                                               cell_vs[v[0]] > cell_vs[v[3]]);
      else
      {
        throw std::runtime_error(
                                ...
                                const std::vector<std::int64_t> cell_vs)
    {
          ...
          and cell_vs[v[0]] < cell_vs[v[7]])
      {
        const Eigen::Array<std::int8_t, 1, 2> tri_orders
            = calculate_triangle_orders(
                Eigen::Array<int, 1, 3>(v[1], v[2], v[4]), cell_vs);
        return Eigen::Array<std::int8_t, 1, 4>(0, 0, tri_orders[0],
                                               tri_orders[1]);
      }
      else if (cell_vs[v[1]] < cell_vs[v[0]] and cell_vs[v[1]] < cell_vs[v[2]]
               and cell_vs[v[1]] < cell_vs[v[3]]
               and cell_vs[v[1]] < cell_vs[v[4]]
               and cell_vs[v[1]] < cell_vs[v[5]]
               and cell_vs[v[1]] < cell_vs[v[6]]
               and cell_vs[v[1]] < cell_vs[v[7]])
      {
        const Eigen::Array<std::int8_t, 1, 2> tri_orders
            = calculate_triangle_orders(
                Eigen::Array<int, 1, 3>(v[3], v[0], v[5]), cell_vs);
        return Eigen::Array<std::int8_t, 1, 4>(1, 0, tri_orders[0],
                                               tri_orders[1]);
      }
      else if (cell_vs[v[2]] < cell_vs[v[0]] and cell_vs[v[2]] < cell_vs[v[1]]
               and cell_vs[v[2]] < cell_vs[v[3]]
               and cell_vs[v[2]] < cell_vs[v[4]]
               and cell_vs[v[2]] < cell_vs[v[5]]
               and cell_vs[v[2]] < cell_vs[v[6]]
               and cell_vs[v[2]] < cell_vs[v[7]])
      {
        const Eigen::Array<std::int8_t, 1, 2> tri_orders
            = calculate_triangle_orders(
                Eigen::Array<int, 1, 3>(v[0], v[3], v[6]), cell_vs);
        return Eigen::Array<std::int8_t, 1, 4>(3, 0, tri_orders[0],
                                               tri_orders[1]);
      }
      else if (cell_vs[v[3]] < cell_vs[v[0]] and cell_vs[v[3]] < cell_vs[v[1]]
               and cell_vs[v[3]] < cell_vs[v[2]]
               and cell_vs[v[3]] < cell_vs[v[4]]
               and cell_vs[v[3]] < cell_vs[v[5]]
               and cell_vs[v[3]] < cell_vs[v[6]]
               and cell_vs[v[3]] < cell_vs[v[7]])
      {
        const Eigen::Array<std::int8_t, 1, 2> tri_orders
            = calculate_triangle_orders(
                Eigen::Array<int, 1, 3>(v[1], v[2], v[7]), cell_vs);
        return Eigen::Array<std::int8_t, 1, 4>(2, 0, tri_orders[0],
                                               tri_orders[1]);
      }
      else if (cell_vs[v[4]] < cell_vs[v[0]] and cell_vs[v[4]] < cell_vs[v[1]]
               and cell_vs[v[4]] < cell_vs[v[2]]
               and cell_vs[v[4]] < cell_vs[v[3]]
               and cell_vs[v[4]] < cell_vs[v[5]]
               and cell_vs[v[4]] < cell_vs[v[6]]
               and cell_vs[v[4]] < cell_vs[v[7]])
      {
        const Eigen::Array<std::int8_t, 1, 2> tri_orders
            = calculate_triangle_orders(
                Eigen::Array<int, 1, 3>(v[0], v[6], v[5]), cell_vs);
        return Eigen::Array<std::int8_t, 1, 4>(0, 1, tri_orders[0],
                                               tri_orders[1]);
      }
      else if (cell_vs[v[5]] < cell_vs[v[0]] and cell_vs[v[5]] < cell_vs[v[1]]
               and cell_vs[v[5]] < cell_vs[v[2]]
               and cell_vs[v[5]] < cell_vs[v[3]]
               and cell_vs[v[5]] < cell_vs[v[4]]
               and cell_vs[v[5]] < cell_vs[v[6]]
               and cell_vs[v[5]] < cell_vs[v[7]])
      {
        const Eigen::Array<std::int8_t, 1, 2> tri_orders
            = calculate_triangle_orders(
                Eigen::Array<int, 1, 3>(v[4], v[7], v[1]), cell_vs);
        return Eigen::Array<std::int8_t, 1, 4>(0, 2, tri_orders[0],
                                               tri_orders[1]);
      }
      else if (cell_vs[v[6]] < cell_vs[v[0]] and cell_vs[v[6]] < cell_vs[v[1]]
               and cell_vs[v[6]] < cell_vs[v[2]]
               and cell_vs[v[6]] < cell_vs[v[3]]
               and cell_vs[v[6]] < cell_vs[v[4]]
               and cell_vs[v[6]] < cell_vs[v[5]]
               and cell_vs[v[6]] < cell_vs[v[7]])
      {
        const Eigen::Array<std::int8_t, 1, 2> tri_orders
            = calculate_triangle_orders(
                Eigen::Array<int, 1, 3>(v[7], v[4], v[2]), cell_vs);
        return Eigen::Array<std::int8_t, 1, 4>(2, 2, tri_orders[0],
                                               tri_orders[1]);
      }
      else if (cell_vs[v[7]] < cell_vs[v[0]] and cell_vs[v[7]] < cell_vs[v[1]]
               and cell_vs[v[7]] < cell_vs[v[2]]
               and cell_vs[v[7]] < cell_vs[v[3]]
               and cell_vs[v[7]] < cell_vs[v[4]]
               and cell_vs[v[7]] < cell_vs[v[5]]
               and cell_vs[v[7]] < cell_vs[v[6]])
      {
        const Eigen::Array<std::int8_t, 1, 2> tri_orders
            = calculate_triangle_orders(
                Eigen::Array<int, 1, 3>(v[3], v[5], v[6]), cell_vs);
        return Eigen::Array<std::int8_t, 1, 4>(2, 1, tri_orders[0],
                                               tri_orders[1]);
      }
      else
      {
    ...
    /// @return Permutations to rotate and reflect the dofs
    std::array<std::vector<int>, 2>
    quadrilateral_rotation_and_reflection(const int face_dofs,
                                          const int blocksize)
    {
      // This will only be called at most once for each mesh
      const int blocks = face_dofs / blocksize;
                                         ...
                                         const int blocksize)
    {
      ...
      for (int side = side_length; side > 0; --side)
      {
        ...
        for (int row = 0; row < side; ++row)
        {
          ...
          for (int sub = 2 + (side_length - side); sub <= side_length - row;
               ++sub)
          {
            dof -= sub;
            for (int k = 0; k < blocksize; ++k)
    ...
    /// @return Permutations to rotate and reflect the dofs
    std::array<std::vector<int>, 4>
    hexahedron_rotations_and_reflection(const int volume_dofs,
                                        const int blocksize)
    {
      // This will only be called at most once for each mesh
      const int blocks = volume_dofs / blocksize;
      ...
      std::vector<int> rotation1(volume_dofs);
      {
        ...
        for (int lst = area - side_length; lst < blocks; lst += area)
          for (int st = lst; st < lst + side_length; ++st)
            for (int dof = st; dof >= lst - area + side_length;
                 dof -= side_length)
              for (int k = 0; k < blocksize; ++k)
