 ...
 const std::vector<double> &triangulation)
{
  ...
  // simplices of the flat triangulation vector. Note that we assume
  // that the simplices are of the same type as the cell.
  ...
  for (std::size_t i = 0; i < triangulation.size()/offset; ++i)
  {
    ...
    case 2:
      local_triangulation =
        IntersectionTriangulation::triangulate_intersection_triangle_triangle(simplex_cell,
                                                                              simplex);
      break;
    case 3:
      local_triangulation =
        IntersectionTriangulation::triangulate_intersection_tetrahedron_tetrahedron(simplex_cell,
                                                                                    simplex);
      break;
    default:
      dolfin_error("IntersectionTriangulation.cpp",
 ...
 const std::vector<Point>& tri_1)
{
  ...
  for (std::size_t i = 0; i < 3; i++)
  {
    const Point p0 = tri_0[i];
    const Point p1 = tri_1[i];
