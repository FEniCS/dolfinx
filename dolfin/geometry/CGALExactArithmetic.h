...
namespace
{
  ...
  inline Tetrahedron_3 convert_to_cgal(const dolfin::Point& a,
				       const dolfin::Point& b,
				       const dolfin::Point& c,
				       const dolfin::Point& d)
  {
    return convert_to_cgal_3d(a, b, c, d);
  }
...
namespace dolfin
{
  ...
  inline bool cgal_collides_tetrahedron_point(const Point& p0,
					      ...
					      const Point& q0)
  {
    const Tetrahedron_3 tet = convert_to_cgal(p0, p1, p2, p3);
  ...
  inline bool cgal_collides_tetrahedron_triangle(const Point& p0,
						 ...
						 const Point& q2)
  {
    return CGAL::do_intersect(convert_to_cgal(p0, p1, p2, p3),
			      convert_to_cgal_3d(q0, q1, q2));
  }
							  ...
							  const Point& q1)
  {
	...
	return convert_from_cgal(*s);
      ...
      {
	dolfin::error("Unexpected behavior in CGALExactArithmetic triangulate_segment_segment_3d");
      }
    }
							 ...
							 const Point& q1)
  {
	...
	if (s->target() != I0.source() && s->target() != I0.target() && s->target() != I1.source() && s->target() != I1.target())
      ...
      {
	dolfin::error("Unexpected behavior in CGALExactArithmetic cgal_intersection_segment_interior_segment_interior_3d");
      }
    }
							   ...
							   const Point& q1)
  {
	...
	return convert_from_cgal(*s);
      ...
      {
	dolfin::error("Unexpected behavior in CGALExactArithmetic cgal_intersection_triangle_segment_3d");
      }
    }
    return std::vector<dolfin::Point>();
  }
							    ...
							    const Point& q2)
  {
	...
	for (Point_3 p : *cgal_points)
	{
      ...
      {
	dolfin::error("Unexpected behavior in CGALExactArithmetic cgal_intersection_triangle_triangle_3d");
      }
					...
					const Point& q2)
  {
    ...
    std::vector<Point> intersection = cgal_intersection_triangle_triangle_3d(p0, p1, p2, q0, q1, q2);
