...
public:
  ...
  ///         A vector of points s.t. the intersection is the convex hull of
  ///         these points.
  static std::vector<Point> intersection(const std::vector<Point>& points_0,
  ...
  static std::vector<Point> intersection_point_point_2d(const Point& p0,
                                                        const Point& q0);

  /// Compute intersection of points p0 and q0 (3D)
  static std::vector<Point> intersection_point_point_3d(const Point& p0,
                                                        const Point& q0);

  /// Compute intersection of segment p0-p1 with point q0 (1D)
  static std::vector<double> intersection_segment_point_1d(double p0, double p1,
                                                           double q0);

  /// Compute intersection of segment p0-p1 with point q0 (2D)
  static std::vector<Point> intersection_segment_point_2d(const Point& p0,
                                                          const Point& p1,
                                                          const Point& q0);

  /// Compute intersection of segment p0-p1 with point q0 (3D)
  static std::vector<Point> intersection_segment_point_3d(const Point& p0,
                                                          const Point& p1,
                                                          const Point& q0);

  /// Compute intersection of triangle p0-p1-p2 with point q0 (2D)
  static std::vector<Point> intersection_triangle_point_2d(const Point& p0,
                                                           const Point& p1,
                                                           const Point& p2,
                                                           const Point& q0);

  /// Compute intersection of triangle p0-p1-p2 with point q0 (3D)
  static std::vector<Point> intersection_triangle_point_3d(const Point& p0,
                                                           const Point& p1,
                                                           const Point& p2,
                                                           const Point& q0);

  /// Compute intersection of tetrahedron p0-p1-p2-p3 with point q0 (3D)
  static std::vector<Point> intersection_tetrahedron_point_3d(const Point& p0,
                                                              const Point& p1,
                                                              const Point& p2,
                                                              const Point& p3,
                                                              const Point& q0);
  ...
  intersection_segment_segment_1d(double p0, double p1, double q0, double q1);

  /// Compute intersection of segment p0-p1 with segment q0-q1 (2D)
  static std::vector<Point> intersection_segment_segment_2d(const Point& p0,
                                                            const Point& p1,
                                                            const Point& q0,
                                                            const Point& q1);

  /// Compute intersection of segment p0-p1 with segment q0-q1 (3D)
  static std::vector<Point> intersection_segment_segment_3d(const Point& p0,
                                                            const Point& p1,
                                                            const Point& q0,
                                                            const Point& q1);

  /// Compute intersection of triangle p0-p1-p2 with segment q0-q1 (2D)
  static std::vector<Point> intersection_triangle_segment_2d(const Point& p0,
                                                             const Point& p1,
                                                             const Point& p2,
                                                             const Point& q0,
                                                             const Point& q1);
  ...
  static std::vector<Point> intersection_triangle_segment_3d(const Point& p0,
                                                             const Point& p1,
                                                             const Point& p2,
                                                             const Point& q0,
                                                             const Point& q1);
  ...
  intersection_tetrahedron_segment_3d(const Point& p0, const Point& p1,
                                      const Point& p2, const Point& p3,
                                      const Point& q0, const Point& q1);

  /// Compute intersection of triangle p0-p1-p2 with triangle q0-q1-q2 (2D)
  static std::vector<Point>
  intersection_triangle_triangle_2d(const Point& p0, const Point& p1,
                                    const Point& p2, const Point& q0,
                                    const Point& q1, const Point& q2);

  /// Compute intersection of triangle p0-p1-p2 with triangle q0-q1-q2 (3D)
  static std::vector<Point>
  intersection_triangle_triangle_3d(const Point& p0, const Point& p1,
                                    const Point& p2, const Point& q0,
                                    const Point& q1, const Point& q2);

  /// Compute intersection of tetrahedron p0-p1-p2-p3 with triangle q0-q1-q2
  /// (3D)
  static std::vector<Point> intersection_tetrahedron_triangle_3d(
      const Point& p0, const Point& p1, const Point& p2, const Point& p3,
      const Point& q0, const Point& q1, const Point& q2);

  /// Compute intersection of tetrahedron p0-p1-p2-p3 with tetrahedron
  /// q0-q1-q2-q3 (3D)
  static std::vector<Point> intersection_tetrahedron_tetrahedron_3d(
      const Point& p0, const Point& p1, const Point& p2, const Point& p3,
      const Point& q0, const Point& q1, const Point& q2, const Point& q3);

private:
  /// Compute intersection of triangle p0-p1-p2 with segment q0-q1 (3D)
  static std::vector<Point> _intersection_triangle_segment_3d(const Point& p0,
                                                              const Point& p1,
                                                              const Point& p2,
                                                              const Point& q0,
                                                              const Point& q1);

  static std::vector<Point> _intersection_tetrahedron_tetrahedron_3d(
      const Point& p0, const Point& p1, const Point& p2, const Point& p3,
      const Point& q0, const Point& q1, const Point& q2, const Point& q3);
};
}
