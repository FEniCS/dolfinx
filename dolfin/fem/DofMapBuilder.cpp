...
void DofMapBuilder::build(DofMap& dofmap,
                          ...
                          bool distributed)
{
  // FIXME: Improve reuse between this and the standard build function
  ...
  for (dolfin::CellIterator cell(dolfin_mesh); !cell.end(); ++cell)
  {
    ...
    // Tabulate standard UFC dof map
    std::vector<uint>& dofs = dofmap._dofmap[cell->index()];
    dofmap._ufc_dofmap->tabulate_dofs(&dofs[0], ufc_mesh, ufc_cell);

    // Renumber by counting, starting at zero
    for (uint i = 0; i < dofs.size(); i++)
    {
      map_iterator it = restricted_dofs.find(dofs[i]);
      if (it == restricted_dofs.end())
      {
        const uint dof = restricted_dofs.size();
        restricted_dofs[dofs[i]] = dof;
        dofs[i] = dof;
      }
      else
        dofs[i] = it->second;
