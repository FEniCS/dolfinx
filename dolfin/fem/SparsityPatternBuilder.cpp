...
void SparsityPatternBuilder::build(GenericSparsityPattern& sparsity_pattern,
				                           const Mesh& mesh, 
                                   UFC& ufc, 
                                   const std::vector<const DofMap*> dof_maps)
{
  ...
  */
  ...
  if (ufc.form.num_interior_facet_integrals() != 0)
  {
    ...
    for (FacetIterator facet(mesh); !facet.end(); ++facet)
    {
      ...
      // Tabulate dofs for each dimension on macro element and get local macro dimensions
      for (uint i = 0; i < ufc.form.rank(); ++i)
      {
        ...
        ufc.macro_local_dimensions[i] = dof_maps[i]->local_dimension(ufc.cell0)
                                      + dof_maps[i]->local_dimension(ufc.cell1);
      }
