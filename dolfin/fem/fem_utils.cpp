    ...
    const std::vector<std::size_t> & entity_indices)
{
  ...
  for (std::size_t restricted_entity_index = 0; restricted_entity_index < num_marked_entities; ++restricted_entity_index)
  {
    MeshEntity entity(mesh, entity_dim, entity_indices[restricted_entity_index]);

    // Get the first cell connected to the entity
    const Cell cell(mesh, entity.entities(top_dim)[0]);

    // Find local entity number
    for (std::size_t i = 0; i < cell.num_entities(entity_dim); ++i)
    {
      if (cell.entities(entity_dim)[i] == entity.index())
      {
        local_entity_ind = i;
        break;
      }
    ...
    for (std::size_t local_dof = 0; local_dof < dofs_per_entity; ++local_dof)
    {
      ...
      entity_to_dofs[dofs_per_entity*restricted_entity_index + local_dof] = global_dof;
    }
  }
...
//-----------------------------------------------------------------------------
std::vector<dolfin::la_index> dolfin::aggregate_entity_dofs(
    const FunctionSpace& space,
    std::size_t entity_dim,
    const std::vector<std::size_t> & entity_indices)
{
  ...
  // Get some dimensions
  const std::size_t top_dim = mesh.topology().dim();
  const std::size_t dofs_per_entity = dofmap.num_entity_dofs(entity_dim);
