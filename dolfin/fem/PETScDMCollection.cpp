 ...
 const FunctionSpace& fine_space)
{
  ...
  // matrix
  std::vector<std::vector<dolfin::la_index_t>> col_indices(m_owned, std::vector<dolfin::la_index_t>(eldim));
  ...
  for (unsigned int i = 0; i < found_ids.size(); ++i)
  {
    ...
    for (unsigned k = 0; k < data_size; k++)
    {
      ...
      int p = finemap->index_map()->global_block_index_owner(global_fine_dof/data_size);
      ...
      for (unsigned j = 0; j < eldim; j++)
      {
        ...
        int pc = coarsemap->index_map()->global_block_index_owner(coarse_dof/data_size);
        if (p == pc)
