...
//=============================================================================
// In this file we declare some typemaps for the dolfin::ArrayView type
//=============================================================================

//-----------------------------------------------------------------------------
// Macro for defining an in-typemap for NumPy array -> dolfin::ArrayView
//
// TYPE       : The primitive type
// TYPECHECK  : The SWIG specific name of the type used in the array type checks values
//              SWIG use: INT32 for integer, DOUBLE for double aso.
...
//              argument
//-----------------------------------------------------------------------------
%define IN_NUMPY_TYPEMAP_FOR_DOLFIN_ARRAYVIEW(TYPE, TYPECHECK, NUMPYTYPE, TYPE_NAME, ARGNAME, CONSTARRAY)

%typemap(in, fragment=convert_numpy_to_array_with_check(TYPE_NAME)) (CONSTARRAYVIEW dolfin::ArrayView<TYPE> &ARGNAME) (std::size_t size, TYPE* data)
{
  if (!convert_numpy_to_array_with_check_ ## TYPE_NAME($input, size, data))
    return NULL;
  $1 = new dolfin::ArrayView<TYPE>(size, data);
}

// Clean up the constructed Array
//%typemap(freearg) CONSTARRAYVIEW dolfin::ArrayView<TYPE>& ARGNAME
//{
//  delete $1;
//}

%typecheck(SWIG_TYPECHECK_ ## TYPECHECK ## _ARRAYVIEW) CONSTARRAYVIEW dolfin::ArrayView<TYPE> & ARGNAME
{
  $1 = PyArray_Check($input) ? 1 : 0;
}
...
//-----------------------------------------------------------------------------
// Macro for defining an out-typemap for dolfin::ArrayView -> NumPy array
//
// TYPE       : The primitive type
// NUMPYTYPE  : The NumPy type that is going to be checked for
//-----------------------------------------------------------------------------
%define OUT_NUMPY_TYPEMAP_FOR_DOLFIN_ARRAYVIEW(TYPE, NUMPYTYPE)

%typemap(out) dolfin::ArrayView<const TYPE>
{
  std::cout << "** In out typemap" << std::endl;
  // Create NumPy array
  npy_intp size = (&$1)->size();
  PyObject* op = PyArray_SimpleNew(1, &size, NUMPYTYPE);

  if (op == NULL)
    SWIG_exception(SWIG_TypeError, "Error in conversion of dolfin::ArrayView< TYPE > to NumPy array.");

  // Get data
  TYPE* data = reinterpret_cast<TYPE*>(PyArray_DATA(op));

  // Set data from ArrayView
  for (int i = 0; i < (&$1)->size(); i++)
    data[i] = (&$1)->operator[](i);
...
// Instantiate argument name specific typemaps for non const arguments
 //IN_NUMPY_TYPEMAP_FOR_DOLFIN_ARRAYVIEW(double, DOUBLE, NPY_DOUBLE, double, values, )
 //IN_NUMPY_TYPEMAP_FOR_DOLFIN_ARRAYVIEW(std::size_t, INT32, NPY_UINTP, size_t, indices,)
 //IN_NUMPY_TYPEMAP_FOR_DOLFIN_ARRAYVIEW(double, DOUBLE, NPY_DOUBLE, double, vertex_values, )
 //IN_NUMPY_TYPEMAP_FOR_DOLFIN_ARRAYVIEW(dolfin::la_index, INT32, NPY_UINTP, dolfin_index, indices,)

// Instantiate argument name independent typemaps for all
// const Array <{int, uint, double}>& arguments
 //IN_NUMPY_TYPEMAP_FOR_DOLFIN_ARRAYVIEW(double, DOUBLE, NPY_DOUBLE, double, , const)
 //IN_NUMPY_TYPEMAP_FOR_DOLFIN_ARRAYVIEW(std::size_t, INT32, NPY_UINTP, size_t, , const)
 //IN_NUMPY_TYPEMAP_FOR_DOLFIN_ARRAYVIEW(int, INT32, NPY_INT, int, , const)
 //IN_NUMPY_TYPEMAP_FOR_DOLFIN_ARRAYVIEW(dolfin::la_index, INT32, NPY_UINT, dolfin_index, , const)

OUT_NUMPY_TYPEMAP_FOR_DOLFIN_ARRAYVIEW(std::size_t, NPY_UINTP)
OUT_NUMPY_TYPEMAP_FOR_DOLFIN_ARRAYVIEW(int, NPY_INT)
 //OUT_NUMPY_TYPEMAP_FOR_DOLFIN_ARRAYVIEW(const int, NPY_INT)
OUT_NUMPY_TYPEMAP_FOR_DOLFIN_ARRAYVIEW(double, NPY_DOUBLE)


%typemap(out) dolfin::ArrayView<const dolfin::la_index>
{
  //std::cout << "***Inside type map" << std::endl;
...
%typemap(out) dolfin::ArrayView<const dolfin::la_index>
{
  ...
  $result = PyArray_SimpleNew(1, &adims, NPY_INT);
  const dolfin::la_index* data = static_cast<const dolfin::la_index*>(PyArray_DATA(reinterpret_cast<PyArrayObject*>($result)));
  std::copy($1.begin(), $1.end(), data);
}
*/
