...
//-----------------------------------------------------------------------------
// Ignore const array interface (used if the ArrayView type is a const)
//-----------------------------------------------------------------------------
%define CONST_ARRAYVIEW_IGNORES(TYPE)
%ignore dolfin::ArrayView<const TYPE>::array();
%ignore dolfin::ArrayView<const TYPE>::__setitem__;
%enddef
...
%define ARRAYVIEW_EXTENSIONS(TYPE, TEMPLATE_NAME, TYPE_NAME)

// Construct value wrapper for dolfin::ArrayView<TYPE>
// Valuewrapper is used so a return by value Array does not make an
// extra copy in any typemaps
%feature("valuewrapper") dolfin::ArrayView<TYPE>;

// Cannot construct an Array from another Array.
// Use NumPy Array instead
%ignore dolfin::ArrayView<TYPE>::ArrayView(const ArrayView& other);
%template(TEMPLATE_NAME ## ArrayView) dolfin::ArrayView<TYPE>;
%feature("docstring") dolfin::ArrayView::__getitem__ "Missing docstring";
%feature("docstring") dolfin::ArrayView::__setitem__ "Missing docstring";
%feature("docstring") dolfin::ArrayView::array "Missing docstring";
%extend dolfin::ArrayView<TYPE>
{
  TYPE _getitem(unsigned int i) const { return (*self)[i]; }
  void _setitem(unsigned int i, const TYPE& val) { (*self)[i] = val; }
...
// Run Array macros, which also instantiate the templates
//-----------------------------------------------------------------------------
CONST_ARRAYVIEW_IGNORES(double)
CONST_ARRAYVIEW_IGNORES(int)
ARRAYVIEW_EXTENSIONS(double, Double, double)
ARRAYVIEW_EXTENSIONS(unsigned int, UInt, uint)
ARRAYVIEW_EXTENSIONS(int, Int, int)
