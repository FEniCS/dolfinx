...
{
  rampfactor =  ode.get("ODE fixed-point stabilization ramp").get_real();

  alpha = ode.get("ODE fixed-point damping").get_real();
                                             ...
                                             real d0, real d1)
{
  ...
  for (uint n = 0; n < method.nsize(); n++)
  {
    ...
    for (uint i = 0; i < ts.N; i++)
    {
      ts.x[noffset + i] /= alpha;      
      ts.x[noffset + i] += ts.u0[i] - ts.x[noffset+i];
      ts.x[noffset + i] /= k;
    }
  }
  ...
  for (uint n = 0; n < method.nsize(); n++)
  {
    ...
    for (uint i = 0; i < ts.N; i++) 
    {
      ...
      for (uint m = 0; m < method.qsize(); m++)
      {
	ts.x[noffset + i] += method.nweight(n,m) * ts.fq[m*ts.N + i];
      }
    }
  }
