...
//-----------------------------------------------------------------------------
void MeshTransformation::rescale(Mesh& mesh, const double scale, const Point& center )
{
	// Get mesh geometry
	MeshGeometry& geometry = mesh.geometry();
	const std::size_t gdim = geometry.dim();
	...
	// 1. Displace all points by -dx so that the center coincides with the origin: x = x - x_center
