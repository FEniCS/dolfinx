...
void ALE::move(Mesh& mesh, Mesh& new_boundary,
               const MeshFunction<unsigned int>& vertex_map,
               ALEMethod method)
{
  // Only implemented in 3D so far
  if (mesh.topology().dim() != 3)
    error("Mesh interpolation only implemented in 3D so far.");
...
//-----------------------------------------------------------------------------
void ALE::meanValue(real* new_x, unsigned int dim, Mesh& new_boundary,
                    Mesh& mesh, const MeshFunction<unsigned int>& vertex_map,
                    Vertex& vertex)
{
...
//-----------------------------------------------------------------------------
void ALE::computeWeights(real* w, real** u, real* d,
                         unsigned int dim, unsigned int num_vertices)
{
...
//-----------------------------------------------------------------------------
real ALE::dist(const real* x, const real* y, unsigned int dim)
{
  real s = 0.0;
  for (unsigned int i = 0; i < dim; i++)
    s += (x[i] - y[i])*(x[i] - y[i]);
