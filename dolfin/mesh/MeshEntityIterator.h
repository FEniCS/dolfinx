...
public:
      ...
      : _entity(), _pos(0), pos_end(0), index(0)
  {
    ...
    if (opt == "regular")
      ...
      dolfin_error("MeshEntityIterator.h", "initialize MeshEntityIterator",
                   "unknown opt=\"%s\", choose from "
                   "opt=[\"regular\", \"ghost\", \"all\"]",
                   opt.c_str());
  }
  ...
  MeshEntity& operator*() { return *operator->(); }

  /// Member access operator
  MeshEntity* operator->()
  {
    _entity._local_index = (index ? index[_pos] : _pos);
    return &_entity;
  }
