...
TetrahedronCell::triangulate_intersection(const Cell& cell_0, const Cell& cell_1) const
{
  ...
  std::vector<Point> tmp; tmp.reserve(points.size());
  ...
  if (points.size()==4) {
    ...
    if (std::abs(det)>Tettol) {
      if (det<-Tettol) {
        std::swap(points[0],points[1]);
      }
      // 4 tets in 3D => 12 doubles
                                          ...
                                          Point& pt) const
{
  ...
  const double Samepointtol=1e-13;
