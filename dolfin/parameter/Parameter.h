...
public:
  ...
  Parameter(std::string key, T x)
      : _value(x), _access_count(0), _change_count(0), _is_set(true), _key(key),
  ...
  Parameter(std::string key, T min, T max)
      : _value(T(0)), _range(std::array<T, 2>({{min, max}})), _access_count(0),
        _change_count(0), _is_set(false), _key(key),
        _description("missing description")
  {
    check_key(key);
  }
  ...
  Parameter(std::string key, std::set<std::string> range)
      : _value(std::string("")), _range(range), _access_count(0),
        _change_count(0), _is_set(false), _key(key),
        _description("missing description")
  {
    check_key(key);
