...
namespace dolfin
{
  //FIXME: FREDRIK: Can we use the PETScVectorDeleter from PETScVector.h here?
  class PETScCuspVectorDeleter
  {
  public:
    void operator() (Vec* x)
    {
  ...
  /// This class provides a vector class based on PETSc's Cusp vectors.
  /// They are implemented as using the Cusp library for GPU-accelerated 
  /// linear algebra. It is a simple wrapper for a PETSc cusp vector pointer 
  /// (Vec) implementing the GenericVector interface.
  ///
  /// The interface is intentionally simple. For advanced usage,
  /// access the PETSc Vec pointer using the function vec() and
  /// use the standard PETSc interface.

  class PETScCuspVector : public GenericVector, public PETScObject
  {
  public:

    /// Create empty vector
    explicit PETScCuspVector(std::string type="global");

    /// Create vector of size N
    PETScCuspVector(uint N, std::string type="global");

    /// Create vector
    PETScCuspVector(const GenericSparsityPattern& sparsity_pattern);

    /// Copy constructor
    PETScCuspVector(const PETScCuspVector& x);

    /// Create vector from given PETSc Vec pointer
    explicit PETScCuspVector(boost::shared_ptr<Vec> x);

    /// Destructor
    virtual ~PETScCuspVector();
    ...
    virtual PETScCuspVector* copy() const;
    ...
    virtual const PETScCuspVector& operator*= (double a);

    /// Multiply vector by another vector pointwise
    virtual const PETScCuspVector& operator*= (const GenericVector& x);
