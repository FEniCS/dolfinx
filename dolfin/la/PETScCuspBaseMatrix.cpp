...
//-----------------------------------------------------------------------------
dolfin::uint PETScCuspBaseMatrix::size(uint dim) const
{
  assert(dim <= 1);
  if (A)
  {
...
//-----------------------------------------------------------------------------
std::pair<dolfin::uint, dolfin::uint> PETScCuspBaseMatrix::local_range(uint dim) const
{
  assert(dim <= 1);
  if (dim == 1)
...
//-----------------------------------------------------------------------------
void PETScCuspBaseMatrix::resize(GenericVector& y, uint dim) const
{
  assert(A);
  ...
  boost::shared_ptr<Vec> x(new Vec(0), PETScCuspVectorDeleter());
  if (dim == 0)
