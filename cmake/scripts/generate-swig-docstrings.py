...
We are trying to achieve a layout as close as possible to the numpy docstring
format, as this is one of few available standards. There also exists
a bridge to sphinx in the "napoleon" extension.
...
def replace_example(text, classname, signature):
    ...
    if not "*Example*" in text:
        ...
        print('Example in ', classname, signature)
...
def map_argument_and_return_types(text, classnames):
    """Map C++ types in the @param and @return sections to corresponding
    ...
    if "@return" in new_text:
        # Get lines and find line number with @return
        lines = new_text.split("\n")
        r_index = ["@return" in l for l in lines].index(True)
        l = "".join(lines[r_index].split()[1:])
        lines[r_index]  = "  Returns\n  -------\n  %s\n"%(map_cpp_type(l.strip(), classnames))
        new_text = "\n".join(lines)

    if "@param" in new_text:
        # Get lines and find line numbers with @param
        lines = new_text.split("\n")
        done_header = False
        for i, l in enumerate(lines):
            if "@param" in l:
                if not done_header:
                    ...
                    newl = "  "
                ls = l.split()
                lines[i] = newl + ls[1] + " : " + map_cpp_type(ls[2].strip("()"), classnames)
...
def write_docstrings(output_file, module, header, docs, classnames):
    ...
    output("// Documentation extracted from: (module=%s, header=%s)\n" % (module, header))
