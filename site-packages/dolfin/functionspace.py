...
__date__ = "2008-11-03 -- 2008-12-11"
...
#
# Comments marked 'UFL:' are the places that needs to be updated for the move to UFL.
#

__all__ = ["FunctionSpace", "MixedFunctionSpace", "VectorFunctionSpace",
           "FunctionSpaceBase","FunctionSpaceFromCpp","FunctionSpaceFromCpp",
           "create_ufc_function_spaces"]

import copy

# Import FFC and SWIG-generated extension module (DOLFIN C++)
import ffc # UFL: import ufl instead
import cpp
from jit import jit
...
class FunctionSpaceBase(cpp.FunctionSpace):
    ...
    def __init__(self, mesh, element):
        ...
        if not isinstance(element, (ffc.FiniteElementBase)): # UFL: ufl.FiniteElementBase
            cpp.error("Illegal finite element for creation of function space, not a finite element: " + str(element))

        # Store element
        # Note: self._element cannot be a private attribut as we want to be able to 
    ...
    def __str__(self):
        "Pretty-print."
        return "<Function space of dimension %d (%s)>" % (self.dofmap().global_dimension(), str(self._element))

class FunctionSpaceFromCpp(FunctionSpaceBase):
    " FunctionSpace represents a finite element function space."
    def __init__(self, cppV):
        ...
        # Get the ffc.FiniteElement from the compiled element sigature
        self._element = eval(cppV.element().signature(), ffc.__dict__) # UFL
...
class FunctionSpace(FunctionSpaceBase):
    ...
    def __init__(self, mesh, family, degree):
        ...
        element = ffc.FiniteElement(family, domain, degree) # UFL: ufl.FiniteElement
...
class MixedFunctionSpace(FunctionSpaceBase):
    ...
    def __init__(self, spaces):
        ...
        # Create element
        element = ffc.MixedElement([V.element() for V in spaces]) # UFL: ufl.MixedElement(*[...])
