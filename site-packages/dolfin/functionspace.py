...
class FunctionSpaceBase(dolfin.cpp_FunctionSpace):
    ...
    def __init__(self, mesh, element):
        ...
        if not isinstance(mesh, dolfin.Mesh):
            dolfin.error("Illegal mesh for creation of function space, not a mesh: " + str(mesh))
        if not isinstance(element, (ffc.FiniteElementBase)):
            dolfin.error("Illegal finite element for creation of function space, not a finite element: " + str(element))
...
class FunctionSpace(FunctionSpaceBase):
    ...
    def __init__(self, mesh, family, degree):
        ...
        if not dim in dim2domain:
            dolfin.error("Invalid mesh dimension: " + str(dim))
...
class MixedFunctionSpace(FunctionSpaceBase):
    ...
    def __init__(self, spaces):
        ...
        # Check arguments
        if not len(spaces) > 0:
            dolfin.error("Mixed function space needs at least one subspace.")
        if not all([isinstance(V, FunctionSpaceBase) for V in spaces]):
            dolfin.error("Invalid subspaces: " + str(spaces))
        #if not all([V.mesh() == spaces[0].mesh() for V in spaces]):
        #    dolfin.error("Nonmatching meshes for mixed function space: " + str([V.mesh() for V in spaces]))
