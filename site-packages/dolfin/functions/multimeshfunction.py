...
class MultiMeshFunction(ufl.Coefficient, cpp.MultiMeshFunction):
    ...
    *Examples*
    ...
    def assign(self, rhs, parts=None):
        """
        Parameters: 
            rhs: Either a list of dolfin.Functions or a MultiMeshFunction
            parts: If rhs is a list of dolfin.Functions, then parts corresponds to 
                   the list of part indices where to assign.
        """
        # Assign a MultiMeshFunction into a MultiMeshFunction
	if parts is None:
            self.vector()[:]= rhs.vector()[:]
        elif len(rhs) >= 1 and len(rhs) == len(parts):
            # Assign a list of Functions into a MultiMeshFunction
    ...
    def interpolate(self,v):
        ...
        *Example of usage*
        ...
        # Developer note: Interpolate do not set inactive dofs to zero,
        # and should be fixed
        n = []
        vs = []
        # Check argument
        if isinstance(v, cpp.GenericFunction):
            num_parts = self._V.num_parts()
            for part in range(num_parts):
                vp = super(MultiMeshFunction, self).part(part,True)
                vp.interpolate(v)
                vs.append(vp)
                n.append(part)
            self.assign(vs,parts=n)
            ...
            if  self._V.multimesh().id() != v._V.multimesh().id():
                ...
                for part in range(num_parts):
                    vp = super(MultiMeshFunction, self).part(part,True)
                    vmm = v.part(part,True) # Deepcopy of part of argument
                    vp.interpolate(vmm)
                    vs.append(vp)
                    n.append(part)
                self.assign(vs,parts=n)
