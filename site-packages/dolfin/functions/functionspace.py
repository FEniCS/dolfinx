...
class FunctionSpace(ufl.FunctionSpace, cpp.FunctionSpace):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if nargs == 1:
            # FIXME: dolfin_error
            assert len(kwargs) == 0
            return FunctionSpaceFromCpp(args[0])
        else:
            o = object.__new__(cls, *args, **kwargs)
            #o.__class__ = FunctionSpace
            return o
    #    elif nargs == 2:
    #        return FunctionSpaceFromElement(*args, **kwargs)
    #    else:
    #        return _create_FunctionSpace(*args, **kwargs)
    ...
    def _init_from_ufl(self, mesh, element, constrained_domain=None):
        ...
        *Arguments*
        ...
        ufl.FunctionSpace.__init__(self, mesh.ufl_domain(), element) # TODO: Inherit from UFL

        # Lets be agressive in abusing dynamic typing shall we...
        self._dolfin_element = dolfin_element # TODO: Is it necessary to store this reference?
        #self.__class__ = FunctionSpace
    ...
    def _init_legacy(self, mesh, family, degree, form_degree=None,
    ...
    def ufl_function_space(self):
        ...
        #return super(ufl.FunctionSpace, self)
        return self
    ...
    def collapse(self, collapsed_dofs=False):
       ...
       *Returns*
        ...
        # Initialize the cpp.FunctionSpace
        cppV = cpp.FunctionSpace(mesh, dolfin_element, dolfin_dofmap)

        # Initialize the ufl.FunctionSpace
        cppV._ufl_function_space = ufl.FunctionSpace(mesh.ufl_domain(), element)
        #ufl.FunctionSpace.__init__(self, mesh.ufl_domain(), element) # TODO: Inherit from UFL

        # Lets be agressive in abusing dynamic typing shall we...
        cppV._dolfin_element = dolfin_element # TODO: Is it necessary to store this reference?
        ...
        # https://bitbucket.org/fenics-project/dolfin/issue/71)
        # TODO: Make constrained_domain private with getter? Attribute access breaks interface consistency.
        cppV.constrained_domain = constrained_domain

        return cppV


class FunctionSpaceFromCpp(FunctionSpace):
    "FunctionSpace represents a finite element function space."
    def __new__(cls, cppV):
        ...
        # FIXME: This fails probably because ufl.FunctionSpace uses __slots__
        cppV.__class__ = FunctionSpace
        ...
        ufl.FunctionSpace.__init__(cppV, ufl_domain, ufl_element) # TODO: Inherit from ufl.FunctionSpace

        return cppV


# FIXME: Fix merged docstrings
doc = """
Create function space on given mesh for given finite element.
