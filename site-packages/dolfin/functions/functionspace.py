...
def _get_common_mesh_and_constrained_domain(spaces, fstype):
    ...
    if not all(V.mesh().id() == mesh.id() for V in spaces):
        cpp.dolfin_error("functionspace.py", "Nonmatching meshes for " + fstype + " function space: " \
                  + str([V.mesh() for V in spaces]))
    # Also check that all spaces share same constrained_domain map we pass on
    if not all(V.dofmap().constrained_domain.id() == constrained_domain.id() for V in spaces):
        cpp.dolfin_error("functionspace.py", "Nonmatching constrained_domain for " + fstype + " function space: " \
...
class MixedFunctionSpace(FunctionSpaceBase):
    ...
    def __init__(self, spaces):
        ...
        *Examples of usage*
        ...
        # Get common mesh and constrained_domain, must all be the same
        mesh, constrained_domain = _get_common_mesh_and_constrained_domain(spaces, "mixed")
