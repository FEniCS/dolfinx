...
def create_ufl_cell(mesh):
    tdim = mesh.topology().dim()
    gdim = mesh.geometry().dim()
    if not tdim in dim2domain:
        cpp.dolfin_error("functionspace.py",
                         "create ufl cell",
                         "Not supported topological dimension: %d" % tdim)
    return ufl.Cell(dim2domain[tdim], ufl.Space(gdim))
