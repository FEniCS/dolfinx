...
class FunctionSpaceBase(cpp.FunctionSpace):
    ...
    def __init__(self, mesh, element):
        ...
        *Arguments*
        ...
        if not isinstance(mesh, cpp.Mesh):
            cpp.error("Illegal mesh for creation of function space, "\
                      "not a mesh: " + str(mesh))
        if not isinstance(element, (ufl.FiniteElementBase)):
            cpp.error("Illegal finite element for creation of function "\
                      "space, not a finite element: " + str(element))
    ...
    def collapse(self, collapsed_dofs=None):
        "dummy"
        args = (self,) if collapsed_dofs is None else (self, collapsed_dofs)
        return FunctionSpaceFromCpp(cpp.FunctionSpace.collapse(*args))

    # Assign generated docstring from the C++ libray
    collapse.__doc__ = cpp.FunctionSpace.collapse.__doc__
...
class FunctionSpaceFromCpp(FunctionSpaceBase):
    ...
    def __init__(self, cppV, element=None):
        ...
        if not isinstance(cppV, (cpp.FunctionSpace)):
            cpp.error("Illegal argument for creation of function space, not "\
                      "an other cpp.FunctionSpace: " + str(cppV))
    ...
    def __init__(self, mesh, family, degree, form_degree=None,
        ...
        *Examples of usage*
        ...
        if not isinstance(family, str):
            cpp.error("Illegal family for creation of function space, "\
                      "not a string: " + str(family))
        if not isinstance(degree, int):
            cpp.error("Illegal degree for creation of function space, "\
                      "not an integer: " + str(degree))
    ...
    def __init__(self, spaces):
        ...
        *Examples of usage*
        ...
        if not all(isinstance(V, FunctionSpaceBase) for V in spaces):
            cpp.error("Invalid subspaces: " + str(spaces))
        #if not all(V.mesh() == spaces[0].mesh() for V in spaces):
        #    cpp.error("Nonmatching meshes for mixed function space: " \
