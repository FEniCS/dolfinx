...
def _check_mul_and_division(e, linear_comb, scalar_weight=1.0):
    """
    ...
    if isinstance(e, Product):
        if isinstance(ops[0], ScalarValue) and isinstance(ops[1], (Product, Division)):
            linear_comb = _check_mul_and_division(ops[1], linear_comb, \
                                         scalar_weight*float(ops[0]))
        elif isinstance(ops[0], ScalarValue) and isinstance(ops[1], Function):
            linear_comb.append((ops[1], scalar_weight*float(ops[0])))
        elif isinstance(ops[0], ScalarValue) and isinstance(ops[1], Sum):
            linear_comb = _check_and_extract_functions(ops[1], linear_comb, \
                                                       scalar_weight*float(ops[0]))
        else:
            _error()
    ...
    # If we have a Division
        ...
        # If we have a Product or Division operand
        if isinstance(ops[1], ScalarValue) and isinstance(ops[0], (Product, Division)):
            linear_comb = _check_mul_and_division(ops[0], linear_comb, \
                                                  scalar_weight/float(ops[1]))
        # If we have a Scalar operand
        elif isinstance(ops[1], ScalarValue) and isinstance(ops[0], Function):
            linear_comb.append((ops[0], scalar_weight/float(ops[1])))
        elif isinstance(ops[1], ScalarValue) and isinstance(ops[0], Sum):
            linear_comb = _check_and_extract_functions(ops[0], linear_comb, \
                                                       scalar_weight*float(ops[1]))
        else:
            _error()
    else:
...
def _check_and_extract_functions(e, linear_comb=None, scalar_weight=1.0):
    ...
    # If not Product or Division we expect Sum
        ...
        for op in e.operands():
            linear_comb = _check_and_extract_functions(op, linear_comb, scalar_weight)
...
def _check_and_contract_linear_comb(expr, self):
    """
    Utility func for checking and contracting linear combinations of
    Functions
    """
    linear_comb = _check_and_extract_functions(expr)
    funcs = []
    weights = []
    funcspace = None
    ...
    *Examples*
    ...
    def __init__(self, *args):
        ...
        # If creating a dolfin.Function from a cpp.Function
            ...
            if  len(args) == 1:
                ...
                cpp.Function.__init__(self, V, args[0].vector())
    ...
    def assign(self, rhs):
        ...
        *Arguments*
        ...
        if isinstance(rhs, (cpp.Function, cpp.Expression)):
            # Avoid self assignment
            if self == rhs:
                return
                
            self._assign(rhs)
        elif isinstance(rhs, AlgebraOperator):
