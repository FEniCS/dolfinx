...
class Constant(ufl.Coefficient, cpp.Constant):
    ...
    def __init__(self, value, cell=None):
        ...
        *Arguments*
            ...
            cell
                Optional argument. A :py:class:`Cell <dolfin.cpp.Cell>` which defines 
                the geometrical dimensions the Constant is defined for.

        The data type Constant represents a constant value that is unknown at 
        compile-time. Its values can thus be changed without requiring re-generation
        and re-compilation of C++ code. 

        *Examples of usage*

        .. code-block:: python

            c = Constant(6.0)               # scalar
            p = Constant(pi/4) 
