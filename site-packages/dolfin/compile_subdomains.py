...
def expression_to_subdomain(e, classname):
    ...
    if not isinstance(e, str):
        raise RuntimeError("Invalid expression type: %s" % e)
...
def compile_subdomain_code(code, classnames=None, module_name=None):
    ...
    if module_name is None:
        module_name = "dolfin_compile_subdomain_%d" % md5.md5(repr(code) + repr(classnames)).hexdigest()
...
if __name__ == "__main__":
    code, cn = generate_subdomains(["x[0] >= 1.0-DOLFIN_EPS", "on_boundary && x[1] < 0.5+DOLFIN_EPS"])
    print code
    print cn
