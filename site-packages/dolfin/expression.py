...
def create_expression_class(name,
    ...
    def __init__(self, *args, **kwargs):
        ...
        if element is None and len(args) > 0:
            element = _auto_select_element(args[0], degree)
...
"""
    ...
    for i, cpp_base in enumerate(cpp_bases):
        ...
        return_expressions.append(create_expression_class("CompiledExpression", cpp_base)(V=spaces[i]))
...
def _auto_select_element(cpparg, degree=None):
    "Automatically select an appropriate element for cpparg."

    # Check fo None
    if cpparg is None:
        cpp.error("Unable to automatically select element for expression. "\
                  "You must specify it manually using the 'element' keyword argument.")
    ...
    if len(shape) == 0:
        cpp.debug("It's a scalar!")
        element = ufl.FiniteElement("Quadrature", None, degree)
    elif len(shape) == 1:
        cpp.debug("It's a vector!")
        element = ufl.VectorElement("Quadrature", None, degree, shape[0])
    else:
        cpp.debug("It's a tensor!")
        element = ufl.TensorElement("Quadrature", None, degree, shape)
