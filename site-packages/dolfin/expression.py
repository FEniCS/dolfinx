...
def Expression_factory(name,
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "eval" in dict_ or "eval_data" in dict_:
            ...
            cpp_base.__init__(self, self._ufl_element.cell().geometric_dimension(),
                              list(self._ufl_element.value_shape()))
        # If a simple expression
        elif dim_needs_to_be_passed:
            cpp_base.__init__(self, self._ufl_element.cell().geometric_dimension())
        else:
...
class Expression(object):
    ...
    >>> f1 = Expression(('cos(x[0])', 'sin(x[1])'), V = V1)
    ...
    def __new__(cls, cpparg = None, defaults = None, V = None, element = None):
        ...
        cpp_base = compile_expressions([cpparg], [defaults],
    ...
    def __call__(self, *args, **kwargs):
        ...
        value_size = product(self._ufl_element.value_shape())
...
def Expressions(*args, **kwargs):
    ...
    >>> f0, f1, f2 = Expressions(V0, 'A*sin(x[0]) + B*cos(x[1])', {'A':2.0,'B':3.0},
                               V1, code,
                               V2, (('cos(x[0])','sin(x[1])'),
                                    ('sin(x[0])','cos(x[1])')))
...
"""
    ...
    for i, cpp_base in enumerate(cpp_bases):
        ...
        if only_cpp:
            return_expressions.append(cpp_base(spaces[i]))
        else:
            return_expressions.append(Expression_factory("CompiledExpression", cpp_base)(spaces[i]))
