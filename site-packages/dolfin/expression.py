...
"""
    ...
    # FIXME: Hake please have a look at this

    # Simple temporary fix
    #return [Expression(arg, **kwargs) for arg in args]

    # Check the element/degree kwarg
    degree  = kwargs.pop("degree", None)
    element = kwargs.pop("element", None)
    if len(kwargs) > 1:
        raise TypeError, "Can only define one kwargs and that can only be 'degree' or 'element'."
    ...
    for cpp_base in cpp_bases:
        if element is not None:
            return_expressions.append(create_expression_class("CompiledExpression", cpp_base)(element=element))
        else:
            raise RuntimeError, "Degree in Expressions not working yet."
            return_expressions.append(create_expression_class("CompiledExpression", cpp_base)(degree=degree))
