...
def compute_vertex_map(mesh0, mesh1):
    ...
    *Arguments*
    ...
    if vertices0 is None or vertices1 is None:
        error("Unable to computed vertex map; parent vertex indices are missing.")
...
def init_parent_edge_indices(submesh, mesh):
    ...
    if parent_vertex_indices is None:
        error("Unable to compute edge map; data 'parent vertex indices' is missing.")
    ...
    for local_edge in edges(submesh):
        ...
        if not len(common_edges) == 1:
            error("Unable to compute edge map; parent vertices do not share exactly one common edge.")
