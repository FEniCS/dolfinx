...
class InfiniteCylinder(Primitive):
    ...
    def _write(self):
        ...
        y = self.y
        r = self.r
        return "cylinder(%g, %g, %g; %g, %g, %g; %g);" % \
            (x[0], x[1], x[2], y[0], y[1], y[2], r)

def Cylinder(x0, x1, x2, y0, y1, y2, r):
    "A cylinder from x to y with radius r"

    # Build cylinder from primitives
    c = InfiniteCylinder(x0, x1, x2, y0, y1, y2, r)
...
class OpenCone(Primitive):
    ...
    def _write(self):
        ...
        rx = self.rx
        ry = self.ry
        return "cone(%g, %g, %g; %g; %g, %g, %g; %g);" % \
            (x[0], x[1], x[2], rx, y[0], y[1], y[2], ry)
