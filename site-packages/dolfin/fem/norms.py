...
def norm(v, norm_type="L2", mesh=None):
    ...
    *Examples of usage*
    ...
    if isinstance(v, MultiMeshFunction):
        multimesh = True
        dX = ufl.dx(mesh)+ufl.dC(mesh)
    else:
        # Developer note, you have already imported dx, so why overwrite it
        dx = ufl.dx(mesh)
    ...
    if isinstance(v, GenericVector):
        ...
        if norm_type.lower() == "l2":
            ...
            cpp.dolfin_error("norms.py",
                             ...
                             % str(norm_type))
    elif (isinstance(v, Coefficient) and isinstance(v, (MultiMeshFunction))):
        if norm_type.lower() == "l2":
            M = v**2*dX
        elif norm_type.lower() == "h1":
            M = (v**2 + grad(v)**2)*dX
...
def errornorm(u, uh, norm_type="l2", degree_rise=3, mesh=None):
    ...
    *Arguments*
    ...
    if multimesh:
        if rank == 0:
            V = MultiMeshFunctionSpace(mesh, "Discontinuous Lagrange", degree)
        elif rank == 1:
            # Developer note: Is there any other way of doing this? I here
            # assume that the mesh
