...
class Form(cpp.Form):
    ...
    def __init__(self, form,
        ...
        if isinstance(form, ufl.Form):
            ...
            if mesh is None:
                domains = self.form_data.preprocessed_form.domains()
                if len(domains) == 1:
...
def _extract_function_spaces(form_data, compiled_form, given_function_spaces):
    ...
    if given_function_spaces:
        ...
        #function_spaces = [func.function_space() for func in ufl_form.arguments()] # FIXME: Do this instead
...
def _extract_coefficients(form_data, given_coefficients):
    ...
    if given_coefficients:
        ...
        #coefficients = ufl_form.coefficients() # FIXME: Do this instead

        # Type checking coefficients
        if not all(isinstance(c, cpp.GenericFunction) for c in coefficients):
            raise TypeError(coefficient_error +
...
def _extract_subdomains(form_data, override_subdomains):
    ...
    #domain, = ufl_form.domains() # FIXME: Use this instead
    #subdomain_data = ufl_form.subdomain_data()[domain] # FIXME: Use this instead
