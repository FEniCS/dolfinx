    ...
    def __init__(self, form,
        ...
        if isinstance(form, ufl.Form):
            ...
            if isinstance(form, ufc.form):
                ...
                # FIXME: Did form.form_data have anything we needed instead of the below extraction?
                self._compiled_form = form._compiled_form
                cpp.dolfin_error("form.py",
                                 "creating dolfin.Form",
                                 "FIXME: Wondering what dolfin.Form constructor is used for???")
            else:
                cpp.dolfin_error("form.py",
                                 "creating dolfin.Form",
        ...
        if additional_keys:
            raise TypeError("Invalid keys in subdomain_data: %s" % additional_keys)
        for data in self.subdomain_data.values():
            if not isinstance(data, MeshFunction): # TODO: This wasn't here before, disable if it's too restrictive
                raise TypeError("Invalid subdomain_data type %s" % type(data))
