...
class Form(cpp.Form):
    ...
    def __init__(self, form,
        ...
        # Unless subdomains are given explicitly, extract subdomains
        # from form_data
        if subdomains is None:
            self.subdomains = _extract_subdomains(self.form_data)
        else:
            self.subdomains = subdomains

        # Attach subdomains
        self.set_cell_domains(self.subdomains["cell"])
        self.set_exterior_facet_domains(self.subdomains["exterior_facet"])
        self.set_interior_facet_domains(self.subdomains["interior_facet"])
...
def _extract_subdomains(form_data):
    ...
    for domain in domain_types:
        if form_data.domain_data.has_key(domain):
            subdomains[domain] = form_data.domain_data[domain]
        else:
            subdomains[domain] = None
