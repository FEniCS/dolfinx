...
def derivative(form, u, du=None, coefficient_derivatives=None):
    if du is None:
        ...
        if isinstance(u, Function):
            ...
            # TODO: Test the following if block!
            # Get mesh
            mesh = u[0].function_space().mesh()
            # TODO: Test by assertion?
            assert all(mesh.id() == v.function_space().mesh().id() for v in u)

            # Build mixed element
            element = ufl.MixedElement((v.function_space().ufl_element() for v in u))

            # Build mixed function space
            # TODO: Is it really needed to build a function space including dofmap?!
            #V = FunctionSpace(mesh, [w.function_space() for w in u])
            V = ufl.FunctionSpace(mesh, [w.function_space() for w in u])

            #du = ufl.split(Argument(V, number, part))
            du = ufl.split(ufl.Argument(V, number, part))
        else:
            cpp.dolfin_error("formmanipulation.py",
                             "compute derivative of form",
...
def increase_order(V):
    ...
    if n > 0:
        ...
        # TODO: Remove deprecated MixedFunctionSpace
