...
class LocalSolver(cpp.LocalSolver):
    ...
    def __init__(self, a, L=None):
        """
        Create a local solver for a linear variational problem a(u, v) = L(v).
        It is possible to ommit L, in that case solve(x, b) has to be used.
        This function will reuse factorization of the local systems.
        """

        # Store input UFL forms and solution Function
        self.a_ufl = a
        self.L_ufl = L
        # wrap LHS form als UFL form
        a = Form(a)

        if L == None:
           cpp.LocalSolver.__init__(self, a)
        else:
            # Wrap forms (and check if linear form L is empty)
            if L.empty():
                L = cpp.Form(1, 0)
            ...
            # Initialize C++ base class
            cpp.LocalSolver.__init__(self, a, L)
