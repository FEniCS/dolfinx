...
def solve(*args, **kwargs):
    ...
    *3. Solving nonlinear variational problems*
    ...
    if isinstance(args[0], ufl.classes.Equation):
        ...
        # Go adaptive if 4 or 5 arguments:
        if len(args) in (4, 5):
            adaptive_args = args[3:] if bcs else args[2:]
            return _solve_adaptive(eq, u, bcs, adaptive_args, kwargs)
...
def _solve_adaptive(eq, u, bcs, args, kwargs):
    ...
    if (not len(args) == 2):
        cpp.dolfin_error("solving.py",
                         ...
                         "Wrong number of arguments, expecting adaptive_solve(lhs == rhs, u, bcs, tol, M)")

    tol = args[0]
    M = args[1]

    if (not isinstance(tol, (float, int)) or not isinstance(M, ufl.Form)):
        cpp.dolfin_error("solving.py",
                         "solve variational problem adaptively",
                         "Wrong type of arguments, expecting adaptive_solve(lhs == rhs, u, bcs, tol, M)")
