...
def assemble(form,
    ...
    *Examples of usage*
        ...
        In the simplest case, one may use assemble to solve the standard 
        Poisson problem as follows:
        ...
        incorporating boundary conditions. Thereafter, the ``bc.apply`` 
        call performs the necessary modifications to the linear system.
...
def assemble_system(A_form,
    ...
    To solve the linear system AU=b one may use the alternative
    assemble_system instead of multiple calls to :py:func:`assemble
    <dolfin.fem.assembling.assemble>`. That is, the statements
    ...
    The boundary conditions are now applied to the matrices and vectors
    prior to assembly. The statement above is valid even if ``bc`` is a list
    of ``DirichletBC`` instances. For more info and options, see 
