...
def assemble_multimesh(form,
    ...
    # Extract multimesh function spaces for arguments, the
    #number of parts and create a cpp.MultiMeshForm
    if rank > 0:
        if (rank == 1 and isinstance(coefficients[0], MultiMeshFunction)):
                multimesh = coefficients[0].function_space().multimesh()
        ...
        V_multi = [v.function_space for v in arguments]
        num_parts = coefficients[0].function_space().num_parts()
        multimesh = coefficients[0].function_space().multimesh()
    ...
    for part in range(num_parts):
        ...
        # Handeling coefficients of different types
        if (rank ==1 and isinstance(coefficients[0], MultiMeshFunction)):
                coeff = coefficients[0].part(part)
                dolfin_form.set_coefficient(0, coeff)
                dolfin_form.coefficients[0]=coeff
