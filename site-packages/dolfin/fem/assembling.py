...
def _create_dolfin_form(form,
    ...
    if isinstance(form, cpp.Form) and not hasattr(form, "_compiled_form"):
        ...
        if mesh is not None:
            dolfin_form.set_mesh(mesh) # NB! This modifies the input dolfin_form!
        # NB! This will effectively ignore function_spaces, coefficients, form_compiler_parameters!
    else:
        # Wrap form
    ...
    # Get actual mesh from form (needed to convert SubDomain to MeshFunction below)
    mesh = dolfin_form.mesh()

    # Typecheck and convert domains
    cell_domains = _as_meshfunction(cell_domains, mesh, 0, "cell_domains")
    exterior_facet_domains = _as_meshfunction(exterior_facet_domains, mesh, 1, "exterior_facet_domains")
    interior_facet_domains = _as_meshfunction(interior_facet_domains, mesh, 1, "interior_facet_domains")
...
def assemble(form,
    ...
    *Arguments*
    ...
    Parameter ``reset_sparsity`` is deprecated. Sparsity pattern of
    ``tensor`` is reset iff ``tensor.empty()`` holds.
...
def _as_meshfunction(domains, mesh, codim, domains_argument_name):
    "Typecheck and convert domains to MeshFunction."
    # Type check of input
    if not isinstance(domains, (cpp.SubDomain, cpp.MeshFunctionSizet, types.NoneType)):
        raise TypeError("expected a 'SubDomain', 'MeshFunction' of 'Sizet' or 'None', for argument '%s'" % domains_argument_name)

    # Convert SubDomain to MeshFunction # FIXME: This is magical and unsafe, can we deprecate or just remove?
    if isinstance(domains, cpp.SubDomain):
        dim = mesh.topology().dim() - codim
        mf = cpp.MeshFunction("size_t", mesh, dim)
        mf.set_all(1)       # Mark cells with 1 by default
        domains.mark(mf, 0) # Mark cells within domain with 0
        return mf
    else:
        return domains
