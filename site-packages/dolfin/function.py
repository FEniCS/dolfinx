...
__date__ = "2009-10-06 -- 2009-10-17"
...
class Function(ufl.Function, cpp.Function):
    ...
    def __init__(self, *args):
        ...
        if isinstance(args[0], Function):
            ...
            # If using sub-function constructor
                ...
                if not i < V.num_sub_spaces():
                    raise RuntimeError, "Can only extract subfunctions with i = 0..%d"%V.num_sub_spaces()
                self._V = V.sub(i)
    ...
    def _sub(self, i, deepcopy = False):
        ...
        if self._V.num_sub_spaces() == 1:
            raise RuntimeError, "No subfunctions to extract"
        if not i < self._V.num_sub_spaces():
            raise RuntimeError, "Can only extract subfunctions with i = 0..%d"%self._V.num_sub_spaces()

        # Create and instantiate the Function
        if deepcopy:
            return Function(self._V.sub(i), cpp.Function._sub(self, i))
        else:
    ...
    def split(self, deepcopy = False):
        ...
        if not isinstance(self._V, MixedFunctionSpace):
            raise RuntimeError, "No subfunctions to extract"
        return tuple(self._sub(i,deepcopy) for i in xrange(self._V.num_sub_spaces()))
