...
class Function(ffc.Function, dolfin.cpp_Function):
    ...
    def __init__(self, element, *others):
        "Create Function"
        # check to see if *others contains list of coefficients and check to see that it is a dolfin vector - print a warning
        if len(others) == 2:
            if not isinstance(others[1], (dolfin.Vector, dolfin.GenericVector)):
                print "WARNING:  Coefficients should be dolfin Vector or Matrix"
