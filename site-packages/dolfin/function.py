...
class FunctionCreator(type):
    ...
    class will inherit either from a cpp.Function (which is a
    dolfin::Function), a user-defined C++ function that inherits
    dolfin::Function or dolfin::DiscreteFunction, which is a
    ...
    def __new__(cls, name, bases, dict_):
        ...
        if name == "Function":
            ...
            def __new__(cls, V, **kwargs):
                ...
                if len(kwargs) > 0:
                    if 'cppcode' in dict_ or 'cppexpr' in kwargs:
        ...
        # If cppcode or cppexpr is provided send it to compile_function
            ...
            dict_.pop('cppcode',None)
            dict_.pop('cppexpr',None)
            dict_.pop('defaults',None)
            
        else:
            raise TypeError, "Some error..."
        
        # Extend the bases with ffc.Function and cpp_BaseFunction
        bases = (ffc.Function, cpp_BaseFunction)
        
        # Attach the init function to the dictionary
        dict_['__init__'] = _init_function(cpp_BaseFunction, dict_.get('__init__', None))
