...
def Function_factory(name,
    ...
    def Function_sub(self,i):
        ...
        ret_func = object.__new__(Function_factory("DiscreteSubFunction", cpp.DiscreteFunction, \
                                from_subfunc = True),self._V.spaces[i],subfunc=self._sub(i))
        ret_func.__init__(self._V.spaces[i], subfunc=self._sub(i))
        return ret_func
...
def Functions(*args,**kwargs):
    ...
    >>> f0, f1, f2 = Functions(V0, 'sin(x[0]) + cos(x[1]', V1, cppcode,
                               V2, (('cos(x[0])','sin(x[1]'),
                               ('sin(x[0])','cos(x[1]')))
