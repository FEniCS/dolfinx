...
def Function_factory(name, cpp_base, user_eval = None, user_init = None,
    ...
    @param subfunc   : If true, create a Function taking a SubFunction in __init__
    """
    ...
    def Function_sub(self,i):
        ...
        A sub function can be extract from a discrete function that is in a
        a MixedFunctionSpace or in a VectorFunctionSpace. The sub function is a
        function that resides in a sub spaces of this function.
        ...
        if not i < len(self._V.spaces):
            raise RuntimeError, "Can only extract sub functions with i = 0..%d"%len(self._V.spaces)
        return Function_factory("DiscreteSubFunction", cpp.DiscreteFunction, \
                                subfunc = True)(self._V.spaces[i],self._sub(i))
    ...
    def construct__new__(cpp_base, subfunc = False):
        " Help function that return a __new__ function"
        def __new__(cls, V, *args, **kwargs):
            """ Instantiate a new Function
            
            @param V : A FunctionSpace
            """
            # If initialize with a SubFunction
            if subfunc:
                assert(len(args) == 1)
                assert(isinstance(args[0],cpp.SubFunction))
                
                # Instantiate cpp base class
                ret_func = cpp_base.__new__(cls, args[0])
            else:
                # Instantiate cpp base class
                ret_func = cpp_base.__new__(cls, V)
                    
            # Instantiate ffc base class.
    ...
    def construct__init__(cpp_base, user_init = None, subfunc = False):
    ...
    dict_["__init__"]       = construct__init__(cpp_base, user_init, subfunc)
    dict_["__new__"]        = construct__new__(cpp_base, subfunc)
...
class Function(object):
    ...
    def __new__(cls, V, **kwargs):
        ...
        """
        # Check passed kwargs
        allowed_kwargs = ['cppcode', 'cppexpr', 'defaults', 'subfunc']
        ...
        # Check type of V
        if not isinstance(V, (list, tuple, FunctionSpaceBase)):
...
def extract_basis_functions(V, extraction_function):
    if not isinstance(V, MixedFunctionSpace):
        raise TypeError, "Unable to create basis functions, not a mixed function space."
