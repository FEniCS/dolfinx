...
def Function_factory(name,
    ...
    @param from_file : If true, create a Function taking a filename in __init__
    """
    ...
    def Function_sub(self,i):
        ...
        # Call object.__new__(DiscreteSubFunction,args), so we avoid calling Function.__new__
        func = object.__new__(Function_factory("DiscreteSubFunction", cpp.DiscreteFunction, \
                                from_subfunc = True),self._V.spaces[i],subfunc=self._sub(i))
        func.__init__(self._V.spaces[i], subfunc=self._sub(i))
        return func
    ...
    def construct__init__(cpp_base, user_init=None,
        ...
        if from_file:
            def __init__(self,*args,**kwargs):
                ...
                # Initialize cpp base class
                cpp_base.__init__(self, kwargs["filename"])
                
                # Initialize a FunctionSpace from read in function
                self._V = FunctionSpaceFromCpp(cpp_base.function_space(self))
                
                # Initialize ffc base class.
                ffc.Function.__init__(self, self._V.element())
...
class FunctionMetaClass(type):
    def __new__(cls, name, bases, dict_):
        ...
        if len(bases) == 3 and bases[0] == ffc.Function and \
               issubclass(bases[1], cpp.Function) and bases[2] == Function:
            # Return the instantiated class
            return type.__new__(cls, name, bases,dict_)
        
        # Check the cppcode and eval attributes
        if 'cpparg' in dict_  and 'eval' in dict_:
            raise TypeError, "Cannot create class with both 'eval' and 'cpparg' attributes defined."
        ...
        # If cpparg is provided, case 6. from docstring
        if 'cpparg' in dict_:

            print dict_
            # Check the handed attributes and return an args tuple
            cpparg   = dict_['cpparg']
            defaults = dict_.get("defaults",None)
...
class Function(object):
    ...
    0. Initialized from a file
    --------------------------

    If a discrete function previously has been stored in a '.xml' file it
    can be retreived by:

    >>> f = Function(filename='myfunc.xml')
    ...
    def __new__(cls,V=None,cpparg=None,defaults=None,filename=None,**kwargs):
        """ Instantiate a new Function

        @param V        : Function Space argument, optional if filename is given
        @param cpparg   : Optional C++ argument, see docstring
        @param defaults : Optional C++ argument, see docstring
        @param filename : If passed, read a instantiate a Function from file
        """
        ...
        # If the user is not passing a FunctionSpace, it must pass a filename
        if V is None:
            if filename is None:
                raise TypeError, "Either pass a FunctionSpace as V or a valid filename"
            if not isinstance(filename,str):
                raise TypeError, "The filename must be a 'str'"
            if not ("xml" in filename or "xml.gz" in filename):
                raise TypeError, "The filename must be a '.xml' or '.xml.gz' file."
            return object.__new__(Function_factory("FunctionFromFile",
                                                   cpp.DiscreteFunction,
                                                   from_file=True),
                                  filename=filename)
        
        # If a FunctionSpace is provided
        if not isinstance(V,cpp.FunctionSpace):
            # Check if user has passed an xml filename
            if isinstance(V,str) and ("xml" in V or "xml.gz" in V):
                raise TypeError, "Illegal argument for creation of Function. First argument must be a FunctionSpace. \nIf Function is to be read from file: use f = Function(filename='%s')"%V
        ...
        # Check that the filename is None.
        if not filename is None:
            raise TypeError, "Cannot pass FunctionSpace together with a filename"
        ...
        if cpparg is None:
            ...
            return object.__new__(Function_factory("DiscreteFunction",cpp.DiscreteFunction),V)
...
"""
    ...
    for i, cpp_base in enumerate(cpp_bases):
        ...
        if only_cpp:
            ...
            func = object.__new__(Function_factory("CompiledFunction", cpp_base),spaces[i])
            func.__init__(spaces[i])
            return_functions.append(func)
...
def BasisFunction__init__(Base):
    def __init__(self, V):
        if not isinstance(V, FunctionSpaceBase):
            raise TypeError, "Illegal argument for creation of basis function, not a FunctionSpace: " + str(V)            
        Base.__init__(self, V.element())
        # We need to attach the FunctionSpace after calling Base.__init__()
        self.attach(V)
    
    return __init__

def extract_basis_functions(V, extraction_function):
    if not isinstance(V, MixedFunctionSpace):
        raise TypeError, "Unable to create basis functions, not a mixed function space: " + str(V)
    basis_functions = extraction_function(V.element())
    for (i, v) in enumerate(basis_functions):
        if isinstance(v, (tuple, list)):
            for j in range(len(v)):
                basis_functions[i][j].attach(V)
        else:
            basis_functions[i].attach(V)
...
def TestFunctions(V):
    ...
    return extract_basis_functions(V, ffc.TestFunctions)

def TrialFunctions(V):
    "Create trial functions from mixed function space."
    return extract_basis_functions(V, ffc.TrialFunctions)
