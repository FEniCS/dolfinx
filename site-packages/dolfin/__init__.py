...
from dolfin.mesh.meshfunction import *
...
# Dynamically add docstrings to dolfin module. For some functions and classes
# this will be redundant, but it is necessary because of definitions in
# *_post.i etc.
import dolfin.docstrings
from dolfin.docstrings.dolfin import cpp as doc_cpp
import types
def add_docstring(doc, mod):
    "Add docstring to modules, functions and member functions (not classes)."
    if isinstance(doc, (types.FunctionType, types.ModuleType)):
        mod.__doc__ = doc.__doc__
    elif isinstance(doc, (types.ClassType, types.TypeType)):
        for k, v in doc.__dict__.items():
            try:
                add_docstring(v, mod.__dict__[k])
            except:
                pass
for key, doc in doc_cpp.__dict__.items():
#    print "\nname: ", key
    try:
        add_docstring(doc, dolfin.cpp.__dict__[key])
    except:
        pass

# Set the docstring of this module and the dolfin.cpp module.
__doc__ = docstrings.dolfin.__doc__
dolfin.cpp.__doc__ = doc_cpp.__doc__

# Delete temporary variables and modules to avoid including them when importing
# this module
del types, add_docstring, key, doc, doc_cpp, dolfin.docstrings
