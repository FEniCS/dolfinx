...
def mpi_jit_decorator(local_jit,*args,**kwargs):
    ...
    Use this function as a decorator to any jit compiler function.
    In a paralell run, this function will first call the jit compilation
    function on the first process. When this is done, and the module is in
    the cache, it will call the jit compiler on the remaining provesses, which
    then will use the cached module.
    ...
    def mpi_jit(*args,**kwargs):
        ...
        if MPI.process_number() == 0:
            # Initilize the Runner thread
            jit_thread = Runner(local_jit,*args,**kwargs)

            # Start the thread
            jit_thread.start()
            
            # Join the thread for 0.2 sec (FIXME: Is this too long or too fast?)
            jit_thread.join(0.2)

            # Check if the thread is finished, if not then it is probably compiling
            if jit_thread.isAlive():
                info("Calling JIT compiler on first process.")
                # Wait until thread is finished
                jit_thread.join()

            # Get the result
            output = jit_thread.get_result()
