...
UFL Function class. In future versions, it will instead inherit from
    ...
    def construct__init__(cpp_base, user_init=None,
        ...
        if from_file:
            def __init__(self, *args, **kwargs):
                ...
                # Removed initialization with kwargs filename
                #assert("filename" in kwargs)
                # Initialize cpp base class
                #cpp_base.__init__(self, kwargs["filename"])
                
                # Using the first args to instantiate a file from disk
                assert isinstance(args[0],str), "Provide a 'str' as filename"
                assert ("xml" in args[0] or "xml.gz" in args[0]), "The file must be a 'xml' or 'xml.gz' file"
                cpp_base.__init__(self, args[0])
                ...
                ufl.Function.__init__(self, self._V.element())
...
class Function(object):
    ...
    Arguments
    ---------
    V        : Function Space argument, can optionally be used as a
                      filename argument
    ...
    If a discrete function previously has been stored in a '.xml' or a 'xml.gz'
    file it can be retreived by:

    >>> f = Function('myfunc.xml')

    optionally

    >>> f = Function('myfunc.xml.gz')
    ...
    def __new__(cls, V, cpparg=None, defaults=None, **kwargs):
        ...
        @param V        : Function Space argument, can optionally be used as a
                          filename argument
        ...
        # If the user is not passing a FunctionSpace, as first argument it must pass
        # a filename
        if not isinstance(V,(str,cpp.FunctionSpace)):
            raise TypeError, "The first argument must be a 'cpp.FunctionSpace' or a 'str', defining a filename"

        # If the user has passed a str, check the filename
        if isinstance(V,str):
            if not ("xml" in V or "xml.gz" in V):
        ...
        # If loading vector from file (Not in docstring yet)
...
# FIXME:
# This function is not needed anymore? Right?
    ...
    for (i, v) in enumerate(basis_functions):
        if isinstance(v, (tuple, list)):
            for j in range(len(v)):
                ...
                basis_functions[i][j]._V = V.spaces[i].spaces[j]
                basis_functions[i][j].function_space = types.MethodType(function_space,basis_functions[i][j],ufl.BasisFunction)
        else:
            basis_functions[i]._V = V.spaces[i]
            basis_functions[i].function_space = types.MethodType(function_space,basis_functions[i],ufl.BasisFunction)
    return basis_functions

#--- Subclassing of ufl.{Basis, Trial, Test}Function ---
#FIXME: We need to confirm that this is the way it should be done

class TrialFunction(ufl.BasisFunction):
    def __init__(self, V):
        if not isinstance(V, FunctionSpaceBase):
            raise TypeError, "Illegal argument for creation of basis function, not a FunctionSpace: " + str(V)            
        ufl.BasisFunction.__init__(self, V.element(), -1)
...
class TestFunction(ufl.BasisFunction):
    def __init__(self, V):
        if not isinstance(V, FunctionSpaceBase):
            raise TypeError, "Illegal argument for creation of basis function, not a FunctionSpace: " + str(V)            
        ufl.BasisFunction.__init__(self, V.element(), -2)
...
# FIXME:
# Should the splitted test and trial functions also owe their own FunctionSpace?
# If we could add these by attaching a function function_space and the protected attribute self._V
# after the split.
def TestFunctions(V):
