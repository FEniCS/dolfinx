...
def errornorm(u, uh, normtype="l2", degree=3, mesh=None):
    ...
    # Check arguments (Only assume a user pass a cpp.Function)
    if not isinstance(u, cpp.Function):
        cpp.error("Illegal function for interpolation, not a Function: " + str(u))
    if not isinstance(uh, cpp.Function):
        cpp.error("Illegal function for interpolation, not a Function: " + str(uh))

    # Get rank
    if not u.has_function_space():
        cpp.error("The passed Functions must have a FunctionSpace")

    if not uh.has_function_space():
        cpp.error("The passed Functions must have a FunctionSpace")

    u_element = cpp.FunctionSpace.element(u.function_space())
    uh_element = cpp.FunctionSpace.element(uh.function_space())

    if not u_element.value_rank() == uh_element.value_rank():
        error("Unable to compute error, functions have different rank.")
    rank = u_element.value_rank()
    ...
    # Compute norm
    return norm(e, type=normtype, mesh=mesh)
