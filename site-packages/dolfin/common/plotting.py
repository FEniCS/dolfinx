...
def mplot_function(ax, f, **kwargs):
    ...
    if f.vector().size() == mesh.num_cells():
        ...
        if gdim == 2 and tdim == 2:
            ...
            # FIXME: Not tested
            return ax.plot_trisurf(mesh2triang(mesh), color=C, shade=True)
        elif gdim == 3 and tdim == 3:
            # Volume
            # TODO: Isosurfaces?
            # Vertex point cloud
        ...
        if mode == "glyphs":
            ...
            if gdim == 2:
                # FIXME: Not tested
                args = Xdef + [C]
                return ax.tripcolor(mesh2triang(mesh), color=C, shading='gouraud')
            elif gdim == 3:
                # FIXME: Not tested
                return ax.plot_trisurf(mesh2triang(mesh), color=C, shade=True)
...
def _plot_matplotlib(obj, mesh, kwargs):
    ...
    if isinstance(obj, cpp.Function):
        ...
        return mplot_expression(ax, obj, **kwargs)
