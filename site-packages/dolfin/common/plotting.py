...
def _plot_matplotlib(obj, mesh, kwargs):
    if isinstance(obj, (cpp.MultiMeshDirichletBC)):
        cpp.warning("Don't know how to plot type %s" % type(obj))
        return

    if isinstance(obj, cpp.MultiMeshFunction):
        return _plot_matplotlib_multimesh_function(obj, mesh, **kwargs)
    if isinstance(obj, cpp.MultiMesh):
        return _plot_matplotlib_multimesh(obj, **kwargs)
...
def _plot_matplotlib_multimesh(obj, **kwargs):
...
def _plot_matplotlib_multimesh_function(obj, mesh, **kwargs):
    from matplotlib import pylab as plt
    vmin = min(obj.vector()[:])
    vmax = max(obj.vector()[:])

    fig = plt.figure()
    for i in range(obj.function_space().num_parts()):
        p = plot(obj.part(i, deepcopy=True))
