...
_plottable_types = (cpp.Function, cpp.Expression, cpp.Mesh, cpp.DirichletBC) + _meshfunction_types
...
def mplot_mesh(mesh, **kwargs):
    if mesh.geometry().dim() != 2:
        raise AttributeError('Matplotlib plotting backend only supports 2D mesh.')
    return plt.triplot(mesh2triang(mesh), color='#808080')

def mplot_function(f, **kwargs):
    ...
    # Vector function, interpolated to vertices
        ...
        C = np.sqrt(U**2 + V**2)
        return plt.quiver(X, Y, U, V, C)

def mplot_cellfunction(cellfn, **kwargs):
    C = cellfn.array()
    tri = mesh2triang(cellfn.mesh())
    return plt.tripcolor(tri, facecolors=C)

def mplot_meshfunction(obj, **kwargs):
    mesh = obj.mesh()
    tdim = mesh.topology().dim()
    d = obj.dim()
    if tdim == 2 and d == 2:
        return mplot_cellfunction(obj, **kwargs)
    else:
        raise AttributeError("Matplotlib plotting backend doesn't handle meshfunction of dim %d" % d)

def mplot_dirichletbc(obj, **kwargs):
...
def _plot_matplotlib(obj, mesh, kwargs):
    ...
    if mode is not None:
        cpp.warning("Ignoring 'mode' in matplotlib plotter.")

    if isinstance(obj, cpp.Function):
        return mplot_function(obj, **kwargs)
    elif isinstance(obj, cpp.Expression):
        return mplot_expression(obj, **kwargs)
    elif isinstance(obj, cpp.Mesh):
        return mplot_mesh(obj, **kwargs)
    elif isinstance(obj, cpp.DirichletBC):
        return mplot_dirichletbc(obj, **kwargs)
    elif isinstance(obj, _meshfunction_types):
        return mplot_meshfunction(obj, **kwargs)
    else:
...
def _plot_cpp(object, mesh, kwargs):
    ...
    if isinstance(object, cpp.Expression):
        plot_object = cpp.plot(expression, mesh, p)
    else:
...
def plot(object, *args, **kwargs):
    ...
    *Examples of usage*
    ...
    # Get mesh from explicit mesh kwarg or only positional arg
