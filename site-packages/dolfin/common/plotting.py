...
def _has_matplotlib():
    ...
    # Switch to WebAgg or Agg backend if DISPLAY not set.
    # WebAgg also allows plt.savefig() to work correctly.
    if not os.environ.get("DISPLAY"):
        # If we are inside a Docker container use WebAgg,
        # unless the backend is already set to Agg by user.
        if os.path.isfile("/.dockerenv") and matplotlib.get_backend() != 'agg':
            # TODO: Using Docker container introspection it should be possible
            # to print the Docker *exposed* port information for the user,
            # rather than the internal port.
            # See: https://github.com/docker/docker/issues/28574
            new_backend = 'WebAgg'
            cpp.log(cpp.PROGRESS, "Detected Docker-based environment. Switching "
                                  "to 'WebAgg' matplotlib backend.")
        else:
            # Otherwise use straightforward Agg for non-interactive plotting.
            new_backend = 'agg'
            cpp.log(cpp.PROGRESS, "Environment variable DISPLAY not set. Switching "
                                  "to 'Agg' matplotlib backend.")

        try:
            matplotlib.use(new_backend, warn=True)
        except ValueError as e:
            cpp.warning("Switching to '%s' backend failed with the message:" % new_backend)
            print('"%s"' % str(e))
...
def _plot_x3dom(obj, kwargs):
    ...
    try:
        ...
        cpp.warning("Don't have Tornado to serve X3DOM HTML, cannot plot.")
        return
    ...
    # Grab some useful arguments from matplotlib related to WebAgg
    # backend. Seems sensible to let the user configure matplotlib
    # and get the same behaviour with X3DOM served over HTTP.
    import matplotlib
    mpl_rc = matplotlib.rc_params()
    port = mpl_rc.get("webagg.port", 8000)
    open_in_browser = mpl_rc.get("webagg.open_in_browser", True)

    class X3DOMHandler(tornado.web.RequestHandler):
        def get(self):
            self.write(out)

    app = tornado.web.Application([(r"/", X3DOMHandler)])
    app.listen(port)

    try:
        cpp.info("To view figure, visit http://127.0.0.1:%i" % port)
        cpp.info("Press Ctrl+C to stop Tornado server...")
        # TODO: If open_in_browser True then fire up a webbrowser using webbrowser. 
...
def plot(object, *args, **kwargs):
    ...
    *Examples of usage*
    ...
    # Plot
    if interactive and backend == "matplotlib":
        # Warn the user that interactive and agg backend does nothing.
        from matplotlib import get_backend
        if interactive and get_backend() == 'agg':
            cpp.warning("Matplotlib Agg backend does not support interactive plotting.")
        
        import matplotlib.pyplot as plt
        _plot = _plot_matplotlib(object, mesh, kwargs)
        plt.show()
        return _plot
    elif not interactive and backend == "matplotlib":
        # User can run pyplot.plt() manually
        return _plot_matplotlib(object, mesh, kwargs)
    elif interactive and backend == "x3dom":
        # Fire up webserver and serve X3DOM HTML and possibly also open browser.
        _plot_x3dom(object, kwargs)
    elif not interactive and backend == "x3dom":
        cpp.dolfin_error("x3dom backend does not support non-interactive plotting.")
    else:
        assert False, "This code should not be reached."
