...
def project(v, V=None, solver_type="cg"):
    ...
    w = TestFunction(V)
    Pv = TrialFunction(V)
    a = ufl.dot(w, Pv)*ufl.dx
    L = ufl.dot(w, v)*ufl.dx
...
def _extract_function_space(expression):
    ...
    from ufl.compiler.language.algebra import Form
    from ufl.compiler.language.operators import value_rank
