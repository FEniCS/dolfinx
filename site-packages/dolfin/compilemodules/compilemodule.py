...
def expression_to_code_fragments(expr, arguments, constant_members=None):
    "A help function which extract a dict with code snippets from an expression"

    constant_members = constant_members or []
    expr = list(expr)
    ...
    members_code = ["  double %s;" % name for name in variables \
                    if name not in constant_members]
    members_code.extend("  boost::shared_ptr<dolfin::Constant> shared_%s;" % name \
