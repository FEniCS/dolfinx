...
from dolfin.cpp import MPI, info, parameters, warning, info, MPICommWrapper
...
def mpi_jit_decorator(local_jit, *args, **kwargs):
    ...
    *Example*
    ...
    def mpi_jit(*args, **kwargs):
        ...
        if MPI.num_processes(mpi_comm.comm()) == 1:
            return local_jit(*args, **kwargs)

        # Compile first on process 0
        if MPI.process_number(mpi_comm.comm()) == 0:
            output = local_jit(*args, **kwargs)

        # Wait for the first process to finish
        MPI.barrier(mpi_comm.comm())

        # Then compile on all other processes (which may then just
        # read the cache)
        if not MPI.process_number(mpi_comm.comm()) == 0:
            output = local_jit(*args,**kwargs)
