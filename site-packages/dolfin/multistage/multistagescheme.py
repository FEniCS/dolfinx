...
def _butcher_scheme_generator(a, b, c, time, solution, rhs_form):
    ...
    *Arguments*
    ...
    # Only one last stage
    if len(b.shape) == 1:
        last_stage = Form(ufl.inner(sum([float(bi)*ki for bi, ki in zip(b, k)], zero_), v)*DX)
    else:
        # FIXME: Add support for addaptivity in RKSolver and MultiStageScheme
        
        last_stage = [Form(ufl.inner(sum([float(bi)*ki for bi, ki in \
                                          zip(b[0,:], k)], zero_), v)*DX),
                      Form(ufl.inner(sum([float(bi)*ki for bi, ki in \
...
class MultiStageScheme(cpp.MultiStageScheme):
    ...
    def __init__(self, rhs_form, solution, t, bcs, a, b, c, order):
        ...
        ufl_stage_forms, dolfin_stage_forms, last_stage, k, dt, human_form = \
                         _butcher_scheme_generator(a, b, c, t, solution, rhs_form)
