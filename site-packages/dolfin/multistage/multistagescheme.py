...
def _extract_rhs_expressions(rhs_form, v, vector_rhs):
    """
    Given a form, extract the rhs expressions, by replacing the test
    functions with scalar Constants and then differentiate the form
    with respect to each scalar constant. We are then left with each
    compoent.
    """
    assert isinstance(rhs_form, ufl.form), "expected a ufl form"

    # Create scalar coefficients
    if vector_rhs:
        uu = [Constant(1.0) for _ in range(v.ufl_shape[0])]
        rhs_scalars = ufl.as_vector(uu)
    else:
        uu = [Constant(1.0)]
        rhs_scalars = uu[0]
    
    rhs_integrand = rhs_form.integrals()[0].integrand()
    rhs_scalar = replace(rhs_integrand, {v:rhs_scalars})

    # Return each component free from its scalar test function
    return [ufl.algorithms.expand_indices(\
        ufl.algorithms.expand_derivatives(diff(rhs_scalar, ui))) for ui in uu]
