...
def safe_adjoint(x):
  return adjoint(x, reordered_arguments=ufl.algorithms.extract_arguments(x))
...
def _check_form(rhs_form):
    ...
    arguments = ufl.algorithms.extract_arguments(rhs_form)
    if len(arguments) != 1:
...
def _time_dependent_expressions(rhs_form, time):
    ...
    for coefficient in ufl.algorithms.extract_coefficients(rhs_form):
        if hasattr(coefficient, "user_parameters"):
            for c_name, c in coefficient.user_parameters.items():
...
def _butcher_scheme_generator_tlm(a, b, c, time, solution, rhs_form, perturbation):
    ...
    *Arguments*
    ...
    # Stage solutions
    k = [Function(solution.function_space(), name="k_%d"%i) for i in range(size)]
    kdot = [Function(solution.function_space(), name="kdot_%d"%i) for i in range(size)]
    ...
    for i, ki in enumerate(k):
        ...
        stage_form_tlm = safe_action(derivative(stage_form, y_), perturbation) + sum([dt*float(a[i,j]) * safe_action(derivative(forward_forms[j], y_), kdot[j]) for j in range(i+1)])
        if explicit:
    ...
    # Only one last stage
    if len(b.shape) == 1:
        last_stage = Form(ufl.inner(perturbation + sum([dt*float(bi)*kdoti for bi, kdoti in \
                                                       zip(b, kdot)], zero_), v)*DX)
    else:
    ...
    for i in range(size):
        ...
        kdotterm = " + ".join("%(a)sh*action(derivative(f(t_n%(cih)s, y_n + %(kterm)s), kdot_%(i)s" % \
        ...
        if len(kterm) == 0:
            human_form.append("k_%(i)s = f(t_n%(cih)s, y_n)" % {"i": i, "cih": cih})
            human_form.append("kdot_%(i)s = action(derivative(f(t_n%(cih)s, y_n), y_n), ydot_n)" % \
                                                            {"i": i, "cih": cih})
        else:
            human_form.append("k_%(i)s = f(t_n%(cih)s, y_n + %(kterm)s)" % \
                          {"i": i, "cih": cih, "kterm": kterm})
            human_form.append("kdot_%(i)s = action(derivative(f(t_n%(cih)s, y_n + %(kterm)s), y_n) + %(kdotterm)s" % \
    ...
    return ufl_stage_forms, dolfin_stage_forms, last_stage, [x for t in zip(k, kdot) for x in t], dt, human_form
...
def _butcher_scheme_generator_adm(a, b, c, time, solution, rhs_form, adj):
    ...
    *Arguments*
    ...
    # Stage solutions
    k = [Function(solution.function_space(), name="k_%d"%i) for i in range(size)]
    kbar = [Function(solution.function_space(), name="kbar_%d"%i) for i in range(size)]
    ...
    for i, ki in reversed(list(enumerate(kbar))):

        # And now the adjoint linearisation:
        stage_form_adm = ufl.inner(dt * b[i] * adj, v)*DX  + sum([dt * float(a[j,i]) * safe_action(safe_adjoint(derivative(forward_forms[j], y_)), kbar[j]) for j in range(i+1, size)])
        if explicit:
            stage_forms_adm = [stage_form_adm]
        else:
            # Create a F=0 form and differentiate it
            stage_form_adm -= ufl.inner(kbar[i], v)*DX
            stage_forms_adm = [stage_form_adm, derivative(stage_form_adm, kbar[i])]
    ...
    # Only one last stage
    if len(b.shape) == 1:
        last_stage = Form(ufl.inner(adj, v)*DX + sum([safe_action(safe_adjoint(derivative(forward_forms[i], y_)), kbar[i]) for i in range(size)]))
    else:
        raise Exception("Not sure what to do here")
    
    human_form = "unimplemented"
    
    return ufl_stage_forms, dolfin_stage_forms, last_stage, [x for t in zip(k, kbar) for x in t], dt, human_form

class MultiStageScheme(cpp.MultiStageScheme):
    """
    Base class for all MultiStageSchemes
    """
    def __init__(self, rhs_form, solution, t, bcs, a, b, c, order, generator=_butcher_scheme_generator):
        bcs = bcs or []
        t = t or Constant(0.0)
        ufl_stage_forms, dolfin_stage_forms, last_stage, k, dt, human_form = \
                         generator(a, b, c, t, solution, rhs_form)
    ...
    def to_tlm(self, perturbation):
        ...
        This takes \dot{y_n} (the derivative of y_n with respect to a parameter) and computes
        \dot{y_n+1} (the derivative of y_n+1 with respect to that parameter).
        """

        generator = functools.partial(_butcher_scheme_generator_tlm, perturbation=perturbation)
        ...
        return MultiStageScheme(new_form, new_solution, self._t, self._bcs, 
                                self.a, self.b, self.c, self._order, generator=generator)
        
    def to_adm(self, adj):
        r"""
        Return another MultiStageScheme that implements the adjoint linearisation of the ODE solver.

        This takes \bar{y_n+1} (the derivative of a functional J with respect to y_n+1) and computes
        \bar{y_n} (the derivative of J with respect to y_n).
        """
