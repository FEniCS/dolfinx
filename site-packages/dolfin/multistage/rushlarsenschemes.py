...
def _rush_larsen_scheme_generator_tlm(rhs_form, solution, time, order, generalized, perturbation):
    ...
    *Arguments*
    ...
    if order == 1:
        ...
        rl_ufl_form = safe_action(derivative(rl_ufl_form, solution[0]), perturbation)

    elif order == 2:
        # Stage solution for order 2
        if vector_rhs:
            fn_space = solution.function_space()
        else:
            fn_space = solution[0].function_space()
        ...
        y_dot_half_form = safe_action(derivative(y_half_form, solution[0]), perturbation)

        y_one_form = _rush_larsen_step(rhs_exprs, diff_rhs_exprs, linear_terms, \
                                        system_size, solution, stage_solutions[0], dt,\
                                        time, 1.0, 0.5, v, DX, time_dep_expressions)

        rl_ufl_form    = safe_action(derivative(y_one_form, solution[0]), perturbation) + \
                         safe_action(derivative(y_one_form, stage_solutions[0]), stage_solutions[1])
...
def _rush_larsen_scheme_generator_adm(rhs_form, solution, time, order, generalized, perturbation):
    ...
    *Arguments*
    ...
    if order == 1:
        ...
        # If this is commented out, we don't get NaNs.
        # why is solution a list of length zero anyway?
        rl_ufl_form = safe_action(safe_adjoint(derivative(rl_ufl_form, solution[0])), perturbation)
        ...
        y_bar_half_form = safe_action(safe_adjoint(derivative(y_one_form, stage_solutions[0])), perturbation)

        rl_ufl_form    = safe_action(safe_adjoint(derivative(y_one_form, solution[0])), perturbation) + \
                         safe_action(safe_adjoint(derivative(y_half_form, solution[0])), stage_solutions[2])
