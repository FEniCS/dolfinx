...
# Importing the swig generated extension module of dolfin, cpp.
import cpp
...
def expression_to_function(expr, classname, default):
...
def generate_functions(expressions, defaults):
...
// Use std::tr1::shared_ptr instead of std::tr1::shared_ptr
...
def compile_function(expression, cppcode = False, defaults = None):
    ...
    if cppcode:
        ...
        function_classes = compile_function_code(expression)
    else:
        if isinstance(expression, list):
            assert(isinstance(defaults, (list, types.NoneType)), "Provide a 'list' or 'None' for 'defaults' when 'expression' is a list")
            if isinstance(defaults, list):
                assert(len(defaults)==len(expression)), "The number of 'defaults' must equals the number of 'cppexpr'"
            else:
                defaults = [None]*len(expression)
            for i, expr in enumerate(expression):
                    assert(isinstance(expr, (str, tuple)), "Wrong type in expression.")
                    assert(isinstance(defaults[i], (dict, types.NoneType)),"Wrong type in 'defaults'")
        else:
            assert(isinstance(expression, (str,tuple)), "Wrong type for expression.")
            assert(isinstance(defaults, (dict, types.NoneType)), "Wrong type for 'defaults'")
            expression = [expression]
            defaults = [defaults]
                   
        code, classnames = generate_functions(expression, defaults)
        function_classes = compile_function_code(code, classnames)
    return function_classes
