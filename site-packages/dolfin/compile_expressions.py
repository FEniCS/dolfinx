...
__date__ = "2008-06-04 -- 2009-10-05"
...
public:
%(members)s
  %(classname)s(%(opt_geometric_dimension_arg)s):Expression(%(geometric_dimensions)s)
  {
%(value_shape)s
%(constructor)s
  }
...
def expression_to_dolfin_expression(expr, defaults, dim):
    "Generates code for a dolfin::Expression subclass for a single expression."

    # Check and flattern provided expression
    expr, shape = list(flatten_and_check_expression(expr))

    # Extract code fragments from the expr and defaults
    fragments = expression_to_code_fragments(expr, defaults, ["v","x"])

    # Check that passed indices to "x" fit with passed geometrical dimensions
    if dim is not None:
        for e in expr:
            for x_str_ind in re.findall(r"x\[(-?[0-9]+)\]", e):
                x_ind = eval(x_str_ind) 
                if x_ind < 0 or x_ind > dim - 1:
                    raise IndexError, "for 'x[ind]' where ind \xe2\x88\x89 [0,%d): '%s'"%(dim,e)

    # Generate code for geometric dimension
    if dim is None:
        opt_geometric_dimension_arg = "int dim"
        dim = "static_cast<dolfin::uint>(dim)"
    else:
        opt_geometric_dimension_arg = ""
        dim = str(dim)
    ...
    fragments["geometric_dimensions"] = dim
    fragments["opt_geometric_dimension_arg"] = opt_geometric_dimension_arg
...
def compile_expressions(cppargs, defaults_list, dim_list = None):
    ...
    assert(isinstance(dim_list, (list, types.NoneType)))
    assert(len(cppargs) == len(defaults_list))
    if dim_list is None:
        dim_list = [None]*len(defaults_list)
    else:
        assert(len(defaults_list) == len(dim_list))
        
    # Check for uniq sub expressions
    if len(set(hashlib.md5(repr(expr) + repr(default) + repr(dim)).hexdigest()\
               for expr, default, dim in zip(cppargs, defaults_list, dim_list))) \
               != len(cppargs):
    ...
    for cpparg, defaults, dim in zip(cppargs, defaults_list, dim_list):
        ...
        if isinstance(cpparg,str) and "class" in cpparg and "Expression" in cpparg:
            ...
            classname, code = expression_to_dolfin_expression(cpparg, defaults, dim)
...
if __name__ == "__main__":
    cn1, code1 = expression_to_dolfin_expression("exp(alpha)",{'alpha':1.5},None)
    cn2, code2 = expression_to_dolfin_expression(("sin(x[0])", "cos(x[1])", "0.0"),{},2)
    cn3, code3 = expression_to_dolfin_expression((("sin(x[0])", "cos(x[1])"), ("0.0", "1.0")),{},2)
