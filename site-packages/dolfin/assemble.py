...
def assemble(form, mesh, coefficients=None, dof_maps=None,
    ...
    if not hasattr(form, "create_cell_integral"):
        ...
        (compiled_form, module, form_data) = _jit(form, form_compiler_options)
    else:    
...
def assemble_system(A_form, b_form, bc, mesh, A_coefficients=None, b_coefficients=None, A_dof_maps=None, b_dof_maps=None,
    ...
    if not hasattr(A_form, "create_cell_integral"):
        # FFC form, call JIT compile
        (A_compiled_form, module, A_form_data) = _jit(A_form, form_compiler_options)
    else:    
        # UFC form, no need to compile
        compiled_form = form
        A_form_data     = None
    ...
    if not hasattr(b_form, "create_cell_integral"):
        # FFC form, call JIT compile
        (b_compiled_form, module, b_form_data) = _jit(b_form, form_compiler_options)
    else:    
        # UFC form, no need to compile
        compiled_form = form
        b_form_data     = None
...
class Function(ffc_Function, cpp_Function):
    ...
    def __init__(self, element, *others):
        ...
        if isinstance(element, (FiniteElement, MixedElement)) and \
                len(others) == 2 and \
                isinstance(others[0], Mesh) and \
                isinstance(others[1], (Vector, GenericVector)):
            mesh = others[0]
            self.x = others[1]
        ...
        elif isinstance(element, (FiniteElement, MixedElement)):
            ffc_Function.__init__(self, element)
