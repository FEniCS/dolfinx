...
__date__ = "2009-08-15 -- 2009-08-17"
...
"""
    ...
    instant_kwargs['system_headers'] = ["cmath", "iostream",
...
_pydolfin_shared_ptr_classes = ["Function","SubDomain","Mesh","DofMap","FiniteElement","FunctionSpace"]
...
def compile_extension_module(code, module_name="",
                            recompile=False,
                            dolfin_import_files=None,
                            additional_declarations="",
                            additional_system_headers=None,
                            use_numpy_typemaps=True):
    """ Compile a PyDOLFIN compatible extension module, based on C++ DOLFIN code"""
    ...
    expect_arg(bool,use_numpy_typemaps,"use_numpy_typemaps")
    additional_system_headers = \
                expect_list_of(str,additional_system_headers,"additional_system_headers")
    dolfin_import_files = \
                expect_list_of(str,dolfin_import_files,"dolfin_import_files")
    ...
    if dolfin_import_files:
        dolfin_include_dir = get_dolfin_include_dir()
        for f in dolfin_import_files:
            if not os.path.isfile(os.path.join(dolfin_include_dir,"dolfin",f)):
                raise OSError, "Could not find installed '%s' DOLFIN file."% f
                
        dolfin_import_files = [os.path.join("dolfin",f) for f in dolfin_import_files]
    ...
    sys.stdout.flush()
    print "Calling DOLFIN just-in-time (JIT) compiler, this may take some time...",
    sys.stdout.flush()
    ...
    shared_ptr_declarations = extract_shared_ptr_declaration(code, _pydolfin_shared_ptr_classes)
    
    declaration_strs["additional_declarations"] += shared_ptr_declarations[0]
    declaration_strs["shared_ptr_declarations"]  = shared_ptr_declarations[1]
...
def extract_shared_ptr_declaration(code, shared_ptr_classes):
    ...
    # shared_ptr classes in PyDOLFIN
    shared_ptr_format = "SWIG_SHARED_PTR_DERIVED(%s,dolfin::%s,dolfin::%s)"
    shared_ptr_declarations = "\n".join([shared_ptr_format%(derived, base, derived) \
                            for derived, base in re.findall(\
                                r"class[ ]+([\w]+)[ ]*: public[ ]+([\w:]+)", code)\
