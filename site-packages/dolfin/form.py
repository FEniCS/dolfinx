...
class Form(cpp.Form):
    ...
    def __init__(self, form, function_spaces=None, coefficients=None, options=None):
        ...
        self.function_spaces  = _extract_function_spaces(self.form_data, function_spaces)
        (self.coefficients, self._compiled_coefficients) = _extract_coefficients(self.form_data, coefficients)

        # Initialize base class
        cpp.Form.__init__(self, self.function_spaces, self.coefficients, self._compiled_form)

def _extract_function_spaces(form_data, function_spaces):
    ...
    if function_spaces is None:
        ...
        if not isinstance(function_spaces, (list, cpp.FunctionSpaces)):
            raise TypeError, function_space_error
        if isinstance(function_spaces, list):
            if len(function_spaces) != form_data.rank:
            ...
            for i in xrange(form_data.rank):
                _function_spaces.push_back(function_spaces)
...
def _extract_coefficients(form_data, coefficients):
    ...
    return (_coefficients, _compiled_coefficients)
