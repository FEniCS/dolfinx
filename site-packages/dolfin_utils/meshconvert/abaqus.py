...
def convert(ifilename, handler):
    ...
    if not vertex_gap and not cell_gap:
        ...
        for c_index, c_data in elems.items():
            ...
            if process_facets:
                ...
                c_data_tmp.sort()
                mesh_editor.add_cell(c_index, np.array(c_data_tmp, dtype=np.uint))
        ...
        for idx, (c_index, c_data) in enumerate(elems.items()):
            ...
            if process_facets:
                c_data_tmp = np.array(c_data)
                c_data_tmp.sort()
                mesh_editor.add_cell(idx, np.array(c_data_tmp, dtype=np.uint))
        ...
        for idx, (c_id, c_data) in enumerate(elems.items()):
            ...
            if process_facets:
                c_nodes.sort()
                mesh_editor.add_cell(idx, np.array(c_nodes, dtype=np.uint))
    ...
    for value, (name, node_set) in enumerate(node_sets.items()):
        ...
        try:
            ...
            handler.error("Boundary references non-existent node %s" % node)
