...
def convert(ifilename, handler):
    ...
    if not vertex_gap:
        ...
        for idx, (v_id, v_coords) in enumerate(list(nodes.items())):
    ...
    if not vertex_gap and not cell_gap:
        ...
        for idx, (c_index, c_data) in enumerate(list(elems.items())):
            for v_id in c_data:
                if not (0 <= v_id < len(nodes)):
                    handler.error("Element %s references non-existent node %s" % (c_index, v_id))
        ...
        for idx, (c_id, c_data) in enumerate(list(elems.items())):
            c_nodes = []
            for v_id in c_data:
    ...
    if len(list(node_sets.items())) > 0:
        node_cell_map = {}
        for c_dolfin_index, (c_index, c_data) in enumerate(list(elems.items())):
            c_data_tmp = np.array(c_data)
            c_data_tmp.sort()
    ...
    dim = 0
    for value, (name, node_set) in enumerate(list(node_sets.items())):
    ...
    if process_facets:
        ...
        for index, (name, s) in enumerate(list(surface_sets.items())):
            cell_face_list = []
