...
from coefficient_set import coefficient_reference_code, generate_coefficient_set_class
...
def generate_dolfin_classes(prefix, formnames):
    ...
    for fn in formnames:
        coefficient_names.update(fn.coefficient_names) # NB! This whole concept will break down if coefficient_names are auto-generated w%d different for each form!
    ...
    sfc_style = False # TODO: To avoid unnecessary amounts of code generation, generate function spaces indpendenly of forms since they are usually shared. This should preferably be fixed in FFC to keep this code simple.
    if sfc_style:
        for name in coefficient_names:
            functionspace_classname = "%sCoefficientSpace_%s" % (prefix, name)
            # TODO: Need to get these names to generate function space class, but need mapping from coefficient name:
            fe = fn.ufc_finite_element_classnames[i] 
            dm = fn.ufc_dof_map_classnames[i]
            
            code = generate_functionspace_class(functionspace_classnames[i], fn.ufc_finite_element_classnames[i], fn.ufc_dof_map_classnames[i])
            blocks.append(code)
    ...
    for fn in formnames:
        ...
        functionspace_classnames    = ["%s_FunctionSpace%d"  % (classname, i) for i in range(fn.rank + fn.num_coefficients)]
        ...
        basespace_classnames        = ["FunctionSpace%d"     % i for i in range(fn.rank)]
        if fn.rank == 1:
        ...
        # Generate FunctionSpace subclasses TODO: Use typedefs for these if function spaces are defined independently of forms
        for i in range(fn.rank + fn.num_coefficients):
            code = generate_functionspace_class(functionspace_classnames[i], fn.ufc_finite_element_classnames[i], fn.ufc_dof_map_classnames[i])
            blocks.append(code)
        ...
        formclass = generate_form_class(classname, fn.ufc_form_classname,
            ...
            coefficient_classnames, fn.coefficient_names, coefficient_set_classname)
...
def generate_dolfin_code(prefix, header, formnames):
    ...
    # Using prefix as namespace and passing no prefix on
    namespace = prefix # "dolfin"
    prefix = ""
...
def _generate_dolfin_code_sfc_way(formdatas, header, prefix, fcprefix):
    ...
    for fd in formdatas:
        ...
        formnames.append(UFCFormNames(fd.name, fd.function_names, ufc_form_classname, ufc_finite_element_classnames, ufc_dof_map_classnames))
...
if __name__ == "__main__":
    uflfilename = "/home/martinal/dev/fenics/ufl/demo/HyperElasticity.ufl" # TODO: Use a relative path to a dolfin demo here
    hfilename = None #"HyperElasticity.h"
    prefix = "HypEl"
    fcprefix = "sfc"
