...
def generate_dolfin_classes(prefix, formnames):
    ...
    for fd in formnames:
        ...
        coefficient_classnames      = ["%s_Coefficient_%s"   % (classname, n) for n in fd.coefficient_names]
        ...
        for i in range(fd.num_coefficients):
            code = generate_coefficient_class(coefficient_classnames[i], functionspace_classnames[fd.rank+i], i, fd.coefficient_names[i])
            blocks.append(code)
