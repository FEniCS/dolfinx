...
def generate_dolfin_classes(prefix, formdatas, ufc_form_classnames, ufc_finite_element_classnames, ufc_dof_map_classnames):
    ...
    ranks = [fd.rank for fd in formdatas]
    ...
    # Handle each form
    for fdi, fd in enumerate(formdatas):
        
        # Form class name
        ufc_form_classname = ufc_form_classnames[fdi]
        ...
        for i, e in enumerate(fd.elements):
            ufc_fe_name = ufc_finite_element_classnames[fdi][i]
            ufc_dm_name = ufc_dof_map_classnames[fdi][i]
            code = generate_functionspace_class(functionspace_classnames[i], ufc_fe_name, ufc_dm_name)
            blocks.append(code)
        
        # Generate Coefficient subclasses
        for i, w in enumerate(fd.coefficients):
        ...
        formclass = generate_form_class(classname, ufc_form_classname,
            functionspace_classnames,
            basespace_classnames, coefficientspace_classnames,
...
def generate_dolfin_code(formdatas, header, prefix, ufc_form_classnames, ufc_finite_element_classnames, ufc_dof_map_classnames):
    "Generate complete dolfin wrapper code with given generated names."
    assert len(ufc_form_classnames) == len(formdatas)
    assert len(ufc_finite_element_classnames) == len(formdatas)
    assert len(ufc_dof_map_classnames) == len(formdatas)
    
    # Generate dolfin wrapper code
    dolfin_classes = generate_dolfin_classes(prefix, formdatas, ufc_form_classnames, ufc_finite_element_classnames, ufc_dof_map_classnames)
    code = "\n".join((header, stl_includes, dolfin_includes, "\nnamespace dolfin\n{\n", dolfin_classes, "\n} // namespace dolfin"))
...
def _generate_dolfin_code_sfc_way(formdatas, header, prefix, fcprefix):
    ...
    # TODO: We can add typedefs in SFC to avoid this remapping of names, or FFC can start reusing elements to reduce its code size.
    
    # Define generated form classnames 
    ufc_form_classnames = ["%s_%s_form_%s" % (fcprefix, prefix, fd.name) for fd in formdatas]
    
    # Define generated element classnames
    ...
    # Map element classnames to numbered lists
    ufc_finite_element_classnames = [[unique_ufc_finite_element_classnames[e] for e in fd.elements] for fd in formdatas]
    ufc_dof_map_classnames = [[unique_ufc_dof_map_classnames[e] for e in fd.elements] for fd in formdatas]
    
    return generate_dolfin_code(formdatas, header, prefix, ufc_form_classnames, ufc_finite_element_classnames, ufc_dof_map_classnames)
...
def _generate_dolfin_code_ffc_way(formdatas, header, prefix, fcprefix):
    ...
    # Map element classnames to numbered lists
    ufc_finite_element_classnames = [["%s_ufc_finite_element_%d" % (fcn, i) for i in range(len(fd.elements))] for (fcn,fd) in zip(ufc_form_classnames, formdatas)]
    ufc_dof_map_classnames = [["%s_ufc_dof_map_%d" % (fcn, i) for i in range(len(fd.elements))] for (fcn,fd) in zip(ufc_form_classnames, formdatas)]
