...
public:
  ...
  // Create standard function space (reference version)
  %(classname)s(const dolfin::Mesh& mesh, const dolfin::SubDomain& periodic_domain):
    dolfin::FunctionSpace(dolfin::reference_to_no_delete_pointer(mesh),
                          boost::shared_ptr<const dolfin::FiniteElement>(new dolfin::FiniteElement(boost::shared_ptr<ufc::finite_element>(new %(ufc_finite_element_classname)s()))),
                          boost::shared_ptr<const dolfin::DofMap>(new dolfin::DofMap(boost::shared_ptr<ufc::dofmap>(new %(ufc_dofmap_classname)s()), mesh, periodic_domain)))
  {
    // Do nothing
  }

  // Create standard function space (shared pointer version)
  %(classname)s(boost::shared_ptr<const dolfin::Mesh> mesh, const dolfin::SubDomain& periodic_domain):
    dolfin::FunctionSpace(mesh,
                          boost::shared_ptr<const dolfin::FiniteElement>(new dolfin::FiniteElement(boost::shared_ptr<ufc::finite_element>(new %(ufc_finite_element_classname)s()))),
                          boost::shared_ptr<const dolfin::DofMap>(new dolfin::DofMap(boost::shared_ptr<ufc::dofmap>(new %(ufc_dofmap_classname)s()), *mesh, periodic_domain)))
  {
