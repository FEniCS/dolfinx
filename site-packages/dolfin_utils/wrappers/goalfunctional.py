...
attach_coefficient_template = """
    ...
    for (dolfin::uint i = 0; i < %(from)s.num_coefficients(); i++)
    {
      ...
      try {
        const dolfin::uint coefficient_number = %(to)s->coefficient_number(name);
      } catch (...) {
...
attach_domains_template = """
    ...
    %(to)s->dx = %(from)s.cell_domains_shared_ptr();
    %(to)s->ds = %(from)s.exterior_facet_domains_shared_ptr();
    %(to)s->dS = %(from)s.interior_facet_domains_shared_ptr();
"""
#-------------------------------------------------------------------------------
update_ec_template = """
