...
def generate_form_constructor(classname, ufc_form_classname, rank, num_coefficients, coefficient_names, shared_V, shared_w, include_coefficients):
    ...
    if rank:
        ...
        for i in range(rank):
            if shared_V:
                a = "boost::shared_ptr<const FunctionSpace> _pV%d" % i
            else:
        ...
        if not shared_V:
            for i in range(rank):
                code = "    boost::shared_ptr<const dolfin::FunctionSpace> _pV%(i)d = dolfin::reference_to_no_delete_pointer(_V%(i)d);" % { "i": i }
                lines.append(code)
            lines.append("")
    ...
    if include_coefficients:  
        ...
        for i in range(num_coefficients):
            if shared_w:
                ...
                a = "boost::shared_ptr<dolfin::Function> _pw%d" % i 
            else:
                # Not using const since Coefficient possibly attaches name and function space.
        ...
        if not shared_w:
            for i in range(num_coefficients):
                # Not using const since Coefficient possibly attaches name and function space.
                code = "    boost::shared_ptr<dolfin::Function> _pw%(i)d = dolfin::reference_to_no_delete_pointer(_w%(i)d);" % { "i": i }
                lines.append(code)
            lines.append("")
