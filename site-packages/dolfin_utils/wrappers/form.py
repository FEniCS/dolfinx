...
__date__ = "2008-11-06 -- 2009-03-16"
...
%(constructors)s
  ...
  void update_coefficients(const %(coefficient_set_classname)s & coefficients)
  {
%(update_coefficients_body)s
  }
...
def generate_form_constructor(classname, ufc_form_classname, rank, num_coefficients, coefficient_names, shared_V, shared_w, include_coefficients, coefficient_set_classname):
    initializerlist = []
    argumentlist = []
    lines = []

    # TODO: Could even derive these names from .ufl files, at least in some cases. Could do the same as with CoefficientSet, make a FunctionSpaceSet? Need reuse across forms!
    ...
    if rank:
        ...
        for name in function_space_names:
            if shared_V:
                a = "boost::shared_ptr<const dolfin::FunctionSpace> _p%s" % name
            else:
                a = "const dolfin::FunctionSpace & _%s" % name
            argumentlist.append(a)
        
        # Wrap references in shared pointers
        if not shared_V:
            for name in function_space_names:
                code = "    boost::shared_ptr<const dolfin::FunctionSpace> _p%s = dolfin::reference_to_no_delete_pointer(_%s);" % (name, name)
                lines.append(code)
            lines.append("")
        
        # Initialize function spaces
        for i, name in enumerate(function_space_names):
            code = "    _function_spaces[%d] = _p%s;" % (i, name)
            lines.append(code)
    ...
    if include_coefficients:  
        ...
        for name in coefficient_names:
            if shared_w:
                # Not using const since Coefficient possibly attaches name and function space. FIXME: This should be fixed in DOLFIN according to email discussion.
                a = "boost::shared_ptr<dolfin::Function> _p%s" % name
            else:
                # Not using const since Coefficient possibly attaches name and function space.
                a = "dolfin::Function & _%s" % name
            argumentlist.append(a)
        ...
        if not shared_w:
            for name in coefficient_names:
                # Not using const since Coefficient possibly attaches name and function space.
                code = "    boost::shared_ptr<dolfin::Function> _p%s = dolfin::reference_to_no_delete_pointer(_%s);" % (name, name)
                lines.append(code)
            lines.append("")
        
        # Initialize coefficients
        for name in coefficient_names:
            code = "    %s = _p%s;" % (name, name)
            lines.append(code)
        
        lines.append("")

    if coefficient_set_classname:
        assert not include_coefficients # would be doubled up
        
        # Generate argument list
        a = "const %s & coefficients" % coefficient_set_classname
        argumentlist.append(a)
        
        # Initialize coefficients
        code = "    update_coefficients(coefficients);"
        lines.append(code)
...
def generate_form_class(classname, ufc_form_classname, functionspace_classnames, basespace_classnames, coefficientspace_classnames, coefficient_classnames, coefficient_names, coefficient_set_classname):
    ...
    for shared_V in ((False, True) if rank else (False,)):
        ...
        code = generate_form_constructor(classname, ufc_form_classname, rank, num_coefficients, coefficient_names, shared_V, False, False, None)
        constructors.append(code)
        
        # generate constructor with coefficients as a CoefficientSet
        code = generate_form_constructor(classname, ufc_form_classname, rank, num_coefficients, coefficient_names, shared_V, False, False, coefficient_set_classname)
        constructors.append(code)
        
        # generate constructors with coefficients
        if coefficient_names:
            for shared_w in (False, True):
                code = generate_form_constructor(classname, ufc_form_classname, rank, num_coefficients, coefficient_names, shared_V, shared_w, True, None)
                constructors.append(code)
    ...
    args = { "classname": classname,
             ...
             "update_coefficients_body": update_coefficients_body,
             "coefficient_set_classname": coefficient_set_classname,
             "members": members }
...
def _test_form_constructors():
    ...
    print generate_form_constructor(classname, ufc_form_classname, rank, num_coefficients, coefficients, shared_V, shared_w, True, None)

    coefficient_set_classname = "MyCoefficientSet"
    print generate_form_constructor(classname, ufc_form_classname, rank, num_coefficients, coefficients, shared_V, shared_w, False, coefficient_set_classname)
...
def _test_form():
    ...
    # Derived input
    rank = formdata.rank
    num_coefficients = formdata.num_functions
    ...
    print generate_form_class(classname, ufc_form_classname, functionspace_classnames, basespace_classnames,
                              coefficientspace_classnames, coefficient_classnames, coefficient_names, coefficient_set_classname)
