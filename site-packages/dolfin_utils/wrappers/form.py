...
def generate_form_constructor(classname, ufc_form_classname, rank, num_coefficients, coefficient_names, shared_V, shared_w):
    argumentlist = []
    lines = []
    ...
    if num_coefficients:  
        ...
        for i in range(num_coefficients):
            if shared_w:
                a = "std::tr1::shared_ptr<const Function> _pw%d" % i
            else:
                a = "const Function & _w%d" % i
            argumentlist.append(a)
        
        # Wrap references in shared pointers
        if shared_w:
            for i in range(num_coefficients):
                code = "    std::tr1::shared_ptr<const dolfin::Coefficient> _pw%(i)d = dolfin::reference_to_no_delete_pointer(_w%(i)d);" % { "i": i }
                lines.append(code)
            lines.append("")
...
def generate_form_class(classname, ufc_form_classname, functionspace_classnames, basespace_classnames, coefficientspace_classnames, coefficient_classnames, coefficient_names):
    ...
    for shared_V in ((False, True) if rank else (False,)):
        ...
        code = generate_form_constructor(classname, ufc_form_classname, rank, num_coefficients, [], shared_V, False)
        constructors.append(code)
        
        # generate constructors with coefficients
        if coefficient_names:
            for shared_w in (False, True):
                code = generate_form_constructor(classname, ufc_form_classname, rank, num_coefficients, coefficient_names, shared_V, shared_w)
                constructors.append(code)
...
def _test_form_constructors():
    ...
    print generate_form_constructor(classname, ufc_form_classname, rank, num_coefficients, coefficients, shared_V, shared_w)
...
def _test_form():
    ...
    # Generate form code
    print generate_form_class(classname, ufc_form_classname, functionspace_classnames, basespace_classnames, coefficientspace_classnames, coefficient_classnames, coefficient_names)
