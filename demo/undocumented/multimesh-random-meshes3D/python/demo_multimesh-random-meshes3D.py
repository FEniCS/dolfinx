...
parser.add_argument('--num_parts', type=int, help='number of meshes', default=1)
...
def build_multimesh(num_parts,
    ...
    while (multimesh.num_parts() < num_parts):
        ...
        N_z_part = int(max(abs(z1-z0)*N_x, 1))
        mesh = BoxMesh(Point(x0, y0, z0), Point(x1, y1, z1),
                             N_x_part, N_y_part, N_z_part)

        s =  "Add new box mesh ({:.3f}, {:.3f}, {:3f}) x ({:.3f}, {:.3f}, {:.3f}).".format(x0, y0, z0, x1, y1, z1)
