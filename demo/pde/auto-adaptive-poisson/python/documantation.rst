...
This demo is implemented in a single Python file, :download:`demo_auto-adaptive_poisson.py`, which contains both the variational forms and the solver.
...
This description goes through the implementation (in :download:`demo-auto-adaptive_poisson.py`) of a solver for the above described Poisson equation step-by-step.
...
We begin by defining a mesh of the domain and a finite element function space V relative to this mesh. As the unit square is a very standard domain, we can use a built-in mesh provided by the class :py:class:`UnitSquareMesh <dolfin.cpp.mesh.UnitSquareMesh>`. In order to create a mesh consisting of 8 x 8 squares with each square divided into two triangles, we do as follows
...
Now, we have specified the variational forms and can consider the solution of the variational problem. First, we need to define a :py:class:`Function <dolfin.cpp.function.Function>` u to represent the solution. (Upon initialization, it is simply set to the zero function.) A Function represents a function living in a finite element function space.
