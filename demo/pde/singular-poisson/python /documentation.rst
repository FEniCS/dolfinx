...
.. _demo_pde_singular-poisson_python_documentation:
...
With :math:`u,v,f` and :math:`g`, we can write down the bilinear form :math:`a` and the linear form :math:`L` (using UFL operators).
...
We specify a :py:class:`Vector <dolfin.cpp.la.Vector>` for storing the result by defining a :py:class:`Function <dolfin.cpp.function.Function>`. 
...
Orthogonalizing b with respect to the null space makes sure that it doesn't contain any component in the null space. 
